...
Same pattern as tiktok_login.py: the user logs in to instagram.com ONCE inside a Chromium
...
# hashtags or username hit obvious adult terms.
NSFW_RE = re.compile(
...
def export_cookies_txt(path=None, cookies=None):
    ...
    if cookies is None:
        # Playwright call - MUST run on the worker thread (see tiktok_login for the
...
def _query_relevance(item, query):
    """Metadata relevance guard for Instagram's occasionally stale GraphQL timeline feed."""
    desc = str(item.get("desc") or "").casefold()
    author = item.get("author") if isinstance(item.get("author"), dict) else {}
    hay = desc + " " + str(author.get("uniqueId") or "").casefold()
    raw = str(query or "").lstrip("#").strip().casefold()
    if not raw:
        return 0.0
    cjk = [token for token in re.findall(r"[\u3040-\u30ff\u3400-\u9fff]{2,}", raw)]
    if cjk:
        hits = sum(1 for token in cjk if token in hay)
        return hits / float(len(cjk))
    stop = {"japan", "japanese", "video", "reel", "reels", "the", "and", "with", "for"}
    words = [w for w in re.findall(r"[a-z0-9'-]{3,}", raw) if w not in stop]
    if not words:
        return 0.0
...
class Session:
    ...
    def _open(self):
        ...
        # shared memory-lean flags (see tiktok_login.LEAN_BROWSER_ARGS): three headed browsers
        # run at once, and this one never shows a frame of the video it would otherwise decode.
        args = (["--disable-blink-features=AutomationControlled", "--no-first-run",
                 "--no-default-browser-check"]
                + _tt.LEAN_BROWSER_ARGS + _tt.gpu_saving_args())
        ...
        self._ctx = self._p.chromium.launch_persistent_context(
            ...
            **_tt._launch_browser_kwargs())
        _tt.install_lean_routing(self._ctx)
        if not visible:
    ...
    def search(self, query, want=12, status_cb=None, max_scrolls=12, timeout_s=None,
        ...
        for item in collected:
            score = _query_relevance(item, query)
            if score <= 0:
                continue
            item["_query_relevance"] = score
            relevant.append(item)
        collected = relevant
        if raw_count and not collected:
            _status(cb, f"Instagram search {query!r}: discarded {raw_count} unrelated/stale Reel(s).")
