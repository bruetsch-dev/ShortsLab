...
GPT55_MODEL = "google/gemini-3.7-flash"
...
def reuse_existing_scrape_clips(project_dir, scenes_override, status_cb=None):
    ...
    for i, sc in enumerate(scenes_override):
        ...
        sc["clip"] = src.name
        sc["seedance"] = True
        sc["visual_role"] = "topic" if i == 0 else "body"
        sc["scrape_source"] = "tiktok"
        sc["scrape_clip_id"] = src.name
        sc["match_class"] = "REUSED_EXISTING"
...
def generate_project_voiceover(script, project_dir, form, status_cb=None):
    ...
    try:
        ...
        if hook and body:
            ...
            raise
        # NO VOICEOVER IS NOT A DEGRADED RUN, IT IS A DEAD ONE. When WaveSpeed withdrew the
        # Gemini TTS models every request came back 400 "Model not found", and the run carried
        # on to plan beats, scrape footage and cut a timeline against ESTIMATED word timing -
        # forty minutes of work and paid vision calls for a video whose captions and cuts have
        # nothing to line up with. One warning line in the log was the only sign. The estimated
        # path exists for a voiceover that is merely IMPERFECT, not for one that is absent.
        log(status_cb, f"Voiceover generation failed: {exc}")
        raise RuntimeError(
            f"Voiceover generation failed and there is no audio to cut against: {exc}") from exc
...
# A beat is a sentence fragment shown under one shot, so where it ENDS is where the picture cuts.
# Measured on the eating-walk Short: the cuts sat within 4 ms of the last word of each beat - the
# timing was exact - but the beats themselves ended on "you must", "completely still beside",
# "dropping crumbs or", "seen as selfishly". A picture change on a word that demands the next word
# reads as unmotivated, which is what "the cuts are not on the voiceover" actually describes.
# Nothing checked this: the deterministic splitter cuts at conjunctions only, and the LLM plan was
# taken at its word. Merging a dangling beat forward can only reduce the beat count, never leave a
# line uncovered, so it is safe to apply to both.
_DANGLING_TAIL = frozenset("""
a an the this that these those my your his her its our their
at by for from in into of on onto to under with without beside behind between during through
across around near about after before against upon
and or but nor so yet because although while when if than as that whether since unless
is are was were be been being am do does did have has had
can could shall should will would may might must let
not never almost just only very quite really rather more most less such every each
i you he she it we they me him us them one there here who which what
und oder aber weil wenn dass der die das ein eine einen dem den des zu im am
""".split())


def beat_tail_dangles(text):
    """Does this beat end on a word that demands the next one?"""
    parts = re.findall(r"[\w'’-]+[.!?,;:]?", str(text or "").strip())
    if not parts:
        return False
    last = parts[-1]
    if last[-1:] in ".!?":
        return False                      # a finished sentence never dangles
    word = re.sub(r"[^\w'’-]", "", last).lower()
    if not word:
        return False
    if word in _DANGLING_TAIL:
        return True
    # an adverb in mid-sentence is nearly always premodifying what comes next
    return len(word) > 4 and word.endswith("ly")


def repair_beat_boundaries(units, min_words=3):
    """Move a dangling boundary later instead of dissolving the beat.

    Merging the whole beat forward also fixes the grammar, but it collapses the cut rhythm - on
    the eating-walk Short it turned fourteen beats into nine, one of them twenty words long. A
    short-form edit lives on that rhythm. So the trailing function words are handed to the NEXT
    beat instead: "at a crowded night market, you must" + "stop and stand..." becomes
    "at a crowded night market," + "you must stop and stand...". Same number of cuts, each one
    now landing where a phrase actually ends.
    """
    out = [str(u).strip() for u in (units or []) if str(u).strip()]
    for i in range(len(out) - 1):
        guard = 0
        while beat_tail_dangles(out[i]) and guard < 6:
            guard += 1
            head = out[i].split()
            if len(head) <= min_words:
                # cannot be fixed by shifting - hand the whole beat to its neighbour instead
                out[i + 1] = (out[i] + " " + out[i + 1]).strip()
                out[i] = ""
                break
            out[i] = " ".join(head[:-1]).rstrip(" ,;:-")
            out[i + 1] = (head[-1] + " " + out[i + 1]).strip()
        if not out[i]:
            out[i + 1] = (out[i] + " " + out[i + 1]).strip()
    # a beat emptied by the loop, and a dangling last beat, fold into their neighbour
    out = [u for u in out if u]
    while len(out) > 1 and beat_tail_dangles(out[-1]):
        tail = out.pop()
...
def split_micro_units(text):
    ...
    for unit in units:
        if merged and len(re.findall(r"\w+", unit)) <= 3:
            ...
            merged.append(unit)
    return repair_beat_boundaries(merged) or [text]
...
# A picture under ~1.6s is gone before the viewer has read it, and the opener is the only shot
# that has to survive a scroll. Reference Short measured at 2.0s of hook across three shots.
HOOK_MIN_ON_SCREEN_S = 1.6
# ...but never starve the beat the time is taken from.
MIN_BEAT_ON_SCREEN_S = 1.0


class EditorialPlanError(RuntimeError):
    """The edit map cannot support an honest, visibly progressive cut."""


def adjacent_duplicate_editorial_actions(beats):
    """Indexes whose visible contract merely repeats the preceding beat.

    This is deliberately about the action/result pair, not shared subjects or sequence IDs. A
    process may stay in one restroom or kitchen, but each cut still needs to advance the action.
    """
    duplicates = []
    previous = None
    for index, beat in enumerate(beats or []):
        if not isinstance(beat, dict):
            ...
            continue
        key = (
            re.sub(r"\s+", " ", str(beat.get("required_action") or "").strip().casefold()),
            re.sub(r"\s+", " ", str(beat.get("required_result") or "").strip().casefold()),
        )
        if key[0] and previous == key:
            duplicates.append(index)
        previous = key if key[0] else None
    return duplicates


def merge_adjacent_duplicate_editorial_actions(beats):
    """Merge repeated adjacent contracts into one honest visual hold.

    The correction prompt usually supplies a different observable moment.  If it cannot, the
    correct fallback is one longer beat for that one visible moment, never an aborted production
    run and never two pretend cuts to the same action.  The spoken spans stay contiguous and the
    merged beat ends where the final repeated beat ended.
    """
    merged = []
    for source in beats or []:
        ...
        if not merged:
            ...
            continue
        prior = merged[-1]
        repeated = adjacent_duplicate_editorial_actions([prior, beat]) == [1]
        if not repeated:
            merged.append(beat)
            continue
        text = clean_text(" ".join([
            str(prior.get("exact_voice_text") or prior.get("voice_line") or prior.get("script") or ""),
            str(beat.get("exact_voice_text") or beat.get("voice_line") or beat.get("script") or ""),
        ]))
        for key in ("exact_voice_text", "voice_line", "script"):
            prior[key] = text
        for key in ("end", "end_time"):
            if key in beat:
                prior[key] = beat[key]
        queries = []
        for query in list(prior.get("search_queries") or []) + list(beat.get("search_queries") or []):
            value = str(query).strip()
            if value and value.casefold() not in {item.casefold() for item in queries}:
                queries.append(value)
        prior["search_queries"] = queries[:3]
        if not prior.get("sequence_id") and beat.get("sequence_id"):
            prior["sequence_id"] = beat["sequence_id"]
    return merged
...
def normalize_micro_beat_plan(raw_beats, title, script, target_duration):
    if not isinstance(raw_beats, list):
        return []
    # THE MODEL IS NOT ASKED WHERE THE PICTURE MAY CUT, only what each beat says - and it ends
    # beats on "you must", "still beside", "crumbs or". Since the cut lands on the beat's last
    # word, that is a picture change on a word that demands the next one. Repair the boundaries
    # before anything is built from them; the beat count is kept, only where one ends moves.
    _texts = [clean_text(str((b or {}).get("exact_voice_text") or (b or {}).get("voice_line")
                             or (b or {}).get("script") or ""))
              for b in raw_beats if isinstance(b, dict)]
    if len([t for t in _texts if t]) == len(_texts) and len(_texts) > 1:
        _fixed = repair_beat_boundaries(_texts)
        # THE LENGTH GUARD KILLED THE ONLY THING THIS DOES. It used to require the repair to
        # return exactly as many beats as it was given, to protect the cut rhythm. But when a
        # fragment is too short to fix by shifting a word, repair_beat_boundaries MERGES it into
        # its neighbour - so the repair returned one fewer, the guard threw the whole result
        # away, and the beats shipped dangling. Measured on the run that finished at 04:13:
        # fifteen beats in, eight of them ending on "almost never", "you must", "until you",
        # "crumbs or"; the repair produced fourteen clean ones and every one was discarded.
        # A merge is a real outcome, so the merged beats' TIMES have to merge with them: the
        # group keeps the first beat's start and the last one's end.
        if _fixed and _fixed != _texts:
            # MAP BY WORDS, NOT BY CHARACTERS. The repair only ever moves whole words across a
            # boundary or merges two beats, so the word SEQUENCE is conserved and the word index
            # is an exact ruler. Matching on string length instead let the grouping drift: it
            # over-consumed early beats and left the last four sharing a single zero-length
            # instant at the end of the video.
            _orig_words = [len(t.split()) for t in _texts]
            _bounds, _run = [], 0
            for _n in _orig_words:                      # word index where each original ends
                _run += _n
                _bounds.append(_run)
            _groups, _seen = [], 0
            for _t in _fixed:
                _first_word = _seen
                _seen += len(_t.split())
                _take = [_i for _i, _end in enumerate(_bounds)
                         if _end > _first_word and _end - _orig_words[_i] < _seen]
                _groups.append((_take, _t))
            if _seen == sum(_orig_words) and all(g for g, _ in _groups):
                _merged = []
                for _idx, _t in _groups:
                    _first = dict(raw_beats[_idx[0]])
                    _last = raw_beats[_idx[-1]]
                    _first["exact_voice_text"] = _first["voice_line"] = _first["script"] = _t
                    for _k_end in ("end_time", "end"):
                        if _k_end in _last:
                            _first[_k_end] = _last[_k_end]
                    _merged.append(_first)
                raw_beats = _merged
    normalized = []
    last = 0.0
        ...
        normalized.append(
            {
                ...
                # A FRAGMENT IS NOT A CLAIM. These two defaults turned every unplanned beat into
                # an action contract whose required_action was its own slice of the sentence, so
                # the reviewer was asked to find footage evidencing "Japanese people almost" or
                # "or spilling sauce on the pavement is seen". It correctly answered that those
                # are not actions - ten of fourteen scenes in one export - and the whole
                # assignment then had nothing it could accept. A beat the planner did not give an
                # action for is a beat with nothing specific to prove: that is what `context` is,
                # and context asks only that the setting fits.
                "editorial_role": item.get("editorial_role")
                                  or ("action" if str(item.get("required_action") or "").strip()
                                      else "context"),
                "sequence_id": str(item.get("sequence_id") or ""),
                "required_action": str(item.get("required_action") or "").strip(),
                "required_result": str(item.get("required_result") or ""),
                # The searches the beat wrote for itself, and the qualifiers that narrow them
                # without being visible. Dropping these here would leave the scraper building
                # queries out of the contract sentence again, which is where "aerial drone
                # complex multi" came from.
                "search_queries": [str(q).strip() for q in (item.get("search_queries") or [])
                                   if str(q).strip()][:3],
                "search_context": str(item.get("search_context") or "").strip(),
                "micro_beat": True,
            }
        )
        last = normalized[-1]["end"]
    # THE OPENER HAS TO REGISTER. Measured against a reference Short: its hook holds 2.0s across
    # three shots, ours held 1.1s on one - and that one was a dancer with nothing to do with the
    # subject, which Gemini named as the video's worst problem. A picture under ~1.6s is gone
    # before the viewer has read it, and the hook is the only shot that has to survive a scroll.
    # The riser is a SOUND that peaks on the impact word; it never had to decide how long the
    # first PICTURE stays up. Take the time from the second beat, which is still covered by its
    # own narration and merely starts a little later.
    if len(normalized) > 1:
        first, second = normalized[0], normalized[1]
        short_by = HOOK_MIN_ON_SCREEN_S - (float(first["end"]) - float(first["start"]))
        room = (float(second["end"]) - float(second["start"])) - MIN_BEAT_ON_SCREEN_S
        take = min(short_by, room)
        if take > 0.05:
            first["end"] = round(float(first["end"]) + take, 2)
            second["start"] = round(float(first["end"]), 2)
    return normalized
...
_TOPIC_KEY_STOP = {"the", "a", "an", "of", "in", "on", "for", "and", "about", "why", "how",
                   "what", "is", "are", "do", "does", "japan", "japanese", "shorts", "short",
                   "video", "facts", "fact", "things", "thing"}


def _topic_key_tokens(topic):
    """Normalised token set for a requested topic: case, plurals and word order do not matter.

    History matching used exact string equality, so "japanese high schools" and "japanese high
    school" were two unrelated topics and the avoid-list for one was invisible to the other -
    which is how the same three school facts kept coming back."""
    import re as _re
    words = _re.findall(r"[a-z0-9]+", str(topic or "").casefold())
    out = set()
    for word in words:
        if len(word) > 3 and word.endswith("ies"):
            word = word[:-3] + "y"
        elif len(word) > 3 and word.endswith("es") and word[-3] in "sxzh":
            word = word[:-2]
        elif len(word) > 3 and word.endswith("s") and not word.endswith("ss"):
            word = word[:-1]
        if word and word not in _TOPIC_KEY_STOP:
            out.add(word)
    return out


def _same_topic_history(recent, topic):
    """Past runs of the SAME subject, however the user happened to phrase it."""
    want = _topic_key_tokens(topic)
    if not want:
        # a topic that is nothing but stop words ("japan") has no distinguishing tokens; fall
        # back to the literal string so it still remembers its own past runs
        needle = str(topic or "").strip().casefold()
        return [row for row in recent if isinstance(row, dict) and row.get("script")
                and str(row.get("requested_topic") or row.get("topic") or "").strip().casefold()
                == needle] if needle else []
    rows = []
    for row in recent:
        if not isinstance(row, dict) or not row.get("script"):
            continue
        have = _topic_key_tokens(row.get("requested_topic") or row.get("topic") or "")
        if not have:
            continue
        overlap = len(want & have) / float(len(want | have))
        # a clear majority of the meaningful words in common, in either direction
        if overlap >= 0.5 or want <= have or have <= want:
            rows.append(row)
    return rows


def _trim_script_history(rows, per_topic=10, total=150):
    """Keep the history per SUBJECT, not just the last N overall.

    The flat 24-entry cap meant twenty runs of one broad topic evicted every other topic's
    memory, so the avoid-list for "japanese high schools" was empty again the next time it was
    asked for and the same three school facts came back. Each subject keeps its own recent
    window; the global cap only bounds the file.
    """
    kept, per = [], {}
    for row in reversed(rows):                      # newest first
        if not isinstance(row, dict):
            continue
        tokens = _topic_key_tokens(row.get("requested_topic") or row.get("topic") or "")
        key = " ".join(sorted(tokens)) or str(row.get("requested_topic") or "").strip().casefold()
        if per.get(key, 0) >= per_topic:
            continue
...
def _script_creator_history():
    try:
        ...
        return [item for item in data if isinstance(item, dict)][-150:]
    except Exception:
        return []
...
_EXPLAINER_TOPIC_MARKERS = (
    "chip", "chips", "semiconductor", "semiconductors", "tsmc", "wafer", "foundry",
    "taiwan", "china", "chinese government", "beijing", "cross-strait", "geopolit",
    "geopolitical", "sanction", "export control", "supply chain", "trade war", "military",
    "invasion", "threaten", "threat", "economy", "economic", "technology", "tech industry",
)


def is_explainer_topic(topic):
    """Whether a topic needs an analysis script with illustrative social-video B-roll.

    This is deliberately a routing decision, not a source-of-truth decision: TikTok may supply
    good footage of the concrete things named in an explainer, but it never establishes the
    underlying comparison or political explanation.
    """
    normalized = re.sub(r"\s+", " ", clean_text(topic or "").casefold())
    return bool(normalized and any(marker in normalized for marker in _EXPLAINER_TOPIC_MARKERS))


def generate_viral_script(topic="", status_cb=None, instructions="", format_mode="standard",
                          token_limit=None, reasoning_model=None, footage_source="scrape"):
    ...
    explainer_topic = is_explainer_topic(topic)
    system = (
        "You are an elite short-form scriptwriter for factual TikTok/Shorts narration. "
        ...
        "Never claim all Japanese people or all women behave one way; say 'many', name the "
        "setting, or explain the specific norm whenever scope varies. "
        ...
        "documented fact or practice. Never invent statistics - use a number only when it is "
        ...
        "ONE SUBJECT, UNPACKED - NEVER A LIST OF SEPARATE THINGS. Measured against a reference "
        "Short that works: it spends five beats on ONE place - a shop where taking things is "
        "allowed - the rule, its name, the sixty seconds, the catch, the consequence. Every "
        "beat is the same subject seen closer, so all its footage comes from one location and "
        "nothing on screen ever jumps. A 'three things' script does the opposite: three "
        "unrelated subjects force the edit into a new world twice, and a third of the footage "
        "then reads as unrelated no matter how good each clip is. So: pick ONE surprising "
        "thing and open it up. Beat 1 states it, and every later beat adds the detail a viewer "
        "would ask for next - what is it called, how does it work, what is the catch, what "
        "happens if you get it wrong, who does this to themselves. Each sentence must be "
        "answerable with footage of the SAME place or the SAME action as the one before it. "
        "'First' and 'then' are fine when they order the STEPS of one action - wash, carry the "
        "towel, fold it, soak. They are forbidden when they introduce a different subject: "
        "'then, a taxi driver...' after two beats about a train is the seam of a list, and it is "
        "the point where the footage has to jump and the viewer stops following. "
        "THE FIRST SENTENCE PAYS FOR THE WHOLE VIDEO. It is ONE strange, concrete, checkable "
        "claim - the single oddest true thing about this subject - stated flat, as a fact. Not a "
        "setup, not a promise, not a contrast. Measured on our own finished Shorts: 'Tokyo train "
        "pushers only shove you if you board butt first', 'slicing this Japanese fish legally "
        "requires a heavy padlocked metal box' and 'capsule hotels will kick you onto the street "
        "every single morning' all work - each names an object or an action you can picture in "
        "one second and each is odd enough to argue with. 'Japanese convenience stores look "
        "effortless, but the 3am shift is ruthless' does not: it names no object, no number and "
        "no action, so there is nothing to see and nothing to disbelieve. BANNED HOOK SHAPES: "
        "'X looks Y, but Z' - 'here is what nobody tells you about X' - 'X is not what you "
        "think' - 'X is more than it seems'. All four announce that a surprise is coming instead "
        "of delivering one. "
        "AND THE SCRIPT HAS TO GO SOMEWHERE. The last beat is the payoff - the consequence, the "
        "cost, the thing the viewer did not see coming - never the last item on a list. The "
        "convenience-store script above ended on 'they twist every bottle so the labels face "
        "forward', which is a chore, so a viewer who left halfway lost nothing. Each beat raises "
        "the stakes on the one before it: what it is, how far it actually goes, what happens when "
        "someone gets it wrong, what it costs the person doing it. If your final sentence could "
        "be swapped with your third and nothing would feel lost, the script has no shape yet. "
    ...
    if explainer_topic:
        system += (
            "TOPIC OVERRIDE: this is a technology, economics or geopolitics explainer, not a "
            "culture-fact video. Do not force it into a Japan example, a social custom or a "
            "national stereotype. Follow the user's named actors and frame the explanation around "
            "a specific, defensible comparison or question. "
        )
    # A scraped run edits over footage OTHER people filmed, so a true-but-unfilmable beat is
    # worthless: the Tokyo future short asked for holographic convenience-store registers and the
    # inside of a sealed underground bicycle silo, and six of its fifteen beats could never be
    # covered. A generated run renders whatever it is told, so the constraint would only limit it.
    if str(footage_source or "scrape").strip().lower() not in ("generate", "generated", "ai") and explainer_topic:
        system += (
            "EXPLAINER VISUAL REALITY: this is an explanatory Short cut from real social-video "
            "B-roll, including TikTok. The footage illustrates the narration; it is not the proof "
            "for an economic, technical or political claim. Build every sentence around a concrete "
            "searchable visual motif: a wafer, fab exterior, cleanroom worker, chip package, cargo "
            "port, map, public speech, military exercise, factory floor or other thing people "
            "actually film. Keep the analytical claim and the visual motif connected, but never "
            "pretend a random clip proves a motive, a capability comparison or a future outcome. "
            "For comparisons, state the dimension being compared instead of saying one country or "
            "company is simply 'better'. For geopolitics, name the institution or government making "
            "a claim; never turn it into a claim about an entire population. Do not predict war or "
            "state disputed motives as settled fact. "
        )
    elif str(footage_source or "scrape").strip().lower() not in ("generate", "generated", "ai"):
        system += (
            "FOOTAGE REALITY: this narration is edited over REAL phone footage other people already "
            "filmed and posted. Every sentence must describe something an ordinary person can point a "
            "phone at and would actually upload: a visible action, a machine someone operates, a place "
            "you can stand in. Never write a beat whose picture exists only in a manufacturer render, a "
            "sealed interior, a press demo, a prototype or a concept - if you cannot picture a real "
            "TikTok showing exactly that moment, the claim may be true and is still unusable, so choose "
            "a different one. Always prefer the moment a PERSON does something over the mechanism that "
            "follows: 'she slides the plate into the slot' is filmable, 'the plates are flushed away "
            "through underground pipes' is not. "
            "THE TEST IS NOT 'CAN A PHONE POINT AT IT' - IT IS 'DOES ANYONE POST IT'. Both are "
            "measured, from two runs of this exact pipeline. WORKED: a meal-ticket machine being "
            "operated, a suitcase handed across a hotel counter, snow melting on a heated road - "
            "every one is a person doing something visible in a public place, and the footage came "
            "back 7-9 out of 10 on relevance. FAILED COMPLETELY: printing a residence certificate "
            "at a convenience-store copier, an app-booked luggage-storage service, a QR-code "
            "umbrella share - 519 search results, and not one beat scored above 5.8, because those "
            "things are posted as explainer videos with text cards and screenshots, never as "
            "filmed action. So: REJECT a fact whose core is a service, an app, a booking, a "
            "subscription, a piece of administration or anything whose outcome is a document. "
            "PREFER a fact whose core is a machine somebody operates, a physical object changing "
            "hands, a place people move through, or a visible reaction on a face. If the most "
            "interesting moment of your fact happens on a phone screen or on paper, it is the "
            "wrong fact. "
            "AND THE PLACE MUST ALLOW A CAMERA. Measured 2026-09-02: a Short about onsen bathing "
            "etiquette searched Japanese phrases correctly, inspected 169 sources and had 6% of "
            "them accepted - against 20% for the same pipeline on convenience stores - because "
            "nobody films inside a naked bathhouse. Changing rooms, bath halls, hospital wards, "
            "school toilets, police stations and anywhere phones are banned or people are "
            "undressed will return staged re-enactments and talking heads, never the moment. If "
            "the moment happens where a phone would not be tolerated, pick a different moment. "
            "PHYSICAL IS NOT ENOUGH - IT MUST BE THE REASON PEOPLE FILM THE PLACE. Measured on a "
            "capsule-hotel run: the beats that matched were the pod itself, the pull-down curtain "
            "and the corridor of capsules - what everyone who visits shows off. The beats that "
            "failed were all SUPPORTING INFRASTRUCTURE: scanning a wristband to unlock the lift, "
            "the gender-segregated floor, chaining a suitcase inside the luggage room. Every one "
            "of those is physical, filmable and really there, and not one of them cleared the "
            "match floor, because nobody points a camera at the security arrangements of a place "
            "they are excited about. So before you keep a fact, ask what a visitor would actually "
            "film and post about this subject, and write about THAT. Safety systems, access "
            "control, storage, back-of-house procedure and anything whose point is that it "
            "prevents a problem are the wrong facts even when you can picture them perfectly. "
            "EVERY WORD YOU WRITE IS SPOKEN ALOUD. Everything above describes what footage "
            "EXISTS - it is briefing, not vocabulary for the script. Never address the camera, "
            "the editor, or the viewer's attention: no 'now film the payoff', no 'cut to', no "
            "'here we see', no shot descriptions. Measured leak: a run on Tokyo train etiquette "
            "opened a paragraph with 'Now film the payoff:' and the narrator read those words "
            "out loud in the finished video. Write only the sentence the audience hears.")
    if topic:
        ...
        same_topic = _same_topic_history(recent, topic)[-14:]
        # ONE LINE PER PAST RUN. Handing the model 420 characters of each old script buried the
        # ban in prose: it wrote "A Japanese high school classroom can become a haunted house"
        # and then, three runs later, "In Japan, a high school classroom can become a haunted
        # house". The first sentence of a Fact Short IS its fact, so a list of just those is
        # short enough for the model to actually check itself against.
        _used = []
        for _row in same_topic:
            _first = re.split(r'(?<=[.!?])\s+', str(_row.get('script') or '').strip())[0].strip()
            if _first and _first not in _used:
                _used.append(_first)
        avoid = '\n'.join('- ' + line for line in _used)
        ask = (
            f'USER-LOCKED TOPIC: "{topic}". The complete script MUST be specifically about this exact topic. '
            "Do not replace it with a familiar culture-fact, school, dating, or social-rules topic unless "
            "the user explicitly named that subject. Keep the country, company, comparison and time frame "
            ...
            "OBVIOUS-FIRST IS BANNED. Before choosing, name to yourself the five facts about this "
            "topic that every Japan video has already used - they are the first things that come "
            "to mind, which is exactly why the viewer has seen them three times this week. You may "
            "use NONE of those five. Write the fact somebody who has already watched fifty Japan "
            "videos has not seen: a specific rule, object, moment or number from inside the "
            "subject, not its famous summary."
            + ("\nA broad topic has many corners - school life alone covers entrance "
               "exams, club hierarchy, commuting, lunch duty, uniforms, festivals, part-time "
               "work bans, graduation rituals and after-school businesses. Pick a corner none "
               "of the recent outputs below touched, not a new angle on the same corner."
               "\nRECENT OUTPUTS FOR THIS TOPIC - actively choose different facts, hook "
               f"and structure:\n{avoid}" if avoid else "")
        )
        if explainer_topic:
            ask += (
                "\nANALYSIS TOPIC: make one clear explanatory argument, then unpack it in a fair "
                "sequence: the concrete thing being compared or disputed, the mechanism behind it, "
                "and the consequence. Use simple words, define one technical term only if needed, "
                "and avoid universal rankings, unsupported numbers and dramatic predictions. Each "
                "sentence must retain a concrete TikTok-searchable visual motif for the editor."
            )
        temperature = 0.92
    else:
        # The retention brief steers an AUTOMATIC pick - its own wording says "automatic Fact
        # Shorts" - and it was being applied to user-locked topics as well. On a subject as wide
        # as "japanese high schools" it collapsed the whole space onto one corner every time
        # (hair rules, uniforms, confession culture) no matter what the avoid-list said.
        system += (
            "HIGH-RETENTION EDITORIAL BRIEF: prioritize intimate everyday social behaviour "
            "people recognize instantly: couples, dating, public affection, friendship, beauty, "
            "masks, makeup, privacy and the small pressures young women and men navigate. Build "
            "these as a visible behaviour, then the real social reason behind it. ")
...
- SELF FACT-CHECK before returning: re-read every sentence and rewrite any claim a well-informed
  viewer would call false - a custom presented as law, an invented consequence, an invented
  number, an unproved causal claim, or a rare edge case presented as universal.
...
numbers from the script, spelled EXACTLY as written in the script"]}}"""
    ...
    for attempt in range(3):
        ...
        # The ceiling must cover the model's REASONING as well as the script, and this prompt is
        # long (the footage briefing, the editorial lens, the recent-script history). Measured
        # 2026-09-02 on openai/gpt-5.6-luna with the real prompt: at max_tokens 300 and at 1200
        # the reply came back finish_reason="length" with message.content EMPTY on all three
        # attempts - which surfaced to the user as "Script creator returned no usable script".
        # 4000 returns a script first try. (A trimmed toy prompt succeeded at 600, which is why
        # the old `max(280, token_limit * 2)` looked sufficient and was not: a 150-token limit
        # produced a fatal 300.)
        model_output_tokens = max(4000, token_limit * 6)
    ...
    try:
        # A stub-length answer is a test double or a broken response, never something worth
        # teaching the next run to avoid. Eighteen such rows had already displaced real history.
        if len(script.strip()) < 40:
            raise ValueError("stub-length script is not recorded in the topic history")
        ...
        _SCRIPT_TOPIC_HISTORY.write_text(
            json.dumps(_trim_script_history(recent), ensure_ascii=False, indent=1),
            encoding="utf-8")
    except Exception:
        pass
...
def regenerate_script_from_reference(original_script, instructions="", status_cb=None,
    ...
    # Same reason as the script creator: a rewrite that reads better but asks for footage nobody
    # films leaves the scraper with nothing to cover it with.
    footage_rule = ("" if str(footage_source or "scrape").strip().lower() in
                    ("generate", "generated", "ai") else
                    '- FOOTAGE REALITY: this narration is edited over REAL phone footage other people already filmed and posted. Every sentence must describe something an ordinary person can point a phone at and would actually upload - a visible action, a machine someone operates, a place you can stand in. Never write a beat whose picture exists only in a manufacturer render, a sealed interior, a press demo, a prototype or a concept. Prefer the moment a PERSON does something over the mechanism that follows.\n')
...
{footage_rule}- Unless the user requests another format or length, stay close to the original word count.
...
DUAL_THINKER_MODEL = "google/gemini-3.7-flash"
DUAL_CRITIC_MODEL = "google/gemini-3.7-flash"
...
def collaborate_json(messages, max_tokens=1800, temperature=0.15, timeout=180,
    ...
    log(status_cb, f"Collaborative {label}: {thinker} drafts -> {critic} reviews...")
    proposal = None
    try:
...
def comprehend_script(title, script, reasoning_model=None, collaborate=False, status_cb=None):
    ...
    try:
        if collaborate:
            data = collaborate_json(messages, max_tokens=900, temperature=0.2, status_cb=status_cb, label="script comprehension", thinker=reasoning_model or "google/gemini-3.7-flash", critic=reasoning_model or "google/gemini-3.7-flash") or {}
        else:
...
def llm_micro_beat_plan(title, script, visual_script, target_duration, base_scenes,
                        reasoning_model=None, status_cb=None, collaborate=False,
                        footage_source="generate"):
    if not os.environ.get("WAVESPEED_API_KEY"):
        return []
    scraping = str(footage_source or "generate").strip().lower() in ("scrape", "scraped", "social")
    if scraping:
        # EVERY BEAT COSTS ONE FOUND VIDEO. Measured 2026-09-03: a 30s script planned as 24 beats
        # became 14 chapters, the run found 7 distinct usable sources, and 8 shots were filled
        # with footage found for another sentence. Beats are the demand side of that shortage.
        pacing = (
            "HOW MANY BEATS YOU MAY SPEND. The pictures for this video are NOT generated. Each "
            "beat is filled with a separate real video that a stranger filmed and posted, which "
            "then has to be found by search, downloaded, and passed by a reviewer that rejects "
            "roughly nine candidates in ten. A beat is therefore expensive, and a beat nobody "
            "has filmed comes out as an unrelated clip sitting under your line.\n"
            "HOW LONG A SHOT MAY BE. 2.55 SECONDS IS THE AVERAGE, not the ceiling to aim at. A "
            "long source can always give a longer excerpt, so length is never a technical limit - "
            "it is a retention one. Measured across five finished Shorts (2026-09-04): medians of "
            "3.7 to 5.1 seconds and single shots of 6.3, 7.0 and 7.9, and they read as dead. A "
            "reference Short that holds attention runs a 1.14s median and 13 shots in 20 seconds.\n"
            "Match the measured Trash reference: aim for 2.0 to 3.2 seconds a beat, averaging "
            "about 2.55, which gives a 30-33 second script roughly 12 to 14 beats. Go past 3.2 "
            "seconds only for a real reveal or a payoff, "
            "never because a sentence is long. When a sentence is long, split it into the SAME "
            "action seen differently - the hands instead of the room, the reaction instead of the "
            "act, the object after the person. Two shots of one action are two beats, and that is "
            "what a montage is made of. Merging is only for lines that have no picture of their "
            "own.\n"            "BEFORE YOU WRITE A BEAT, NAME THE VIDEO IT NEEDS. If you cannot picture an ordinary "
            "phone clip that somebody would actually have uploaded - a machine being operated, an "
            "object changing hands, a room full of people doing something, a face reacting - then "
            "that beat has no footage and must be merged into its neighbour. Abstract lines "
            "(a rule, a feeling, a consequence, a statistic) do not get their own beat: they play "
            "over the concrete shot next to them.\n")
    else:
        pacing = (
            "Split the voice script into micro-beats, usually 0.8 to 2.2 seconds, but keep timing "
            "coherent with the provided timed base scenes. MEASURED: a reference Short that holds "
            "attention runs a 1.14s MEDIAN shot and 13 shots in 20 seconds. Our own render came "
            "back at a 2.67s median over 33 seconds and read as slow and disconnected. A beat "
            "over ~2.2s is a held picture and needs a reason - a reveal, a face, a payoff. Do not "
            "let a long sentence become one long beat: give it two or three beats that each show "
            "a different part of the SAME action, which is how the reference gets 3 shots out of "
            "one line.\n")
    prompt = (
        "You are cutting a vertical Short. Build its edit map before any media exists.\n"
        "The voice script is not background narration - it IS the edit. Every beat has to be "
        "justified by the exact words spoken over it, and the viewer should follow the same idea "
        "with the sound off. Captions are burned in word-by-word and frame-synced to the voice, "
        "so cuts land on spoken words rather than between sentences.\n"
        "Beat 1 is the hook. It carries the single strongest, most curiosity-provoking image in "
        "the video and has to read in the first second. Across neighbouring beats vary the shot "
        "scale and the kind of image so the video never feels like a slideshow of one idea, and "
        "let the tension build toward a payoff rather than running flat.\n"
        + pacing +
        "Voice script = content and timing. Visual script = image ideas and mood. If they "
        "disagree, the voice line wins; if the visual script offers a good picture, take it.\n"
        "No generic cinematic filler. No stock-looking people, rooms, maps, books or props that "
        "do not answer the exact line being spoken.\n"
        ...
        "Plan sequences before shots: adjacent beats continuing the same event share sequence_id. "
        "Use editorial_role=context for setting, action for visible events, payoff for their result. "
        "Abstract claims about price, popularity, customs, scale, or comparisons are context: "
        "use relevant illustrative footage of the specific subject, without inventing a transaction "
        "or demanding visible numerical proof. Keep required_result empty for context. "
        "A narration beat may be illustrated by multiple short shots; action and result need not "
        "occur in one continuous source shot. "
        "Name required_action and required_result (empty when none); require visible evidence for "
        "actions/results. Prefer successive moments of the same person/place over unrelated new "
        "sources. Source variety is subordinate to continuity; do not invent unseen actions. "
        "Every adjacent beat must advance to a visibly different moment. Never copy the same "
        "required_action into adjacent beats just because one sentence was split: either keep that "
        "voice span in one beat, or specify the next observable step (approach, inspect, touch, lock, "
        "change, react, leave). A new camera angle of the same unchanged object is not progression.\n"
        # THE BEAT WRITES ITS OWN SEARCHES. Everything downstream can only rearrange the words it
        # is given: with no search_queries the scraper had to build them out of the contract
        # sentence, and a contract sentence leads with the camera ("Aerial drone shot descending
        # over a complex multi-way Japanese intersection"), so the searches came out as "aerial
        # drone complex multi". The model that wrote the sentence can write the search instead.
        "search_queries: 2-3 short phrases somebody would actually type to FIND this shot - the "
        "subject, the action that matters, and the place when it narrows the result. Write them "
        "as phrases, not sentences: 'Tokyo intersection with traffic lights', 'Japan traffic "
        "light power outage', 'customer returning empty skewer to vendor'. No camera words "
        "(aerial, drone, close-up, POV, shot, footage), no platform names.\n"
        # A DATE OR A CAUSE IS NOT SOMETHING THE PICTURE CAN SHOW. "post-2011 reconstruction zone"
        # and "without electricity" cannot be read off a clip: a roundabout with cars on it looks
        # the same either way. Keeping them out of required_action stops the review from failing
        # footage for not evidencing something no footage could.
        "search_context: qualifiers that narrow the search but CANNOT be seen in a clip - a year, "
        "a cause, a legal status, a place name that does not change the picture. Never put these "
        "in required_action or required_result.\n"
        "{editorial_role,sequence_id,required_action,required_result,search_queries,search_context,exact_voice_text,start_time,end_time,beat_purpose,scene_objective,visual_meaning,viewer_emotion,visual_hook_type,required_visual_information,best_media_type,shot_type,camera_motion,subject_motion,environment_motion,emotional_action,must_show,must_not_show,crop_plan,voice_match_score_target}.\n"
    ...
    try:
        if collaborate:
            plan = collaborate_json(payload["messages"], max_tokens=payload["max_tokens"],
                                    ...
                                    label="edit-map", thinker=reasoning_model or "google/gemini-3.7-flash", critic=reasoning_model or "google/gemini-3.7-flash") or {}
        else:
        ...
        duplicate_actions = adjacent_duplicate_editorial_actions(beats)
        if duplicate_actions and not collaborate:
            bad = ", ".join(
                f"{i}/{i + 1}: {beats[i].get('required_action')}"
                for i in duplicate_actions)
            correction = (
                "The edit map is invalid because adjacent cuts repeat an identical visible "
                f"action/result contract: {bad}. Rewrite the full micro_beats array. Preserve "
                "every voice word and all timing coverage, but give each adjacent beat a visibly "
                "different next moment and matching search_queries. Do not solve this by changing "
                "camera angle, and do not invent an action the spoken span cannot support. Return "
            ...
            log(status_cb, "Micro-Beat Planner: duplicate adjacent actions found; correcting the edit map...")
            retry_data = post_json_url(WAVESPEED_LLM_API, retry_payload, timeout=180)
            retry_plan = extract_json_object(retry_data["choices"][0]["message"]["content"])
            retry_beats = normalize_micro_beat_plan(
                (retry_plan or {}).get("micro_beats", []), title, script, target_duration)
            corrected = retry_beats or beats
            remaining = adjacent_duplicate_editorial_actions(corrected)
            if remaining:
                beats = merge_adjacent_duplicate_editorial_actions(corrected)
                log(status_cb,
                    "Micro-Beat Planner: correction still repeated "
                    f"{len(remaining)} adjacent action(s); merged them into honest continuation beat(s).")
            else:
                plan, beats = retry_plan, corrected
        if beats:
...
def build_micro_beat_plan(title, script, visual_script, target_duration, base_scenes,
                          reasoning_model=None, status_cb=None, collaborate=False,
                          footage_source="generate"):
    result = llm_micro_beat_plan(title, script, visual_script, target_duration, base_scenes,
                                 reasoning_model=reasoning_model, status_cb=status_cb,
                                 collaborate=collaborate, footage_source=footage_source)
...
def plan_config(project_dir, title, script, target_duration, allow_seedance=True, max_seedance=3, static_gpt_image_count=4, scenes_override=None, director_plan=None, visual_sections=None, speaker_hook_enabled=False, image_model="openai/gpt-image-2/text-to-image", video_model="seedance-2.0"):
    ...
    for index, scene in enumerate(scenes, 1):
        ...
        scene_data = {
            ...
            "editorial_role": scene.get("editorial_role"),
            "sequence_id": scene.get("sequence_id", ""),
            "required_action": scene.get("required_action", ""),
            "required_result": scene.get("required_result", ""),
            # This dict is a whitelist: a key that is not named here is dropped, which is exactly
            # how `seedance_start_trim` went missing and left the caption pass reading second 0 of
            # every source. The searches a beat wrote for itself would go the same way.
            "search_queries": scene.get("search_queries") or [],
            "search_context": scene.get("search_context", ""),
        ...
        if scene.get("clip"):
            ...
            scene_data["source_has_captions"] = bool(scene.get("source_has_captions"))
            scene_data["blur_captions"] = (bool(scene.get("blur_captions"))
                                             if scene.get("blur_captions") is not None
                                             else scene_data["source_has_captions"])
            # THE IN-POINT TRAVELS WITH THE CLIP. A scraped clip is a 60-90 second TikTok and
            # the beat shows two seconds of it; without `seedance_start_trim` the file name
            # alone points at a DIFFERENT moment than the edit. This branch copied the name and
            # dropped the offset, and the caption cleanup runs 130 lines before the enforcement
            # merge restores it (the pass at :13651, the restore at :13784) - so it measured and
            # rebuilt second 0 of every source. Measured on the eating-walk Short: three scenes
            # were refused at 22.2% / 35.1% / 34.6% coverage, which is exactly what the HEADS of
            # those files measure (21.6 / 34.4 / 34.7); the windows the edit actually shows
            # measure 8.2% / 6.8% / 5.5%, all under the 12% ceiling and all cleanable. The clips
            # went out with the creator's Japanese text burned across them for a caption that is
            # not in the shot. `needs_replacement` was dropped the same way and reached the saved
            # config as None on all fifteen scenes.
            for _carry in ("seedance_start_trim", "scrape_clip_id", "scrape_source",
                           "needs_replacement", "coverage_gap", "assignment_type"):
                if scene.get(_carry) is not None:
                    scene_data[_carry] = scene.get(_carry)
...
def _parse_sse_chat_stream(body):
    ...
    usage, response_model = None, None
...
def post_json_url(url, payload, timeout=75):
    ...
    strict_token_limit = bool(payload.pop("_strict_token_limit", False))
    transport_retries = max(0, min(2, int(payload.pop("_transport_retries", 2))))
    ...
    if not strict_token_limit and isinstance(payload, dict) and payload.get("max_tokens"):
        ...
        # GPT-5.6 only spends a reasoning budget in its explicit Pro/Responses
        # mode.  Treating every standard Luna vision classification as a
        # thinking call silently raised a 400-token verdict to an 8k ceiling,
        # making high-volume source review needlessly expensive.
        gpt56_pro = "gpt-5.6" in model and bool(payload.get("reasoning"))
        if any(token in model for token in ("gemini", "glm", "qwen", "deepseek", "thinking",
                                            "kimi", "moonshot")) or gpt56_pro:
...
def assert_wavespeed_balance(status_cb=None, model="google/gemini-3.7-flash"):
    ...
    try:
        post_json_url(WAVESPEED_LLM_API, {
            ...
            "max_tokens": 64, "_strict_token_limit": True, "_transport_retries": 0,
            "reasoning_mode": "minimal"}, timeout=45)
    except WaveSpeedBalanceError:
        raise
    except Exception as exc:  # noqa: BLE001
...
def build_social_search_plan(title, script, scenes, understanding=None, reasoning_model=None,
    ...
    try:
        ...
        for _attempt in range(2):
            if collaborate:
                plan = collaborate_json(messages, max_tokens=8000, temperature=0.3, status_cb=status_cb, label="social search plan", thinker=reasoning_model or "google/gemini-3.7-flash", critic=reasoning_model or "google/gemini-3.7-flash") or {}
            else:
        ...
        if not buckets:
            return {}
        log(status_cb, f"Built {len(buckets)} social search bucket(s) from the script:")
        for b in buckets:
...
def find_reusable_social_clips(project_dir, title, script, understanding=None,
    ...
    for candidate in selected:
        ...
        for source in candidate["clips"]:
            ...
            # One platform left, so one floor. This used to divide the floor by four for X,
            # whose likes ran about a quarter of TikTok's for the same reach.
            hook_source_floor = MIN_CLIP_LIKES
            try:
...
def create_media_contact_sheet(image_paths, out_path, title="Media"):
    ...
    # The layout, so a caller can convert a coordinate the model gave in SHEET space back into
    # the tile it belongs to. See sheet_point_to_tile().
    out_path.with_suffix(".layout.json").write_text(json.dumps({
        "cols": cols, "rows": rows, "cell_w": cell_w, "cell_h": cell_h,
        "header_h": header_h, "sheet_w": cols * cell_w, "sheet_h": header_h + rows * cell_h,
        "count": len(image_paths)}), encoding="utf-8")
    return out_path


def sheet_point_to_tile(cx, cy, index, layout):
    """Turn a point given in CONTACT-SHEET space into one inside tile `index`.

    The vision planner is handed one sheet with a frame per scene and asked for coordinates
    "0-1 WITHIN each tile". It does not do that - it answers about the picture it was given, which
    is the whole sheet. Measured on the school-rules Short (2026-09-05): the twelve arrows landed
    on (0.18,0.16) (0.45,0.11) (0.86,0.12) (0.15,0.44) ... - each one the centre of its OWN tile
    in a 3x4 grid, so every arrow pointed at itself rather than at anything in the picture.

    Returns (cx, cy) inside the tile, or None when the point does not fall in this tile - which
    is the signal that the model really did answer in tile space and needs no conversion.
    """
    try:
        cols = int(layout["cols"]); rows = int(layout["rows"])
        cell_w = float(layout["cell_w"]); cell_h = float(layout["cell_h"])
        header_h = float(layout["header_h"])
        sheet_w = float(layout["sheet_w"]); sheet_h = float(layout["sheet_h"])
    except (KeyError, TypeError, ValueError):
        return None
    column, row = index % cols, index // cols
    x0 = column * cell_w / sheet_w
    x1 = (column + 1) * cell_w / sheet_w
    y0 = (header_h + row * cell_h) / sheet_h
    y1 = (header_h + (row + 1) * cell_h) / sheet_h
    if not (x0 - 1e-6 <= cx <= x1 + 1e-6 and y0 - 1e-6 <= cy <= y1 + 1e-6):
        return None
    return ((cx - x0) / max(1e-6, x1 - x0), (cy - y0) / max(1e-6, y1 - y0))
...
def assign_clips_to_scenes_by_vision(scenes, clip_paths, project_dir, reasoning_model=None,
    ...
    try:
        ...
        if collaborate:
            plan = collaborate_json(messages, max_tokens=6000, temperature=0.1, status_cb=status_cb, label="clip matching", thinker=reasoning_model or "google/gemini-3.7-flash", critic=reasoning_model or "google/gemini-3.7-flash") or {}
        else:
...
def score_hook_candidates(hook_clips, project_dir, reasoning_model=None, status_cb=None, collaborate=False):
    ...
    try:
        ...
        if collaborate:
            plan = collaborate_json(messages, max_tokens=3000, temperature=0.1, status_cb=status_cb, label="hook casting", thinker=reasoning_model or "google/gemini-3.7-flash", critic=reasoning_model or "google/gemini-3.7-flash") or {}
        else:
            data = post_json_url(WAVESPEED_LLM_API, {
                "model": reasoning_model or GPT55_MODEL, "messages": messages,
...
def sync_scenes_to_voice_timeline(scenes, word_timeline, target_duration=None):
    ...
    # A beat must not start in the middle of a sentence. The pacing splitter cuts a long beat
    # into equal parts, and an equal part lands wherever it lands - so a beat came out reading
    # "for some couples. Matching": the tail of one sentence plus the head of the next. The clip
    # is then chosen for the second half and put on screen at the first half's start. Measured on
    # the couples Short (2026-09-05): the dinner shot came up 2.7 SECONDS before the word
    # "dinner" was spoken, which is exactly what the owner saw.
    #
    # So every internal boundary is pulled to the nearest sentence start within 0.9s. Nothing
    # moves when there is no sentence break nearby.
    starts_after_sentence = []
    for index in range(1, len(word_timeline)):
        previous = str(word_timeline[index - 1].get("word", "")).strip()
        if previous.endswith((".", "!", "?", "…")):
            try:
                starts_after_sentence.append(float(word_timeline[index]["start"]))
            except (KeyError, TypeError, ValueError):
                continue
    if starts_after_sentence:
        for i in range(1, len(ordered)):
            boundary = float(ordered[i]["start"])
            nearest = min(starts_after_sentence, key=lambda t: abs(t - boundary))
            if abs(nearest - boundary) > 0.9:
                continue
            previous_start = float(ordered[i - 1]["start"])
            next_end = float(ordered[i].get("end", nearest + 0.2))
            if nearest <= previous_start + 0.4 or nearest >= next_end - 0.4:
                continue
            ordered[i]["start"] = round(nearest, 3)
            ordered[i - 1]["end"] = round(nearest, 3)
...
def enforce_reference_pacing(scenes, max_s=2.4, min_part_s=1.15):
    ...
    for s in scenes:
        ...
        if dur <= max_s:
            out.append(s); continue
        # Never create a part the coalescer will merge straight back: it runs right after this
        # and reassembled 1.42+1.41 into 2.83s on a measured run. Split into as many parts as
        # fit above the floor, and leave the beat whole when even two would fall under it.
        n = max(2, int(math.ceil(dur / max_s)))
        while n > 2 and dur / n < min_part_s:
            n -= 1
        if dur / n < min_part_s:
            out.append(s); continue
        step = dur / n
...
def enforce_fact_short_max_hold(scenes, max_s=2.75):
    """Last pacing gate for Fact Shorts.

    Earlier planning stages may group words into a convenient search chapter, but
    that grouping must never become a seven-second visual hold in the final
    timeline.  This runs after all chaptering/sync work and only subdivides; it
    cannot merge or stretch an existing cut.
    """
    return enforce_reference_pacing(scenes, max_s=float(max_s), min_part_s=1.05)


# 1.45 sat ABOVE the reference edit's typical shot (1.14s median), so it merged back exactly the
# halves the splitter had just created - a 2.83s beat split into 1.42+1.41 and was reassembled
# into 2.83s on the next line. The floor is meant to stop a 1.0s cut on top of footage that has
# its own edits, not to forbid the pace the reference actually runs at.
def coalesce_short_scrape_scenes(scenes, min_s=1.15, max_s=3.2):
...
def resolve_assigned_scene_clip(scene, clip_dir):
    """The clip a scene already names, resolved to a real path - or "" if there is none.

    `scene_clips` holds only the paths the MATCHER assigned. borrow_motion_for_uncovered and the
    spare-clip path write straight onto the scene dict and can leave that parallel list empty, so
    rebuilding the timeline from the list alone silently undid every borrow: the scrape reported
    four borrowed beats and zero still-fallbacks while the saved project came back with blank
    scenes and repeated clips. The V2 rescue further down is gated `not _v2`, so nothing refilled
    them either.
    """
    named = str((scene or {}).get("clip") or "").strip()
    if not named:
        return ""
    for candidate in (Path(clip_dir) / named, Path(named)):
        try:
...
def enforce_unique_scene_clips(config, project_dir, status_cb=None):
    ...
    if len(scenes) < 2:
        return 0
    clip_dir = Path(project_dir) / "seedance 2.0"
    is_v3 = (str(config.get("_scrape_engine") or "").lower() == "v3"
             or (Path(project_dir) / "review" / "scrape_v3_report.json").exists())
        ...
        for p in (side, alt):
            try:
                if p.exists():
                    ...
                    if str(d.get("clip_id") or ""):
                        return str(d.get("clip_id") or "")
            except Exception:
                continue
        # THE NAME CARRIES THE POST ID WHEN NOTHING ELSE DOES. V4 writes its assignments as
        # `v4_<beat>_<platform>__<id>.mp4` and gives them no sidecar, so every one of them came
        # back from here as "" - and an empty id counts as a different source from every other
        # empty id, which is how the cap below stopped capping.
        match = re.search(r"(?:^|_)((?:tiktok|instagram|youtube)__[A-Za-z0-9]+)", str(name))
    ...
    def _fingerprint(path):
        """Enough of the file to tell a copy from a different clip, cheaply."""
        try:
    ...
    # A DIFFERENT FILE NAME IS NOT A DIFFERENT CLIP. The pool was "every mp4 whose NAME is not on
    # the timeline", and V4 leaves a byte-identical twin of every assignment behind:
    # `scraped_00.mp4` and `v4_00_tiktok__7671326811866893581.mp4` have the same md5. So the
    # replacement shelf held sixteen copies of the footage already on screen, and this guard -
    # which exists to stop repeats - handed a beat a third copy of a source it was already
    # showing twice. Measured on the roundabouts Short (2026-09-09): scene 11 was given the
    # source already running on scenes 01 and 15, which is the operating theatre that ended up
    # under a line about roundabouts.
    on_timeline_prints = set()
    for name in on_timeline:
        if name:
            mark = _fingerprint(clip_dir / name)
            if mark:
                on_timeline_prints.add(mark)

    for p in sorted(clip_dir.glob("*.mp4")):
        if p.name in on_timeline:
            continue
        if _fingerprint(p) in on_timeline_prints:
            continue
        try:
            ...
            meta = {}
        # A raw V3 download without an acceptance sidecar is library material, not a safe
        # automatic substitute. The old default-to-accepted behaviour inserted unreviewed files
        # after V3 had finished its semantic review.
        if is_v3 and not side.exists():
            continue
    ...
    used_files, source_uses = set(), {}
    # A second WINDOW of one source is not the same thing as the same clip twice. The scraper
    # only grants one when the beats are about the same subject and the excerpts are different,
    # non-overlapping moments - "you order at the machine / you hand over the ticket" cut from
    # one continuous video is continuity, and demanding a separate upload per beat is what left
    # four beats of a twelve-beat script empty. The same FILE twice is still a duplicate, and so
    # is a third excerpt of one source.
    max_per_source = 3 if is_v3 else 2
    ...
    for i, sc in enumerate(scenes):
        ...
        # Only a REAL assignment earns the second window - the scraper granted it after checking
        # that the beats share a subject and the excerpts are different moments. A borrowed beat
        # repeating a source is exactly the repetition this guard exists to catch.
        entitled = str(sc.get("assignment_type") or "") not in ("borrowed_clip", "uncovered_still")
        allowed = max_per_source if entitled else 1
        dup = (name in used_files) or (src and source_uses.get(src, 0) >= allowed)
        if not dup:
            used_files.add(name)
            if src:
                source_uses[src] = source_uses.get(src, 0) + 1
            continue
        # find a replacement whose file AND source are unused. A candidate with NO id is not a
        # candidate with a free id: `not c["clip_id"]` waved every sidecar-less file straight past
        # the cap, which is most of what V4 leaves on disk. `_source_of` now reads the id out of
        # the file name, so a missing one really means unknown - and an unknown source is the one
        # case where this guard cannot promise anything, so it goes last rather than first.
        def _free(candidate):
            key = candidate["clip_id"] or _source_of(candidate["name"])
            return not key or source_uses.get(key, 0) < max_per_source
        repl = next((c for c in pool
                     if c["name"] not in used_files and _free(c)
                     and (c["clip_id"] or _source_of(c["name"]))), None)
        if repl is None:
            repl = next((c for c in pool if c["name"] not in used_files and _free(c)), None)
        if repl is None:
            unresolved += 1
        ...
        sc["clip"] = repl["name"]
        sc["asset"] = repl["name"]
        # ALWAYS re-point the source id at the file the scene now plays. Leaving the previous
        # id in place when the replacement has no sidecar made the scene claim a source it no
        # longer contains: every later scene was then compared against a source that is not on
        # the timeline, while the one that IS could be picked again. Clearing it is correct and
        # safe - the check above already falls back to the filename when the id is missing.
        sc["scrape_clip_id"] = repl["clip_id"] or ""
        # AND EVERYTHING ELSE THAT DESCRIBED THE CLIP THAT JUST LEFT. The id was re-pointed here
        # and the in-point was not, so the scene played a new file at the old file's second.
        # Measured on the roundabouts Short (2026-09-09): scene 13 kept in-point 30.95s - a window
        # of tiktok__7532847965329755400, which is 60.3s long - and was handed the file of
        # tiktok__7351667105453870368, which is 25.0s. The seek lands past the end of the file.
        # Scene 11 kept a verdict reading "the Traffic Light Tree sculpture on a UK roundabout"
        # over footage of an operating theatre.
        #
        # A pool clip arrives with no reviewed window for this beat, so 0.0 is the only honest
        # in-point, and the beat is flagged: a swap is a repair, not a verified assignment, and
        # the timeline should not look complete because of one.
        sc["seedance_start_trim"] = 0.0
        for _stale in ("source_trim", "start_trim"):
            if _stale in sc:
                sc[_stale] = 0.0
        for _verdict in ("v4_reason", "observed_action", "director_reason"):
            sc.pop(_verdict, None)
        sc["needs_replacement"] = True
        sc["coverage_gap"] = True
        sc["coverage_gap_reason"] = (f"the duplicate guard replaced {name} with {repl['name']}; "
                                     "no window of this clip was reviewed for this beat")
        sc["duplicate_guard_swapped"] = True
        used_files.add(repl["name"])
        _key = repl["clip_id"] or _source_of(repl["name"]) or repl["name"]
        source_uses[_key] = source_uses.get(_key, 0) + 1
        swaps += 1
...
def place_editor_sfx(config, reasoning_model=None, status_cb=None):
    ...
    if bool(config.get("scrape_transition_only_sfx")):
        events, sfx_events_report = [], []
        # ONE CURATED FOLDER IS THE WHOLE VOCABULARY. This used to name four files and then hunt
        # for them inside the CLASSIFIED library, so whether a cut sound existed depended on which
        # bucket a classifier had dropped it in. soundeffects/transitionsfx is chosen by hand and
        # read directly: everything in it is allowed at a cut, nothing outside it is.
        _tr_files = pipeline.transition_sfx_files(ROOT / "soundeffects")
        approved_pool = {f.name.casefold(): str(f) for f in _tr_files}
        approved_cut_order = tuple(f.name.casefold() for f in _tr_files)
        ...
        approved_assets_present = bool(approved_pool)
        if not approved_assets_present:
            # No curated folder on this install - fall back to the classified library so an
            # older checkout still gets cut sounds instead of silence.
            for category in ("bright_whoosh", "swipe_whoosh", "caption_pop", "ui_click"):
                for candidate in (lib.get(category) or []):
                    approved_pool.setdefault(approved_source_name(candidate), candidate)
        ...
        for i, start in cuts:
            ...
            if approved_assets_present:
                ...
                _n = picked_name
                cat = ("swipe_whoosh" if ("swish" in _n or "swoosh" in _n or "whoosh" in _n)
                       else "ui_click" if "click" in _n else "caption_pop")
            else:
            ...
            # PUT THE HIT ON THE CUT, NOT THE FILE START. Placing the file at the cut meant the
            # audible moment landed late by whatever lead-in the file happened to carry - measured
            # across the transition folder that is 10 to 164 ms, worst on flip-fx.mp3 - and the
            # only correction was a flat 60 ms for anything classed as motion, which is a guess
            # rather than a measurement. Subtracting the file's own transient anchor lines the
            # bubble's pop up with the frame the picture changes on.
            at = max(0.0, float(start) - pipeline.sfx_transient_anchor(path))
...
def render_blocks_enabled(config):
    """Are the hard pre-render gates allowed to REFUSE a render?

    Off by default, on the user's instruction. The checks themselves are worth keeping - a
    repeated-frame stall or an ambient bed under a Clip Short is still worth knowing about - so
    when the gates are off they report what they found and the render proceeds. Set
    ``enforce_render_blocks`` to bring the refusals back.
    """
    try:
        return bool((config or {}).get("enforce_render_blocks", False))
    except AttributeError:
        return False


def validate_scrape_render(config, status_cb=None):
    """Run the pre-render checks; refuse only when the gates are switched on."""
    try:
        _validate_scrape_render_strict(config, status_cb=status_cb)
    except RuntimeError as exc:
        if render_blocks_enabled(config):
            raise
        log(status_cb, f"Pre-render check (not blocking): {exc}")


def _validate_scrape_render_strict(config, status_cb=None):
...
def plan_visual_fx(config, project_dir, reasoning_model=None, status_cb=None, collaborate=False, vfx_amount="medium"):
    ...
    if strips and os.environ.get("WAVESPEED_API_KEY"):
        ...
        for b0 in range(0, len(strips), BATCH):
            ...
            try:
                if collaborate:
                    plan = collaborate_json(messages, max_tokens=4000, temperature=0.2, status_cb=status_cb, label="visual fx", thinker=reasoning_model or "google/gemini-3.7-flash", critic=reasoning_model or "google/gemini-3.7-flash") or {}
                else:
                    data = post_json_url(WAVESPEED_LLM_API, {
                    ...
                    plan = extract_json_object(data["choices"][0]["message"]["content"]) or {}
                try:
                    _layout = json.loads(Path(sheet).with_suffix(".layout.json")
                ...
                for k, v in smap.items():
                    if isinstance(v, dict):
                        try:
                            ...
                            continue
                        # Which tile this scene occupied, so its point can be converted out of
                        # sheet space below.
                        v = dict(v)
                        v["_sheet_layout"] = _layout
                        v["_tile_index"] = (sub_idx.index(_scene_index)
                                            if _scene_index in sub_idx else 0)
                        vision[_scene_index] = v
            except Exception as exc:  # noqa: BLE001
    ...
    for i, sc in enumerate(scenes):
        ...
        # THE ANSWER ARRIVES IN SHEET SPACE, whatever the prompt asks for. The planner hands the
        # model ONE contact sheet with a frame per scene and says "coordinates are 0-1 WITHIN each
        # tile"; the model answers about the picture it was given, which is the whole sheet.
        # Measured on the school-rules Short (2026-09-05): the twelve arrows came back as
        # (0.18,0.16) (0.45,0.11) (0.86,0.12) (0.15,0.44) ... - the centre of each scene's own
        # tile in a 3x4 grid. Every arrow pointed at itself. Convert the point into the tile, and
        # when it lands on the tile's own centre take it for what it is - no target found - and
        # draw nothing rather than an arrow aimed at the middle of the picture.
        _layout = d.get("_sheet_layout")
        if _layout:
            _local = sheet_point_to_tile(cx, cy, int(d.get("_tile_index") or 0), _layout)
            if _local is not None:
                cx, cy = _local
                if abs(cx - 0.5) < 0.12 and abs(cy - 0.5) < 0.12:
                    d = dict(d, confidence=0.0, callout="none")
    ...
    try:
        if collaborate:
            review = collaborate_json(payload["messages"], max_tokens=payload["max_tokens"],
                                      temperature=payload["temperature"], status_cb=status_cb,
                                      label=f"video review ({pass_label})", thinker=reasoning_model or "google/gemini-3.7-flash", critic=reasoning_model or "google/gemini-3.7-flash") or {}
        else:
...
def load_project_config(slug, prepare_media=True, create_media=True):
    """Load the latest saved render config for a project (for the timeline editor).

    ``create_media`` gates the pre-passes that BUILD derived footage (the caption blur and the
    speed re-encode). Reusing one that already exists is free; making one is not, and doing it
    while a page load waits blocks the browser - measured at 9.4 seconds of OCR for two clips
    when the timeline editor was opened. The render job builds them.
    """
...
def apply_timeline_edits_to_config(config, edits, slug, prepare_media=True, create_media=True,
                                   allow_gpu=False):
    ...
    for a in (edits.get("added") or []):
        ...
        try:
            # WHERE DID THIS COME FROM? The library sends an absolute `path` for most items, but
            # when it does not, all that survives is the editor's preview URL - and the basename
            # taken out of it was stored as a project-relative clip reference WITHOUT copying the
            # file. In the browser it still played, because the model held the original URL; after
            # a restart the project only had a name. If a same-named file happened to exist the
            # scene silently showed the WRONG clip, and if none did the slot came back empty.
            src = Path(asset) if asset else None
            if not (src and src.is_absolute() and src.is_file()):
                src = (pipeline.clip_ref_source_path(a.get("clip"))
                       or pipeline.clip_ref_source_path(asset)
                       or src)
            if is_clip and src and src.is_absolute() and src.is_file():
                clip_dir = project_dir / "seedance 2.0"
    ...
    for i, scene in enumerate(scenes):
        ...
        if sid in trim_by_id:
            try:
                ...
                pass
        # A reused source is often a MIXED reel: b-roll, then a hard cut into the creator talking
        # to camera. V3's own audit catches that for freshly scraped windows, but a scene that
        # arrives here through same-script reuse or a hand edit was never measured - one shipped
        # Short played straight across an internal cut into a talking head. Measure the exact
        # range the renderer will play; when it crosses a hard cut, snap the trim to the longest
        # clean segment of the file instead. Deterministic (scene-change detection), no vision.
        # Only while media is actually being prepared for output. This decodes frames -
        # measured at 21 of the 32 seconds it took to OPEN the timeline editor, which loads
        # the same config just to draw the track. It corrects a trim for the RENDER, and a
        # page that is only showing the timeline neither needs it nor should wait for it.
        _clip_name = str(scene.get("clip") or "")
        if (create_media and _clip_name.endswith(".mp4") and scene.get("seedance")
                and str(scene.get("assignment_type") or scene.get("match_class") or "")):
            _clip_path = project_dir / "seedance 2.0" / _clip_name
            _trim = float(scene.get("seedance_start_trim") or 0.0)
            _speed = float(scene.get("timeline_speed") or 1.0) or 1.0
            _need = dur * _speed
            if _clip_path.is_file() and _need > 0.4:
                try:
                    import scrape_v3 as _v3
                    _issue = _v3.technical_window_issue(str(_clip_path), _trim, _trim + _need)
                    if _issue == "hidden_source_cut":
                        import clip_scraper as _cs
                        _cuts = _cs.hard_cut_times(str(_clip_path), pipeline.find_ffmpeg(),
                                                   scan_seconds=180.0) or []
                        _total = float(_cs._probe_duration(
                            str(_clip_path), pipeline.find_ffprobe(pipeline.find_ffmpeg()))
                            or (_trim + _need))
                        _edges = [0.0] + sorted(float(c) for c in _cuts) + [float(_total)]
                        _segments = [(a, b) for a, b in zip(_edges, _edges[1:]) if b - a >= _need + 0.2]
                        if _segments:
                            _a, _b = max(_segments, key=lambda ab: ab[1] - ab[0])
                            _new_trim = round(min(max(_a + 0.1, 0.0), _b - _need - 0.1), 3)
                            log(status_cb, f"Timeline render: {_clip_name} played {_trim:.1f}-"
                                           f"{_trim + _need:.1f}s across an internal cut; moved to "
                                           f"the clean segment at {_new_trim:.1f}s.")
                            scene["seedance_start_trim"] = _new_trim
                        else:
                            log(status_cb, f"Timeline render: {_clip_name} crosses an internal cut "
                                           f"and has no clean segment long enough; left as edited.")
                except Exception:      # noqa: BLE001 - a failed measurement must not stop a render
                    pass
        ...
        if sid in replaced_by_id:
            try:
                # Same lesson as the add path: the chosen media may arrive as the editor's preview
                # URL rather than an absolute path. src.exists() was then False, the except below
                # swallowed it, and the scene silently kept the clip the user had just replaced.
                _rp = replaced_by_id[sid].get("path")
                src = Path(_rp) if _rp else None
                if not (src and src.is_file()):
                    src = pipeline.clip_ref_source_path(_rp) or src
                if src and src.exists():
                    ...
                    if is_video:
                        ...
                        # A replacement is pristine media.  Never inherit the previous clip's
                        # removal pass marker or a failed prior detection can make the new source
                        # look as if it had already been cleaned.
                        scene.pop("caption_removal_passes", None)
                    else:
        ...
        if not want_blur and prepare_media and scene.get("clip") and sid not in blur_by_id:
            # ASK THE CLIP, not the label. This flag was set only where the vision review had
            # called the caption "plain_small", so a clip whose text it described any other way
            # went to render with the creator's caption burned in: measured on a finished Short,
            # five of thirteen scenes still carried Japanese text at 2.8-14.1% of the frame while
            # only four had been cleaned. A measurement costs about two seconds per scene against
            # a render that takes minutes, and the rule is that anything with a detectable caption
            # gets the inpainting pass. An explicit user toggle still wins.
            try:
                import caption_remover as _capcheck
                _ffm, _ = clip_scraper._ffmpeg_tools()
                _clip_path = project_dir / "seedance 2.0" / str(scene["clip"])
                if _ffm and _clip_path.exists():
                    # AT THE SCENE'S OWN IN-POINT. This measured the first 2.2s of the FILE
                    # while the edit cuts somewhere else entirely - 36s in, for scene 01 of the
                    # eating-walk Short. So a clip clean at its head and captioned where the
                    # scene starts was never cleaned at all, and one captioned only at its head
                    # was cleaned for nothing. The timeline path already asks at the in-point
                    # (see prepare_timeline_clips); this one was left behind.
                    _cov = _capcheck.caption_coverage(
                        str(_clip_path), _ffm, seconds=2.2,
                        start=_seconds_of(scene, "seedance_start_trim", "source_trim",
                                          "start_trim"))
                    if _cov < 0:
                        # NOT THE SAME AS CLEAN. caption_coverage returns -1.0 when it could not
                        # measure - a decode that failed, a clip too short to sample - and that
                        # value silently failed the range test below, so an unmeasurable clip was
                        # treated exactly like one with no text on it and shipped as it was.
                        # Cleaning costs GPU minutes; shipping somebody else's caption costs the
                        # video. The removal pass measures again and does nothing when it finds
                        # no glyphs, so sending an unknown clip through it is safe.
                        want_blur = True
                        log(status_cb, f"Caption removal: scene {sid} could not be measured "
                                       f"(coverage came back {_cov}); cleaning it rather than "
                                       f"assuming it is clean.")
                    elif 0.015 < _cov <= _capcheck.MAX_COVERAGE:
                        want_blur = True
                        log(status_cb, f"Caption removal: scene {sid} carries text on "
                                       f"{_cov:.1%} of the frame; cleaning it before render.")
                    elif _cov > _capcheck.MAX_COVERAGE:
                        # Above the ceiling the inpainting cannot rebuild the frame, so this clip
                        # ships with the creator's text on it. That is a decision, and it was
                        # being made in silence.
                        log(status_cb, f"Caption removal: scene {sid} carries text on "
                                       f"{_cov:.1%} of the frame - ABOVE the "
                                       f"{_capcheck.MAX_COVERAGE:.0%} ceiling the removal pass "
                                       f"can rebuild. It will ship with that text; replace the "
                                       f"clip if that matters.")
            except Exception:                                           # noqa: BLE001
                pass
        ...
        if prepare_media and scene.get("clip"):
            try:
                clip_dir = project_dir / "seedance 2.0"
                # A stale caption_blur_src silently RESTORES the old footage. It is written when
                # a clip is cleaned, and it survives when the scene's clip is later replaced - by
                # hand in the editor, by the pool swap above, or by an agent rework. The pre-pass
                # then rebuilds the blur from the OLD source and points the scene at it, so the
                # render shows the clip the user just removed. Measured 2026-09-05 on the couples
                # copy: scene 0 was set to a new hook clip, `caption_blur_src` still said
                # scraped_00.mp4, and both renders came back with the old station shot.
                blur_src = str(scene.get("caption_blur_src") or "")
                current = str(scene.get("clip") or "")
                if blur_src and current and not current.startswith(("capblur_", "speed_"))                         and blur_src != current:
                    scene.pop("caption_blur_src", None)
                    blur_src = ""
                base = str(blur_src or scene.get("timeline_speed_src") or scene["clip"])
                if want_blur and not base.startswith(("speed_", "capblur_")) and (clip_dir / base).exists():
                    ...
                    # THE SECONDS THE SCENE SHOWS, not the first seconds of the file. The clean
                    # ran over the head of the source while the renderer seeks to
                    # `seedance_start_trim` - so a caption sitting where the edit cuts in was
                    # never touched. Measured on the school-rules Short (2026-09-05): a yellow "3"
                    # on the schoolgirl's collar survived the pass untouched, and three of six
                    # cleaned clips still read 0.020 to 0.042 in the finished video.
                    _trim = _seconds_of(scene, "seedance_start_trim", "source_trim", "start_trim")
                    # HOW MANY SECONDS THE SCENE SHOWS - not where they end in the file. `_trim`
                    # is where the window starts, and it is passed separately as `start=`; adding
                    # it here made the DURATION the distance from the head of the source to the
                    # end of the window, so a 1.9s beat 36s into a TikTok sent ProPainter 38.3
                    # seconds of video. Measured in this render's own log: 1150, 1627 and 697
                    # frames rebuilt for scenes of 1.9s, 1.8s and 2.5s - about twenty times the
                    # work, and the reason a thirty-second Short took forty minutes to clean.
                    _used_len = max(1.0, float(scene.get("end", 0) or 0)
                                    - float(scene.get("start", 0) or 0)) + 0.5
                    # The cache key still keys on the END of the window: extending a scene must
                    # build a longer derivative rather than reuse a shorter one.
                    _used_span = _used_len + _trim
                    # The derived clean clip is intentionally limited to the part used on the
                    # timeline. Include that duration in its cache key: if the user later extends
                    # the scene, a longer clean derivative is built instead of freezing at the
                    # end of an older short one.
                    # ...and WHICH VERSION of the remover made it. Without this a clip cleaned by
                    # an older, worse pass is reused for ever - the reason a known-bad fill kept
                    # coming back in finished Shorts while a fresh run of the same source was
                    # clean. Bumping caption_remover.CLEAN_VERSION retires every old derivative
                    # at once, without deleting anything.
                    import caption_remover as _capver
                    _bk = hashlib.sha1(
                        f"{base}|{_used_span:.3f}|v{_capver.CLEAN_VERSION}"
                        .encode("utf-8", "ignore")
                    ).hexdigest()[:10]
                    dest = clip_dir / f"capblur_{sid}_{_bk}.mp4"
                    clean_marker = dest.with_suffix(dest.suffix + ".caption_clean.json")
                    # A historical failed pass could leave behind the initial byte-for-byte copy
                    # at the final capblur name. Existence alone therefore does NOT prove captions
                    # were removed (that stale copy caused original TikTok captions to reappear in
                    # a later render). Only a success sidecar makes a derivative reusable.
                    if dest.exists() and not clean_marker.exists() and create_media:
                        dest.unlink(missing_ok=True)
                    if not dest.exists() and create_media:
                        shutil.copy2(clip_dir / base, dest)
                        found = 0
                        try:
                            # This REMOVES the caption rather than covering it. Blur could
                            # not work on a glyph-tight mask - a Gaussian averages the pixels
                            # inside the mask, and inside a glyph-tight mask there is only the
                            # glyph, so the letter was smoothed into itself and the word shape
                            # survived even at sigma 80. Widening the mask to the whole line hid
                            # the words and wrecked the picture instead: on one shot it smeared
                            # away the goshuin stamp that was the subject. Inpainting discards
                            # the glyph pixels and rebuilds from elsewhere, so the tight mask is
                            # an advantage. The scene name stays capblur_* so older projects,
                            # the toggle-off path and the duplicate guard keep working.
                            import caption_remover as _capfix
                            # Cancel has to reach the fill. Without this the user pressed Cancel
                            # and watched it work through every remaining clip, a minute each,
                            # because the only checkpoint was after the whole loop.
                            _ev = config.get("_cancel_event")
                            _status = config.get("_status_cb")
                            _blur_info = {}
                            found = _capfix.remove_caption_regions(
                                dest, pipeline.find_ffmpeg(),
                                seconds=_used_len, start=_trim,
                                status_cb=_status if callable(_status) else None,
                                info=_blur_info,
                                allow_gpu=allow_gpu,
                                cancel_check=(_ev.is_set if _ev is not None else None))
                            # THE SCENE REMEMBERS WHAT HAPPENED TO IT. The messages below went
                            # to the log and nowhere else, so the saved plan ended up saying
                            # `blur_captions: true` while pointing at raw scraped footage, with
                            # no record of why - four scenes of the walking/eating Short looked
                            # exactly like scenes nobody had tried to clean. A log line is gone
                            # by the time anyone reads the config.
                            if found:
                                scene["caption_clean_result"] = "cleaned"
                            elif _blur_info.get("refused"):
                                scene["caption_clean_result"] = (
                                    f"refused: text covers "
                                    f"{float(_blur_info.get('covered') or 0.0):.1%} of the frame")
                            elif _blur_info.get("skipped"):
                                scene["caption_clean_result"] = f"skipped: {_blur_info['skipped']}"
                            elif _blur_info.get("residual"):
                                # The pass DID run and rebuilt the frame; the text was still
                                # readable afterwards, so the original was put back. That is a
                                # different fact from "there was nothing to remove", and both
                                # were reaching the config as the same sentence.
                                _r = _blur_info["residual"]
                                scene["caption_clean_result"] = (
                                    f"rebuilt, but {float(_r.get('left') or 0):.1%} still read as "
                                    f"text (from {float(_r.get('from') or 0):.1%}); original kept")
                            elif _blur_info.get("hole"):
                                _h = _blur_info["hole"]
                                scene["caption_clean_result"] = (
                                    f"rebuilt area came back as a hole "
                                    f"({float(_h.get('black') or 0):.0%} of it black); original kept")
                            elif _blur_info.get("failed"):
                                scene["caption_clean_result"] = f"failed: {_blur_info['failed']}"
                            else:
                                scene["caption_clean_result"] = "no removable text found"
                            if not found and callable(_status):
                                # THE LINE THE OPERATOR WATCHES SAYS THE SAME THING THE SCENE
                                # RECORDS. `caption_clean_result` was split into four outcomes one
                                # layer up and this sentence was left behind, so a fill that ran
                                # for forty seconds, rebuilt sixty-nine frames and was rejected for
                                # leftover text still announced itself as "found no removable
                                # text". Read live in the export of 2026-09-09 for scenes 01, 02
                                # and 03, each of which had just rebuilt its whole window.
                                _status(f"Caption removal: scene {sid} - "
                                        f"{scene.get('caption_clean_result') or 'no result'}.")
                            if found:
                                clean_marker.write_text(json.dumps({
                                    "source": base,
                                    "scene_id": sid,
                                    "used_seconds": round(_used_span, 3),
                                    "completed": True,
                                }, ensure_ascii=False, indent=2), encoding="utf-8")
                        except Exception as _blur_exc:      # noqa: BLE001
                            print(f"[capblur] scene {sid}: {type(_blur_exc).__name__}: "
                                  f"{_blur_exc}")
                            scene["caption_clean_result"] = f"failed: {type(_blur_exc).__name__}"
                            found = 0
                        if not found:
    ...
    for item in (edits.get("transitions") or []) + (edits.get("sfx") or []):
        ...
        if item.get("start_abs") is not None:
            try:
                ...
                pass
        # WHO put it there. The editor sends start_abs for every sound it round-trips, so the
        # presence of one says nothing; only this flag means a person dragged or typed it. The
        # render leaves a hand-placed sound alone and transient-aligns the rest.
        if item.get("start_abs_user"):
            entry["start_abs_user"] = True
...
def save_timeline_edits(slug, edits):
    ...
    # `prepare_media=False` was passed and `create_media` was not - and `create_media` is the one
    # that BUILDS things. So every save still measured caption coverage on each scene and re-
    # encoded the speed-changed clips, with the browser waiting on it: measured 48.3 seconds to
    # save a 14-scene edit in which nothing had changed. Pressing Render saves first, so that
    # was 48 seconds of "saving" before the render even started.
    config = load_project_config(slug, prepare_media=False, create_media=False)
    apply_timeline_edits_to_config(config, edits, slug, prepare_media=False, create_media=False)
    project_dir = PROJECTS_DIR / slug
...
def _scene_length(scene):
    """How many seconds this scene is on screen.

    A timeline scene states its length as `start`/`end`, NOT as `duration` - that is what
    apply_timeline_edits_to_config writes, what scrape_v4 reads back, and what the renderer uses.
    Asking for "duration" returned 0.0 for every scene, so the caption pass cut a fixed ~3.7s
    window for beats of five and eight seconds and the renderer then slowed the clip past its
    floor and froze it - re-creating downstream exactly the fault the excerpt extension had just
    removed (found in review, 2026-09-04, and visible in my own measurement of five finished
    Shorts, which printed 0.0 for every scene's "duration").
    """
    for pair in (("end", "start"),):
        try:
            span = float((scene or {}).get(pair[0]) or 0.0) - float((scene or {}).get(pair[1]) or 0.0)
        except (TypeError, ValueError):
            span = 0.0
        if span > 0:
            return span
    return _seconds_of(scene, "duration", "dur")


def _seconds_of(scene, *keys):
    """The first of these scene fields that holds a positive number of seconds."""
    for key in keys:
        try:
            value = float((scene or {}).get(key) or 0.0)
        except (TypeError, ValueError):
            continue
        if value > 0:
            return value
...
def _text_free_window(path, ffmpeg, trim, shown, log=None, status_cb=None):
    """The nearest stretch of THIS source that carries no burned-in text, or None.

    ProPainter can only rebuild what neighbouring frames reveal. A caption printed over a busy
    street never uncovers its own background, so the fill comes back still readable and the clip
    is rejected - correct, but it leaves the caption in the finished video when there is nothing
    to put in the beat instead. The source itself is the answer: a TikTok runs thirty to a hundred
    seconds and its captions come and go. Moving the in-point by a few seconds keeps the same
    subject, the same creator, the same look, and drops the text.

    One pass, not a search. Asking caption_coverage() at candidate offsets was the obvious
    implementation and it is unusable: each call decodes and recognises its own frames, measured
    at 20-35 seconds apiece, so a dozen candidates cost ten minutes of a render. Instead the
    source is sampled ONCE - a frame every 1.4s - and the recogniser is asked about each; the
    text-free gaps fall out of that timeline, and the nearest one that is long enough wins.
    """
    try:
        import cv2
        import clip_scraper as _tools
    except Exception:                                                   # noqa: BLE001
        return None
    ocr = None
    try:
    ...
    total = count / fps if fps > 0 else 0.0
    need = float(shown) + 0.4
    if total <= need + 1.0:
        cap.release()
        return None
    step = 1.4
    marks = []                       # (seconds, carries_text)
    t = 0.2
    while t < total - 0.2 and len(marks) < 60:
        cap.set(cv2.CAP_PROP_POS_FRAMES, int(t * fps))
        ok, frame = cap.read()
        if ok:
            try:
                rows = _tools._ocr_text_rows(frame, min_conf=0.30)
            except Exception:                                           # noqa: BLE001
                rows = []
            marks.append((t, bool(rows)))
        t += step
    cap.release()
    if not marks:
        return None
    # the gaps between the samples that carry text
    gaps, run_start = [], None
    for seconds, has_text in marks + [(total, True)]:
        if has_text:
            if run_start is not None and seconds - run_start >= need:
                gaps.append((run_start, seconds))
            run_start = None
    ...
    if not gaps:
        return None
    def _pick(gap):
        lo, hi = gap[0], gap[1] - need
        return min(max(float(trim), lo), max(lo, hi))
    best = min((_pick(g) for g in gaps), key=lambda start: abs(start - float(trim)))
    if abs(best - float(trim)) < 0.25:
        return None                  # it already sits in a clean stretch; nothing to move
    if log:
        log(status_cb, f"Timeline render: {Path(path).name} could not be cleaned at {trim:.1f}s; "
                       f"the same clip carries no text from {best:.1f}s - moving the in-point there.")
    return round(best, 3)


def _clip_source_key(name):
    """Which post a clip file came from, whatever was done to it since.

    `v4_tiktok_7345755827334073601.mp4`, `pool_v4_tiktok_73457558273340_3c6f6e0243.mp4`,
    `unbox_v4_...`, `win4_..._0021650.mp4` and `capblur_02_f0dd28a4b4.mp4` are four derivations of
    one download plus one unrelated file. The post id is the only part that survives every
    derivation, so a run of ten or more digits IS the key; a name without one keeps its own stem.
    """
    text = str(name or "")
    ids = re.findall(r"\d{10,}", text)
    if ids:
        return max(ids, key=len)[:14]
    return Path(text).stem.casefold()


def _clean_replacement_from_pool(project_dir, scene, wanted, log=None, status_cb=None,
                                 taken=None):
    """Another window from THIS project's own pool that carries no burned-in text.

    When the fill cannot rebuild a caption - text over a moving street never uncovers its own
    background - the beat used to go out with the creator's words across it, because the render
    had nothing else to put there. It does: every candidate V4 downloaded is still on disk and
    the report holds its measured caption share, its verdict and its query. A window that was
    good enough to be a candidate for this beat and measures clean is strictly better than the
    one that cannot be cleaned.

    Nothing is downloaded and nothing is re-reviewed; this only re-uses what the run already
    inspected. Returns (filename, in-point, source id) or None - the id because the swap changes
    which VIDEO the beat plays, and the duplicate guard keys on `scrape_clip_id`. Without it the
    scene kept advertising the post it no longer holds, and two beats swapped onto the same source
    read as two different videos.

    Two things the first version got wrong, both measured on the dating recut (2026-09-05):

    * A candidate lives where V4 put it - usually `seedance 2.0/_v4_proxies/` - and the renderer
      resolves a scene's clip as a BARE NAME inside `seedance 2.0/`. Returning the candidate's
      basename therefore pointed six beats at a file the renderer could not find, and each one
      came out as a black title card. The chosen window is copied into the clip folder first.
    * Every failing beat asked independently and every one got the same top-ranked window, so a
      single shot would have covered six beats. `taken` carries what the render has already
      assigned, and a window in it is not offered again.
    """
    try:
        report = json.loads((Path(project_dir) / "review" / "scrape_v4_report.json")
                            .read_text(encoding="utf-8"))
    except Exception:                                                   # noqa: BLE001
        return None
    query = str(scene.get("search_query") or scene.get("query") or "").casefold()
    beat_text = str(scene.get("exact_voice_text") or scene.get("script") or "").casefold()
    used = {str(scene.get("clip") or "")} | {str(name) for name in (taken or ())}
    used_keys = {_clip_source_key(name) for name in used if name}
    best = None
    for cand in report.get("candidates") or []:
        try:
            share = float(cand.get("caption_share", -1.0))
            path = Path(str(cand.get("path") or ""))
            span = float(cand.get("end") or 0) - float(cand.get("start") or 0)
        except (TypeError, ValueError):
            continue
        if not path.name or path.name in used or not path.is_file():
            continue
        # Same footage under another file name: every derived copy (pool_/unbox_/win<v>_/capblur_)
        # keeps the post id, so compare THAT rather than the names. Offering a window back that
        # the edit already plays - or the very clip that just failed - is not a replacement.
        if _clip_source_key(path.name) in used_keys:
            continue
        if share < 0 or share > 0.015 or span < wanted - 0.1:
            continue
        verdict = cand.get("vision") or {}
        relevance = float(verdict.get("relevance") or 0)
        if relevance < 8:
            continue
        # the beat's own query first, then any window the reviewer rated highly
        same_query = str(cand.get("query") or "").casefold() == query and bool(query)
        overlap = 1 if (query and query in beat_text) else 0
        rank = (1 if same_query else 0, overlap, relevance,
                float(cand.get("visual_interest") or 0))
        if best is None or rank > best[0]:
            best = (rank, path, float(cand.get("start") or 0.0),
                    str(cand.get("source_id") or ""))
    if not best:
        return None
    source = best[1]
    # The renderer looks a scene's clip up by bare name inside the clip folder, and the pool sits
    # in a subfolder of it - so the window has to move there before the name means anything.
    clip_dir = Path(project_dir) / "seedance 2.0"
    try:
        clip_dir.mkdir(parents=True, exist_ok=True)
        dest = clip_dir / source.name
        if dest.resolve() != source.resolve():
            if dest.exists() and dest.stat().st_size != source.stat().st_size:
                key = hashlib.sha1(str(source.resolve()).encode("utf-8", "ignore")).hexdigest()[:8]
                dest = clip_dir / f"pool_{source.stem[:28]}_{key}{source.suffix.lower() or '.mp4'}"
            if not dest.exists():
                shutil.copy2(source, dest)
    except Exception:                                                   # noqa: BLE001
        return None
    if log:
        log(status_cb, f"Timeline render: no clean version of {scene.get('clip')} exists; "
                       f"the beat takes {dest.name} at {best[2]:.1f}s from this run's own pool "
                       f"instead of going out with the creator's caption on it.")
    return dest.name, round(best[2], 3), best[3]


def prepare_timeline_clips(config, project_dir, status_cb=None):
    """Clean burned-in text and rebuild letterboxed frames on EVERY clip about to be rendered.

    Both passes existed and neither one ran here. The caption gate lived in the pipeline's own
    scrape render, and the letterbox repair lived in the V4 source gate - but the button the user
    actually presses is Render in the timeline editor, which calls this function's caller and went
    straight to `pipeline.render_video`. That is why finished Shorts kept shipping with other
    creators' Japanese captions burned into them, and why a 16:9 clip dropped into the timeline by
    hand still arrived with black bars.

    It runs over the timeline's own scene list, so a clip the user swapped in, downloaded through
    the manual browser, or dragged from the library gets the same treatment as a scraped one.
    Every step is best-effort: an export must never fail because a clean-up could not run.
    """
    scenes = config.get("scenes") or []
    if not scenes:
        return
    # What the edit already shows. A pool swap must not hand one window to several beats, and it
    # must not offer a shot the viewer is about to see anyway.
    assigned = {str(scene.get("clip") or "") for scene in scenes if scene.get("clip")}
    try:
        ...
        return
    try:
        ffmpeg, _ = _tools._ffmpeg_tools()
    except Exception:                                                   # noqa: BLE001
        ffmpeg = ""
    if not ffmpeg:
        return
    media_dir = Path(project_dir) / "seedance 2.0"
    cleaned = unboxed = 0
    for scene in scenes:
        name = str(scene.get("clip") or "")
        if not name or not name.lower().endswith((".mp4", ".mov", ".webm", ".mkv")):
            continue
        path = media_dir / name
        if not path.is_file():
            continue

        # AN IN-POINT THAT IS NOT INSIDE ITS OWN FILE. Whatever wrote it, the seek lands past the
        # end and the beat renders as a frozen last frame or as nothing. Measured on the
        # roundabouts Short (2026-09-09): scene 13 held in-point 30.95s - a window of a 60.3s
        # source - against the 25.0s file the duplicate guard had swapped in for it. This is the
        # net rather than the cause: any pass that changes a scene's file without changing its
        # in-point lands here, and the render repairs it instead of showing a dead frame.
        try:
            _shown = _scene_length(scene) or 2.75
            _trim_now = _seconds_of(scene, "seedance_start_trim", "source_trim", "start_trim")
            _dur = float(_tools._probe_duration(str(path), pipeline.find_ffprobe(ffmpeg)) or 0.0)
            if _dur > 0.1 and _trim_now > 0.0:
                _fixed = None
                if _trim_now >= _dur:
                    # Past the end of the file: the number describes a different video, so there
                    # is nothing to salvage from it.
                    _fixed = 0.0
                elif _trim_now + _shown > _dur + 0.05:
                    _fixed = max(0.0, _dur - _shown)
                if _fixed is not None:
                    log(status_cb, f"Timeline render: {name} is {_dur:.1f}s but scene "
                                   f"{scene.get('id')} starts at {_trim_now:.1f}s and shows "
                                   f"{_shown:.1f}s; moving its in-point to {_fixed:.1f}s.")
                    for _key in ("seedance_start_trim", "source_trim", "start_trim"):
                        if _key in scene:
                            scene[_key] = round(_fixed, 3)
                    scene.setdefault("seedance_start_trim", round(_fixed, 3))
                    scene["needs_replacement"] = True
        except Exception:                                               # noqa: BLE001
            pass
        # Bars first: the text a creator printed on a matte disappears with the matte, and
        # measuring captions on the rebuilt frame is what the render will actually show.
        # Measure the SECONDS THIS SCENE SHOWS, not the whole file. A post can be full-bleed for
        # ten seconds and then sit its picture inside a black band exactly where the edit cuts in,
        # and asking about the file answers "no band" while a quarter of the rendered screen is
        # dead. Measured in a finished Short (2026-09-04): a 463px band along the bottom at 3.5s
        # and a 477px band along the top at 12s, of a 1920-high frame.
        def _repair_dead_bands(_path, _scene, _name):
            """Measure and rebuild the bars of the window this scene CURRENTLY shows.

            A function rather than a block because the window moves after this runs. The caption
            stage may slide the in-point to miss burned-in text, or swap the clip outright, and
            the bars belong to the window, not to the file. Measured in the school-rules export
            of 2026-09-09: scene 05's bands were measured at 9.06s, the caption stage then moved
            the in-point to 11.40s to get away from the text, and the finished video shows 256
            black rows at the top and 196 at the bottom of a 1920-high frame - a quarter of the
            screen, in an export whose log says the letterbox pass ran.
            """
            try:
                _trim = _seconds_of(_scene, "seedance_start_trim", "source_trim", "start_trim")
                _shown = _scene_length(_scene) or 2.75
                # None when the frame is full-bleed, which is the common case - unpacking it
                # blindly would send every clean clip through the exception path and silently
                # skip the rest.
                _bands = (scrape_v4.measure_letterbox(str(_path), start=_trim,
                                                      end=_trim + _shown)
                          or scrape_v4.measure_dead_bands(str(_path), start=_trim,
                                                          end=_trim + _shown))
                if _bands and (_bands[0] or _bands[1]):
                    _repaired, _note = scrape_v4.unletterbox_clip(
                        str(_path), ffmpeg=ffmpeg, start=_trim, end=_trim + _shown)
                    if _repaired:
                        _new = Path(_repaired)
                        _scene["clip"] = _scene["asset"] = _new.name
                        log(status_cb, f"Timeline render: {_name} had a dead band; {_note}")
                        return _new
            except Exception:                                           # noqa: BLE001
                pass
            return None
        ...
        # Bars first: the text a creator printed on a matte disappears with the matte, and
        # measuring captions on the rebuilt frame is what the render will actually show.
        _rebuilt = _repair_dead_bands(path, scene, name)
        if _rebuilt is not None:
            path = _rebuilt
            unboxed += 1
        try:
            # At the scene's own in-point: a source can be clean where it starts and
            # carry a caption bar eleven seconds in, which is where the edit cuts.
            coverage = caption_remover.caption_coverage(
                str(path), ffmpeg, seconds=2.2,
                start=_seconds_of(scene, "seedance_start_trim", "source_trim", "start_trim"))
        except Exception:                                               # noqa: BLE001
            continue
        if coverage < 0:
            log(status_cb, f"Caption check: {path.name} could not be measured "
                           f"({coverage}); cleaning it rather than assuming it is clean.")
        elif coverage <= caption_remover.MIN_COVERAGE:
            continue
        wanted = _scene_length(scene) or 2.75
        trim = _seconds_of(scene, "seedance_start_trim", "source_trim", "start_trim")

        def look_elsewhere(_path=path, _scene=scene, _wanted=wanted, _trim=trim):
            """No clean version of THIS window: the same source without text, then the pool.

            EVERY EXIT HERE CHANGES WHAT THE VIEWER SEES, so every exit says so. One of the two
            callers used to announce the swap and the other did not, and the quiet one is the
            common case - a fill that ran and was then refused. A finished render was traced
            back and its opening shot had been replaced by a pool clip with nothing in the log
            and nothing in the saved config to show it.
            """
            _sid = str(_scene.get("id") or "?")
            moved = _text_free_window(_path, ffmpeg, _trim, _wanted, log=log, status_cb=status_cb)
            if moved is not None:
                log(status_cb, f"Timeline render: scene {_sid} keeps {_path.name} but moves its "
                               f"in-point {_trim:.2f}s -> {moved:.2f}s to miss the text.")
                for key in ("seedance_start_trim", "source_trim", "start_trim"):
                    if key in _scene:
                        _scene[key] = moved
                _scene.setdefault("seedance_start_trim", moved)
                # The bars belong to the WINDOW, and the window just moved. They were measured
                # at the old in-point, before this function existed to move it.
                _repair_dead_bands(_path, _scene, _path.name)
                return True
            swap = _clean_replacement_from_pool(project_dir, _scene, _wanted,
                                                log=log, status_cb=status_cb, taken=assigned)
            if not swap:
                log(status_cb, f"Timeline render: scene {_sid} keeps {_path.name} with its text - "
                               "no clean window and nothing in the pool to put there.")
                return False
            log(status_cb, f"Timeline render: scene {_sid} drops {_path.name} and takes "
                           f"{swap[0]} at {swap[1]:.2f}s instead.")
            assigned.add(swap[0])
            _scene["clip"] = _scene["asset"] = swap[0]
            for key in ("seedance_start_trim", "source_trim", "start_trim"):
                if key in _scene:
                    _scene[key] = swap[1]
            _scene.setdefault("seedance_start_trim", swap[1])
            # THE BEAT NOW PLAYS A DIFFERENT VIDEO, so it must stop advertising the old one. The
            # duplicate guard keys on `scrape_clip_id` where it exists, so a scene that kept the
            # id of the clip it just dropped hid a real repeat: two beats swapped onto the same
            # pool source read as two different posts.
            if len(swap) > 2 and swap[2]:
                _scene["scrape_clip_id"] = swap[2]
            else:
                _scene.pop("scrape_clip_id", None)
            # A different file at a different second: nothing measured before this point applies
            # to what the beat now shows.
            _repair_dead_bands(_path.parent / swap[0], _scene, swap[0])
            return True

        if coverage > caption_remover.MAX_COVERAGE:
            # More text than the fill can rebuild. This used to `continue` - so the heaviest
            # captions, the full-width speech bubbles and pasted comment cards, were the ONLY
            # ones that got no treatment at all and went out untouched. Measured on the dating
            # recut: five beats shipped with the creator's Japanese text across them. Skip the
            # GPU minutes that cannot succeed and go looking for other footage instead.
            log(status_cb, f"Timeline render: {path.name} carries text on {coverage:.1%} of the "
                           "frame - too much to rebuild; looking for footage without it.")
            if look_elsewhere():
                cleaned += 1
            continue
        # CUT THE WINDOW OUT FIRST, THEN CLEAN IT. Passing "the first n seconds" to the remover
        # still made it fill everything BEFORE the beat too: a beat starting at 11s sent it 15
        # seconds of frames to rebuild for 2.75 seconds of screen time, and one render sat on the
        # GPU for 37 minutes doing it. Cutting first means ProPainter only ever sees the frames
        # the viewer sees; the scene then plays the cut file from its start.
        # A little slack on both sides so a speed change or a nudged in-point still lands inside.
        lead = min(trim, 0.35)
        span = wanted + lead + 0.6
        # The window is cut out of whatever the scene currently points at, which may itself be a
        # cleaned derivative - so a window cut before the clean version moved can still carry the
        # old pass's misplaced patch. Carry the version in the name; the cost of being wrong here
        # is a smeared block in the finished video, the cost of being right is one ffmpeg cut.
        cut = path.with_name(f"win{caption_remover.CLEAN_VERSION}_{path.stem[:26]}_"
                             f"{int((trim - lead) * 1000):07d}.mp4")
        try:
            if not (cut.exists() and cut.stat().st_size > 4096):
                subprocess.run([ffmpeg, "-y", "-v", "error", "-ss", f"{max(0.0, trim - lead):.3f}",
                                "-i", str(path), "-t", f"{span:.3f}",
                                "-c:v", "libx264", "-preset", "veryfast", "-crf", "18",
                                "-pix_fmt", "yuv420p", "-c:a", "copy", str(cut)],
                               check=True, stdin=subprocess.DEVNULL)
        except Exception:                                               # noqa: BLE001
            cut = None
        target = cut if (cut and cut.exists() and cut.stat().st_size > 4096) else path
        log(status_cb, f"Timeline render: {path.name} carries text on {coverage:.1%} of the "
                       f"frame; cleaning the {span:.1f}s this scene shows.")
        try:
            # In place, and it answers 1 or 0 - not a path. Treating the return as a filename
            # meant a successful clean was recorded as a failure and the scene kept pointing at
            # the file it had already rewritten.
            done = caption_remover.remove_caption_regions(
                str(target), ffmpeg, seconds=span + 0.4, status_cb=status_cb)
        except Exception as exc:                                        # noqa: BLE001
            log(status_cb, f"Timeline render: caption removal failed on {path.name} "
                           f"({type(exc).__name__}); rendering it as it is.")
            continue
        if done and target is not path:
            # The scene now plays the cut file, so its old seek offset would skip real picture.
            scene["clip"] = scene["asset"] = target.name
            for key in ("seedance_start_trim", "source_trim", "start_trim"):
                if key in scene:
                    scene[key] = round(lead, 3)
        if done:
            cleaned += 1
        elif look_elsewhere():
            # The rebuild failed or was refused, and the beat would go out with the creator's
            # words burned across it: the same clip without them, or another window entirely.
            # look_elsewhere writes down which of those happened - this branch used to be
            # completely silent, and it is the one that fires most often.
            cleaned += 1
    # The warm filler holds ~5GB of the card between clips, which is right while there are clips
    # left and wrong the moment there are none: the frame render wants that memory next.
    try:
        caption_remover.stop_propainter_worker()
    except Exception:                                                   # noqa: BLE001
        pass
    if cleaned or unboxed:
        log(status_cb, f"Timeline render: cleaned text off {cleaned} clip(s) and rebuilt "
                       f"{unboxed} letterboxed clip(s).")
...
def render_project_timeline(slug, edits, status_cb=None, cancel_event=None,
    ...
    # The user pressed Render: this is the one place the GPU model is allowed to run.
    apply_timeline_edits_to_config(config, edits, slug, allow_gpu=True)
    ...
    prepare_timeline_clips(config, project_dir, status_cb=status_cb)
    # AND AGAIN, NOW THAT IT IS TRUE. prepare_timeline_clips swaps footage - a clip whose text
    # cannot be rebuilt is dropped for another - so the copy written above records what this
    # render INTENDED. Tracing a finished video back to that file therefore pointed at a clip
    # the render never used. This is the file the editor reopens and the one a later trace
    # reads, so it has to be the plan that actually shipped.
    out_config_path.write_text(json.dumps(config_for_json(config), indent=2), encoding="utf-8")
    ...
    quality = config.get("editorial_quality", {"status": "unverified"})
    log(status_cb, f"Timeline export complete: {Path(output).name}; editorial quality: {quality['status']}")
    return {"title": config.get("title", slug), "project_dir": str(project_dir), "video": str(output),
            "editorial_quality": quality}


# Keys that hold LIVE handles rather than data: a threading.Event, the status callback, the
# path of the config being written. They belong to the run, not to the configuration, and they
# are exactly what a deepcopy cannot follow - a threading.Event owns a _thread.lock, so copying
# a config that carried one raised "cannot pickle '_thread.lock' object" and killed Redo SFX
# before it started.
CONFIG_RUNTIME_KEYS = ("_cancel_event", "_status_cb", "_config_path")


def config_variant(config, **overrides):
    """A deep copy of a render config that SHARES its live handles instead of copying them.

    Cancelling has to reach every variant, and the status callback has to keep writing to the
    same job log, so those are carried across by reference on purpose.
    """
    variant = copy.deepcopy({key: value for key, value in config.items()
                             if key not in CONFIG_RUNTIME_KEYS})
    for key in CONFIG_RUNTIME_KEYS:
        if key in config:
            variant[key] = config[key]
    variant.update(overrides)
    return variant
...
def rework_project_sfx(slug, mode="add", reasoning_model=None, sfx_amount="medium",
    ...
    if config.get("clip_source") == "scrape":
        ...
        planned_config = config_variant(config)
    ...
    probe_config = config_variant(config)
...
def replace_timeline_scrape_scenes(slug, scene_ids, status_cb=None, cancel_event=None,
    ...
    try:
        per_clip = max(2.25, min(2.75, max(float(scene.get("end", 0))
                                         - float(scene.get("start", 0))
                                         for scene in target_scenes)))
    except Exception:
        per_clip = 2.3
    ...
    candidate_statuses = []
    got = []
    if backend_up and str(engine or "").lower() == "v3":
        # The SAME engine a fresh run uses, aimed at the marked clips only: it plans visual
        # chapters for those scenes and searches per chapter, instead of flattening them into
        # one keyword bucket. Asked for by name - "a whole scrape just like the v3 one for the
        # selected clips in the timeline".
        import scrape_v3
        clip_scraper.reset_backend_search_health()
        got = scrape_v3.gather_candidates_for_scenes(
            config, target_scenes, project_dir, platforms,
            script_text=full_script, reasoning_model=reasoning_model,
            status_cb=status_cb, cancel_check=cancelled,
            time_budget=float(config.get("scrape_time_budget") or 900.0)) or []
    elif backend_up:
...
def run_project(form, status_cb=None):
    ...
    reasoning_model = form.get("reasoning_model") or "google/gemini-3.7-flash"
    ...
    assert_wavespeed_balance(status_cb=status_cb, model=form.get("reasoning_model") or "google/gemini-3.7-flash")
    ...
    if audio_path and form_flag(form, "use_audio_timing", True):
        ...
        if not audio_analysis:
            try:
                # Path(...) because audio_path is a STRING on a reuse run (set above), and
                # `.name` on a str raises AttributeError - inside the try, so the fallback
                # died here and was logged as "fell back to estimated text timing" without
                # Gemini ever being called. Local alignment was the only route left, and
                # when it failed the run cut to a words-per-minute guess.
                log(status_cb, f"Analyzing audio with Gemini 3.5 Flash: {Path(audio_path).name}")
    ...
    if timing_info["is_estimated"]:
        # A CLIP SHORT IS CUTS ON THE VOICE. Everything downstream - beat boundaries, window
        # lengths, the whole reason this mode exists - is derived from where the words actually
        # fall, and "estimated" means they were divided by a words-per-minute guess instead.
        # Measured on a real run (2026-09-08 23:16): forced alignment timed out, the fallback
        # failed too, word_timestamps.json was written as `[]`, and the run continued and spent
        # the entire scrape budget on a two-line even split. It logged this one warning, which
        # nobody saw. The scrape is the expensive step and it comes AFTER this line, so this is
        # the last place where refusing costs nothing.
        _footage = str((form.get("clip_source") if isinstance(form, dict) else "") or "").strip().lower()
        if _footage == "scrape" and not str(form.get("allow_estimated_timing") or "").strip():
            raise RuntimeError(
                f"Voice timing is estimated ({target_duration:.1f}s) - the forced alignment "
                f"produced nothing, so nothing can be cut to the voice. Look at "
                f"{project_dir / 'input' / 'audio_analysis.json'} (timing_source) and "
                f"{project_dir / 'voice' / 'voice_source.json'} (is_estimated). NOT at "
                f"word_timestamps.json: voice_timing.py writes that as [] unconditionally, so it "
                f"is empty whether alignment worked or not. Rerun the alignment, or set "
                f"allow_estimated_timing=1 to proceed deliberately.")
    ...
    micro_beat_result = build_micro_beat_plan(
        title, script, visual_script, target_duration, base_scenes,
        reasoning_model=reasoning_model, status_cb=status_cb,
        collaborate=collaborate_reasoning,
        footage_source=str(form.get("clip_source") or "generate").strip().lower())
    ...
    if str(form.get("clip_source", "generate") or "generate").strip().lower() == "scrape":
        ...
        # SPLIT AFTER THE WORD SYNC, NOT BEFORE IT. Splitting first and syncing second let the
        # sync pull the halves back apart: a run capped at 2.4s came out with beats of 2.83s and
        # 3.24s and a 2.37s median, against 1.14s in the reference edit. The word timeline decides
        # where the CAPTIONS land; it has no say in how long a PICTURE may stay up, and captions
        # are word-windowed downstream so a cut inside a word costs nothing.
        before_n = len(scenes_override)
        # 2.45s came from a reference Short cut out of footage its editor already had. This
        # block runs ONLY for clip_source == "scrape", where every beat is a separate video that
        # has to be found - so the cap was quietly undoing the beat budget the planner works to.
        # Measured twice on 2026-09-03: a planned 8 beats of ~3.75s became 17 at a 2.45s cap, and
        # a planned 7 beats of ~5s still became 13 at 4.2s. The planner is asked for a shot every
        # 2.5-4s and lands a little over that; the cap has to sit ABOVE where it lands, or the
        # split silently doubles the number of clips the run has to find.
        scenes_override = enforce_reference_pacing(
            scenes_override,
            max_s=(3.2 if mini_story_mode
                   else (3.6 if str(form.get("clip_source", "generate") or "").strip().lower()
                         == "scrape" else 5.0)))
        if len(scenes_override) != before_n:
            log(status_cb, f"Pacing: split long beats into readable Fact Short shots "
                           f"({before_n} -> {len(scenes_override)} beats).")
        before_stabilize = len(scenes_override)
        # 1.8s here overrode the function's own floor and set the real pace of every clip short:
        # split at 2.4, merge under 1.8, so the edit could only ever land in a 1.8-2.4s band and
        # measured out at a 2.35s median. The reference edit runs 1.14s. The floor exists to stop
        # a 1.0s cut on footage that carries its own edits - 1.15s keeps that and stops forbidding
        # the pace that actually holds attention.
        _merge_floor = 1.6 if mini_story_mode else 1.15
        scenes_override = coalesce_short_scrape_scenes(
            scenes_override,
            min_s=_merge_floor,
            max_s=(4.2 if mini_story_mode else 3.6))
        if len(scenes_override) != before_stabilize:
            log(status_cb, f"Pacing: merged isolated sub-{_merge_floor:.2f}s beats "
                           f"({before_stabilize} -> {len(scenes_override)}) to prevent rapid "
                           f"double-cuts in TikTok footage.")
        # THE OPENER, LAST. Applying a hook minimum inside the beat planner is undone here: the
        # beats are re-derived from the audio timing and re-paced afterwards, and a measured run
        # came out with a 1.04s opening shot. This is the end of the timing chain.
        if len(scenes_override) > 1:
            _first, _second = scenes_override[0], scenes_override[1]
            _short_by = HOOK_MIN_ON_SCREEN_S - (float(_first["end"]) - float(_first["start"]))
            _room = (float(_second["end"]) - float(_second["start"])) - MIN_BEAT_ON_SCREEN_S
            _take = min(_short_by, _room)
            if _take > 0.05:
                _first["end"] = round(float(_first["end"]) + _take, 3)
                _second["start"] = round(float(_first["end"]), 3)
                log(status_cb, f"Pacing: held the opening shot to "
                               f"{float(_first['end']) - float(_first['start']):.2f}s so it "
                               f"registers before the scroll.")
        ...
        # Scraped footage is a SUPPLY problem, and the chapter count is the demand. Measured on
        # a live V4 run 2026-09-02: a 24s voiceover was cut into 15 chapters at 2.3s, so the run
        # needed 15 unique verified clips; discovery produced 22 accepted verdicts covering about
        # eight distinct sources, five beats were filled and the run refused to render. Holding a
        # shot for 3.0s instead asks for roughly eight chapters, which is what a run of this kind
        # actually delivers - and 3s cuts are ordinary short-form pacing, not slow ones.
        _scraping = str(form.get("clip_source", "generate") or "generate").strip().lower() == "scrape"
        _target_s = 3.0 if mini_story_mode else 2.55
        _max_s = 3.4 if mini_story_mode else 3.6
        if _scraping:
            _target_s = 2.55
            _max_s = 3.6
        scenes_override = coalesce_scrape_visual_chapters(
            scenes_override, min_s=1.55, target_s=_target_s, max_s=_max_s,
            preserve_hook=False,
        ...
        if canonical_words:
            scenes_override = sync_scenes_to_voice_timeline(
                scenes_override, canonical_words, target_duration=target_duration)
        _before_final_pace = len(scenes_override)
        # The hard gate has to agree with the target above, or it simply splits every chapter the
        # coalesce step just built and the demand goes straight back to fifteen clips.
        _hold_cap = _max_s if (mini_story_mode or _scraping) else 2.75
        scenes_override = enforce_fact_short_max_hold(scenes_override, max_s=_hold_cap)
        if len(scenes_override) != _before_final_pace:
            # The exact words are immediately reassigned to the newly split visual
            # spans. Captions stay voice-synchronous while the footage progresses.
            if canonical_words:
                scenes_override = sync_scenes_to_voice_timeline(
                    scenes_override, canonical_words, target_duration=target_duration)
            log(status_cb, f"Pacing hard gate: split {len(scenes_override) - _before_final_pace} "
                           f"overlong visual hold(s); clips max out at {_hold_cap:.2f}s.")
    ...
    # A form that carries scrape settings but no clip_source is a Clip Short whose source field
    # went missing, and defaulting THAT to "generate" is the worst possible guess: it silently
    # spends a video model on a short that asked for real footage. One shipped project was
    # produced this way - its run_form held search_terms, script_relevancy and a scrape budget,
    # and every clip in it was generated. Believe the settings, and say so out loud.
    if clip_source == "generate" and not str(form.get("clip_source") or "").strip():
        scrape_markers = [key for key in ("search_terms", "scrape_terms", "scrape_platforms",
                                          "scrape_rounds", "scrape_time_budget", "scrape_sort",
                                          "script_relevancy", "scrape_version", "scraping_engine")
                          if str(form.get(key) or "").strip()]
        if scrape_markers:
            clip_source = "scrape"
            log(status_cb, "This run carries scrape settings (" + ", ".join(scrape_markers[:4])
                + ") but no clip source. Treating it as a Clip Short and searching for real "
                  "footage - generating clips with a video model here would be wrong and "
                  "expensive.")
    ...
    if clip_source == "scrape" and not reused_existing_scrape:
        ...
        if clip_scraper is not None:
            ...
            try:
                per_clip = min(2.75, max(2.25, max(float(s["end"]) - float(s["start"]) for s in scenes_override)))
            except Exception:
                per_clip = 2.3
            ...
            # Scraping engine selection. V3 is the production Clip Short default: it plans visual
            # chapters, searches several independent query hypotheses, inspects the downloaded
            # footage and keeps honest gaps when nothing usable survives. This fallback matters for
            # old saved presets/projects that do not carry the newer ``scraping_engine`` field.
            # V4, not V3. A saved preset or an older project carries no ``scraping_engine``, and
            # defaulting those to V3 quietly ran them on the previous engine - none of the V4
            # gates (measured caption coverage, the vision review, the duplicate policy) applied,
            # and the run looked like a V4 run in the UI. Clip Short is V4; an unset value means
            # "not recorded yet", not "the old engine".
            _scrape_engine = str((form.get("scraping_engine") if isinstance(form, dict) else None)
                                 or "v4").strip().lower()
            if _scrape_engine not in ("v1", "v2", "v3", "v4"):
                _scrape_engine = "v4"
            _v3 = (_scrape_engine == "v3")
            # V4 is an isolated Bright-Data-only editor.  It is intentionally not a V2/V3
            # compatibility mode: discovery, candidate manifest and duplicate policy live in
            # scrape_v4.py and no browser/TikTok-login fallback can run on this path.
            _v4 = (_scrape_engine == "v4")
            # V3 is a different editor, not a different matcher: it plans visual CHAPTERS, sources
            # a pool per chapter and cuts the timeline to the footage it found. It shares the
            # "not the V1 bucket engine" gating below, which is what _v2 actually selects for.
            _v2 = _scrape_engine in ("v2", "v3")
            if _v2:
                import scrape_v2
            if _v3:
                import scrape_v3
            ...
            # Scene 0 always participates in normal topic matching.
            use_influencer_hook = False
            ...
            with step_watchdog(form, "Clip scrape", limit_s=0, status_cb=status_cb):
                if _v4:
                    _v4cfg = {"title": title,
                              ...
                              "scrape_terms": form.get("scrape_terms"),
                              # V4 keeps planning, review and logging cheap: one fast
                              # multimodal vision verdict per source, no duplicate critic.
                              "v4_vision_model": (str(form.get("vision_model") or "").strip()
                                                  or "google/gemini-3.7-flash"),
                              # Kept in the project config (rather than process env) so the
                              # Bright-only media fetcher can be verified and resumed from the UI.
                              # This is a zone *name*, never an API key.
                              "bright_unlocker_zone": str(form.get("bright_unlocker_zone") or "").strip(),
                              # Scrape.do renders the real TikTok result page and returns only
                              # canonical post URLs. The existing TikTok session then opens
                              # those selected URLs for delivery; it never keyword-searches.
                              "v4_tiktok_discovery_provider": str(form.get("v4_tiktok_discovery_provider") or "scrapedo").strip(),
                              "v4_scrapedo_geo": str(form.get("v4_scrapedo_geo") or "jp").strip(),
                              # V4 has a second, independent Bright collector: Instagram Reels
                              # via ``url_all_reels``. It is never a browser/login fallback.
                              "v4_instagram_enabled": form_flag(form, "v4_instagram_enabled", True),
                              "v4_instagram_accounts": str(form.get("v4_instagram_accounts") or "").strip(),
                              # Bright stays primary.  This only recovers an already-selected
                              # post through the user's existing TikTok session when Bright
                              # discovery/media delivery explicitly fails.
                              "v4_tiktok_login_fallback": form_flag(form, "v4_tiktok_login_fallback", True),
                              "v4_tts_model": "pro",
                              "v4_tts_voice": "Laomedeia",
                              "v4_tts_speaker": "Narrator"}
                    _v4scenes, _v4clips = scrape_v4.scrape_social_plan_v4(
                        _v4cfg, scenes_override, project_dir, scrape_platforms,
                        per_clip_seconds=per_clip, script_relevancy=script_relevancy,
                        cookies=_cookies, cancel_check=_cancel,
                        understanding=script_understanding, reasoning_model=reasoning_model,
                        status_cb=status_cb, script_text=script)
                    if _v4scenes:
                        scenes_override[:] = _v4scenes
                        scene_total = len(scenes_override)
                    scene_clips = list(_v4clips or [None] * scene_total)
                    ...
                    hook_results = []
                    if isinstance(form, dict):
                        form["_scrape_v4_used"] = True
                        form["_scrape_v4"] = _v4cfg.get("_scrape_v4") or {}
                elif _v3:
                    # SCRAPE V3: editorial chapters. It groups the narration into 4-6 visual
                    # ideas, sources and INSPECTS a pool per chapter, then rebuilds the scene list
                    # around the windows it chose - so the timeline follows the footage instead of
                    # the footage having to satisfy a timeline fixed before the search began.
                    _v3cfg = {"title": title,
                              "voice_speed": resolve_voice_speed(form, "scrape"),
                              "scrape_time_budget": form.get("scrape_time_budget"),
                              "scrape_terms": form.get("scrape_terms")}
                    _v3scenes, _v3clips = scrape_v3.scrape_social_plan_v3(
                        _v3cfg, scenes_override, project_dir, scrape_platforms,
                        per_clip_seconds=per_clip, script_relevancy=script_relevancy,
                        cookies=_cookies, cancel_check=_cancel,
                        understanding=script_understanding, reasoning_model=reasoning_model,
                        status_cb=status_cb, script_text=script)
                    if _v3scenes:
                        # The scene list IS the result here - replace it in place so every later
                        # step (captions, render, timeline editor) reads the chaptered edit.
                        scenes_override[:] = _v3scenes
                        scene_total = len(scenes_override)
                    scene_clips = list(_v3clips or [None] * scene_total)
                    while len(scene_clips) < scene_total:
                        scene_clips.append(None)
                    clip_decision_log = [None] * scene_total
                    best_hook = None
                    hook_results = []
                    if isinstance(form, dict):
                        form["_scrape_v3_used"] = True
                        form["_scrape_v3"] = _v3cfg.get("_scrape_v3") or {}
                elif _v2:
                    ...
                    _v2cfg = {"title": title, "voice_speed": resolve_voice_speed(form, "scrape"),
                              ...
                              # how long this topic is worth searching for is the user's call
                              "scrape_time_budget": form.get("scrape_time_budget"),
                ...
                if use_influencer_hook and hook_pool and not _v2:
                    ...
                    def _hook_likes_ok(result):
                        ...
                        floor = clip_scraper.platform_like_floor(
                            MIN_CLIP_LIKES, result_meta.get("platform", "tiktok"))
                ...
                if use_influencer_hook and best_hook is None and hook_pool and not _v2:
                    best_hook = max(
                        hook_pool,
            ...
            for scene_index in ([] if (strict_material_only or _v2 or _v3 or _v4)
                                else range(1, scene_total)):
            ...
            social_search_report = {
                ...
                "hook_finder": {
                    ...
                    "minimum_likes": MIN_CLIP_LIKES,
                    "minimum_likes_by_platform": {"tiktok": MIN_CLIP_LIKES},
                    "queries": {},
            ...
            for _idx in range(len(scene_clips)):
                ...
                # V3 deliberately uses separated, cut-free windows from one long source inside
                # the SAME visual chapter. The legacy global duplicate cleaner must not erase
                # those approved windows after V3 has fitted them to the narration.
                _is_dup = (not (_v3 or _v4)) and _key in _used_keys
                ...
                _is_capt = (not _v2 and not _v3 and not _v4) and _cap_score(_clip) > _CAP_MAX
            ...
            for i, sc in enumerate(scenes_override):
                ...
                # A BORROWED BEAT ALREADY CARRIES ITS CLIP. `scene_clips` holds only the paths the
                # matcher assigned; borrow_motion_for_uncovered and the spare-clip path write
                # straight onto the scene and can leave that parallel list empty. Reading only the
                # list silently undid every borrow here - the scrape reported "0 still fallbacks"
                # and four borrowed beats, and the saved project came back with blank scenes and
                # the same clip repeated. The V2 rescue further down is gated `not _v2`, so nothing
                # refilled them either. Resolve what the scene already names before discarding it.
                # Resolve a name that does not exist as given, too - not only an empty entry. An
                # engine that put a bare filename in this list produced a truthy value that
                # failed `Path(clip).exists()` against the working directory, and the scene was
                # dropped without a word: measured 2026-09-02, V4 assigned 10 of 10 beats with
                # every file on disk and the saved project came back with one clip and nine
                # slates. The engine now returns full paths; this stops the class of fault.
                if not clip or not Path(clip).exists():
                    clip = resolve_assigned_scene_clip(sc, seedance_target_dir) or clip
                ...
                mclass = ((dlog_by_scene.get(i) or {}).get("match_class")
                          or sc.get("match_class")
                          or ("HOOK_MATCH" if role == "hook_influencer" else None))
                if clip and Path(clip).exists():
                    ...
                    try:
                        ...
                        sc["scrape_clip_id"] = (cm.get("clip_id") or sc.get("scrape_clip_id")
                                                or str(Path(clip).resolve()))
            ...
            if unmatched_scenes and placed and not (_v2 or _v3 or _v4):
                details = ", ".join(str(i) for i in unmatched_scenes[:12])
                report_path = project_dir / "review" / ("scrape_v3_report.json" if _v3
                                                        else "scrape_v2_report.json")
                ...
                if approved_sources:
                    ...
                    seedance_clip_count = len([s for s in scenes_override if s.get("clip")])
            elif unmatched_scenes and (_v2 or _v3 or _v4):
                ...
                log(status_cb, f"Scrape {'V4' if _v4 else ('V3' if _v3 else 'V2')} WARNING: "
                               f"{len(unmatched_scenes)} scene(s) remain uncovered "
                               f"after relevance recovery; continuing with explicit replacement "
                               f"slots, never unrelated continuity footage. Details: {report_path}")
            ...
            scrape_enforcement = {
                ...
                "scene_meta": [{"visual_role": sc.get("visual_role"),
                                ...
                                # A beat filled with the best remaining source instead of a clip
                                # found FOR it. The timeline editor draws these with a red border
                                # and a NO MATCH tag so they can be replaced - but plan_config
                                # rebuilds the scene list, and only the fields named here survive
                                # it. Without them the flag was set by the scraper and silently
                                # gone by the time the project was saved.
                                "coverage_gap": sc.get("coverage_gap"),
                                "coverage_gap_reason": sc.get("coverage_gap_reason"),
                                "v4_reason": sc.get("v4_reason"),
                                "has_clip": bool(sc.get("clip"))}
    ...
    if clip_source == "scrape" and (_v3 or _v4):
        # V3 uncovered chapters are honest empty edit slots, not permission for plan_config to
        # invent a PNG filename. The previous phantom assets did not exist on disk and made the
        # timeline look like a finished six-scene edit even though only two scenes had footage.
        for source_scene, config_scene in zip(scenes_override, config.get("scenes", [])):
            if str(source_scene.get("assignment_type") or "") != "uncovered":
                continue
            config_scene["seedance"] = False
            config_scene["needs_gpt_asset"] = False
            config_scene["asset"] = None
            config_scene["clip"] = None
            config_scene["shots"] = []
            config_scene["assignment_type"] = "uncovered"
    config["project_slug"] = slug
    config["loaded_project_mode"] = recut_mode
    if clip_source == "scrape":
        # Persist the engine that actually produced the edit. Besides making diagnostics honest,
        # this lets every later save/rework/dedup pass retain V3's reviewed-window rules even if
        # the report folder is moved or an older project form omitted the selector.
        config["scraping_engine"] = _scrape_engine
        config["_scrape_engine"] = _scrape_engine
    ...
    # an untouched project can no longer silently change the caption-cleanup behaviour. It no
    # longer blurs: the pass REMOVES the text by inpainting, because a blur through a glyph-tight
    # mask only smooths each letter into itself and the word stays readable.
    # Default ON only where vision actually saw a caption. It used to be on for every source
    # clip, which on one festival project meant eight fills for the one clip that carried text.
    # An explicit choice - the inspector checkbox or the clip menu - still wins either way.
    _caption_default_edits = {
        "scenes": [
            {"id": str(sc.get("id")),
             "blur_captions": (sc.get("blur_captions") is not False
                               if sc.get("blur_captions") is not None
                               else bool(sc.get("source_has_captions")))}
            for sc in (config.get("scenes") or []) if sc.get("clip")
        ]
    }
    if _caption_default_edits["scenes"]:
        log(status_cb, "Caption cleanup: burned-in captions are removed by default for source "
                       "footage (inpainted, not blurred).")
        # allow_gpu, because that sentence was a promise the call did not keep. Without it the
        # remover falls back to the CPU fill, and a delivered Clip Short (2026-08-29) still had
        # the source's own Japanese captions burned across the last shot. This pass is the
        # pre-render cleanup of a run that ALWAYS ends in a render, which is exactly the
        # "explicit render" the GPU path was reserved for - not a background scrape.
    ...
    config["wavespeed"]["reasoning_model"] = form.get("reasoning_model") or "google/gemini-3.7-flash"
    ...
    if clip_source == "scrape":
        ...
        for _i, _cs in enumerate(config.get("scenes", [])):
            if _i < len(_sm):
                ...
                # RESTORE IT, DO NOT OVERWRITE IT. This runs AFTER the caption cleanup
                # (`_caption_default_edits`, ~170 lines up), and that pass can legitimately move
                # the in-point: a window crossing an internal source cut is slid to the longest
                # clean segment, and the clip is then cleaned at the moved second. Stamping the
                # scrape's original trim back over it would leave the render playing a window
                # that was never cleaned and still crosses the cut. plan_config now carries the
                # trim itself, so for a scraped scene this branch has nothing left to do.
                if (_sm[_i].get("seedance_start_trim") is not None
                        and _cs.get("seedance_start_trim") is None):
                    _cs["seedance_start_trim"] = _sm[_i].get("seedance_start_trim")
                _cs["borrowed_from_scene"] = _sm[_i].get("borrowed_from_scene")
                if _sm[_i].get("coverage_gap"):
                    _cs["coverage_gap"] = True
                    _cs["coverage_gap_reason"] = _sm[_i].get("coverage_gap_reason") or ""
                if _sm[_i].get("v4_reason"):
                    _cs["v4_reason"] = _sm[_i].get("v4_reason")
    ...
    # VFX layers chosen at "select the final layers": arrows are baked into the render.
