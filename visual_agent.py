...
def enhance_video_with_arrows(video_path, reasoning_model=None, status_cb=None, out_dir=None,
    ...
    effects, char_events, meme_events = analyze_effects(
        ...
        status_cb=status_cb, emotions=[], memes={}, vfx_amount=vfx_amount)
    if not add_arrows:
    ...
    segments = _overlay_sfx_segments(effects, ffprobe, status_cb=status_cb)
    if segments:
    ...
    # Editable project: split the CLEAN original into scene clips and store every arrow as an
    ...
    return {"video": str(out_path), "original_video": str(video_path), "visual_plan": str(plan_path),
            "arrow_count": n_arrow, "plan_source": "opus_vision",
