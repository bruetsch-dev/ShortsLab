...
# GEMINI TTS WAS WITHDRAWN (2026-08-20). Every google/gemini-*/text-to-speech slug now answers
# 400 "Model not found", so a run generated no voiceover at all and silently fell back to
# ESTIMATED word timing - captions and cuts drift against a track that does not exist. Saved run
# forms still carry tts_model "pro" or "flash", so the aliases stay and resolve to Seed, which
# this module already supports end to end. Probed the same day: omnivoice, minimax/speech-02-turbo
# and elevenlabs/multilingual-v2 are also alive if Seed ever goes the same way.
# Gemini TTS is alive and its aliases point at Google's own endpoints again. They had all been
# remapped onto the ByteDance model, which silently destroyed narrator selection: `is_seed` then
# matched on the RESOLVED id, so every Gemini narrator name failed the Seed voice check and fell
# back to `stokie_en` - all thirty names produced the identical voice, and the UI showed Seed
# voices and the Seed settings box under a heading that said Gemini.
#
# The remap was a workaround for the wrong payload, not for a dead endpoint. WaveSpeed's schema
# for these models takes the narrator as a TOP-LEVEL `voice` string; this code sent a `speakers`
# array instead, and the provider failed every such job ("could not complete the request", 1405).
# Verified 2026-08-27: top-level `voice` completes, the speakers array fails, same model, same
# minute.
GEMINI_TTS_MODELS = {
...
def tts_provider(model):
    """Which provider a model alias ACTUALLY reaches - the one truth both the UI and the server
    must agree on. Reading the alias name instead let the UI offer Gemini narrators for a job
    that ran on ByteDance, where those names do not exist."""
    requested = str(model or DEFAULT_TTS_MODEL).strip()
    resolved = GEMINI_TTS_MODELS.get(requested, requested)
    if requested in INWORLD_TTS_ALIASES or resolved == INWORLD_TTS_MODEL:
        return "inworld"
    if requested in SEED_SPEECH_TTS_ALIASES or resolved == SEED_SPEECH_TTS_MODEL:
        return "seed"
    return "gemini"


def tts_voices_for(model):
    """The voices that will actually work for this model."""
    return {"seed": SEED_SPEECH_TTS_VOICES,
            "inworld": INWORLD_TTS_VOICES}.get(tts_provider(model), GEMINI_TTS_VOICES)


def tts_text_limit(model):
    """Characters this provider accepts in ONE call.

    Only Inworld declares one (2000). Seed declares none and was measured instead: 4500 chars
    completed, 5200 failed. The chunker asks here rather than carrying one global number, so a
    limit that is right for one provider cannot silently break another.
    """
    return INWORLD_TEXT_LIMIT if tts_provider(model) == "inworld" else 0
...
# Seed ignores the Gemini `style` directive by design - it takes its direction through
# `voice_instruction` instead - so every Gemini style needs a Seed twin or the model falls back
# to SEED_DEFAULT_VOICE_INSTRUCTION, which is written for a 30-second Short. A twenty-minute
# explainer read with "strong emphasis on the hook and key words" is exactly the wrong register:
# it never settles, and the emphasis lands on words that are not the point.
# ---- Inworld Realtime TTS-2 -------------------------------------------------------------
# A third provider with a third way of taking direction. Seed reads `voice_instruction`, Gemini
# reads a directive prefixed to the text, and Inworld reads a SQUARE-BRACKET stage direction on
# its own line before the script - the model's own example is:
#   [speak quickly, clearly, and calmly, like an automated travel notification]
#   Your flight confirmation code is 9 2 7 6 A.
# Measured: the same sentence ran 9.1s plain and 11.3s with a "calm, unhurried" direction, so
# the bracket is read as performance and not spoken.
INWORLD_TTS_MODEL = "inworld/realtime-tts-2"
INWORLD_TTS_ALIASES = {"inworld", "realtime-tts-2", INWORLD_TTS_MODEL}
# The endpoint declares maxLength 2000 on `text` - the only TTS here that states a limit at all.
INWORLD_TEXT_LIMIT = 2000
INWORLD_TTS_VOICES = [
    "Alex", "Ashley", "Craig", "Deborah", "Dennis", "Edward", "Elizabeth", "Hades", "Julia",
    "Pixie", "Mark", "Olivia", "Priya", "Ronald", "Sarah", "Shaun", "Theodore", "Timothy",
    "Wendy", "Dominus", "Hana", "Clive", "Carter", "Blake", "Luna", "Yichen", "Xiaoyin",
    "Xinyi", "Jing", "Erik", "Katrien", "Lennart", "Lore", "Alain", "Hélène",
    "Mathieu", "Étienne", "Johanna", "Josef", "Gianni", "Orietta", "Asuka", "Satoshi",
    "Hyunwoo", "Minji", "Seojun", "Yoona", "Szymon", "Wojciech", "Heitor", "Maitê",
    "Diego", "Lupita", "Miguel", "Rafael", "Svetlana", "Elena", "Dmitry", "Nikolai", "Riya",
    "Manoj", "Yael", "Oren", "Nour", "Omar",
]
INWORLD_DEFAULT_VOICE = "Dennis"
INWORLD_LONGFORM_DIRECTION = (
    "speak calmly and unhurried, like a documentary narrator, with clear enunciation and "
    "real pauses between sentences")
INWORLD_SHORT_DIRECTION = (
    "speak with bright, upbeat energy like a short-form creator, punchy but never shouting")
INWORLD_SHORT_HOOK_DIRECTION = (
    "open with confident energy, attack the first words and land the last one deliberately, "
    "like the hook of a viral educational short")


def flatten_for_tts(text):
    """Join lines that a pasted script only broke for READING width.

    A hard-wrapped script carries newlines in the middle of sentences, and every TTS engine here
    reads a line break as a pause - measured on Inworld with the same three sentences: four
    pauses over 0.45s when hard-wrapped, one when flattened. That is the "weird pause in the
    middle of a sentence".

    Paragraph breaks are kept as ONE blank line: they are a real beat, and collapsing them makes
    twenty minutes of narration run together.
    """
    blank_line = chr(10) + chr(10)
    paragraphs = re.split(r"\n\s*\n", str(text or "").strip())
    joined = [" ".join(p.split()) for p in paragraphs if p.strip()]
    return blank_line.join(joined)


# The same performance, said the way each provider wants to hear it. Without this a Gemini
# briefing reaches Inworld verbatim as a 400-character bracket line.
INWORLD_STYLE_EQUIVALENTS = {}


def _register_inworld_equivalents():
    INWORLD_STYLE_EQUIVALENTS.update({
        TTS_STYLE_SKETCH_SHORT_HOOK: INWORLD_SHORT_HOOK_DIRECTION,
        TTS_STYLE_SKETCH_SHORT: INWORLD_SHORT_DIRECTION,
        TTS_STYLE_LONGFORM: INWORLD_LONGFORM_DIRECTION,
        TTS_STYLE_DIRECTIVE: INWORLD_SHORT_DIRECTION,
    })


def inworld_direction(text, direction):
    """Prefix a square-bracket stage direction, the way this model takes performance notes.

    Returns the text unchanged when there is no direction, and never stacks a second bracket on
    a line that already carries one.
    """
    body = str(text or "").strip()
    note = " ".join(str(direction or "").split()).strip("[]").strip()
    if not note or body.startswith("["):
        return body
    return "[" + note + "]" + chr(10) + body


SEED_LONGFORM_VOICE_INSTRUCTION = (
    "Calm, measured documentary narrator reading a long explainer. Warm, thoughtful and "
    "unhurried, at a steady conversational pace with real pauses at full stops and a slightly "
    "longer breath between paragraphs. Let the sentence structure carry the emphasis instead of "
    "stressing individual words; lift only where the script itself turns - a surprise, a "
    "contrast, a conclusion - and settle back afterwards. Relaxed, clear enunciation that stays "
    "easy to listen to for twenty minutes. Never breathless, never salesy, no hook energy."
)

# The opening of a longform still has to earn the next thirty seconds - but it is a documentary
# opening, not a Short's. One notch of lift, then straight into the calm register above.
SEED_LONGFORM_OPENING_INSTRUCTION = (
    "This is the opening of a long explainer video. Begin with quiet confidence and clear "
    "intent, land the first question or claim deliberately, then settle into a calm, measured "
    "documentary pace. No shouting, no rising sell, no short-form hook attack."
)
...
# Sketch Explainer Shorts need a different performance from a long documentary.  The opening is
# intentionally a separate TTS take (see longform_video.generate_voiceover), which lets Gemini
# actually *perform* the hook instead of burying this instruction underneath a minute of calm
# narration.  The rest stays energetic and clear, not permanently shouty.
TTS_STYLE_SKETCH_SHORT = (
    "Read this as a sharp, upbeat educational Short: conversational, curious and easy to follow. "
    "Keep a brisk, confident pace with clean emphasis on the reveal words and short natural pauses. "
    "Sound engaged and lively, never flat, breathless, salesy or shouted"
)
TTS_STYLE_SKETCH_SHORT_HOOK = (
    "This is the opening hook of a vertical educational Short. Deliver the first sentence(s) as an "
    "immediate scroll-stopper: start decisively, lean into the surprising word or question, use a "
    "slightly faster, more animated pace and a confident rising-falling cadence. Make it feel like "
    "a creator revealing something fascinating. Strong and urgent, but never yelling, theatrical, "
...
def generate_speech_gemini(text, out_path, key=None, speaker=DEFAULT_TTS_SPEAKER,
    ...
    """
    key = key or api_key()
    # Reading-width line breaks are a pause to every engine here. Flattened once, centrally, so
    # a hard-wrapped script cannot produce a pause in the middle of a sentence on ANY provider.
    ...
    is_inworld = requested_model in INWORLD_TTS_ALIASES or model_id == INWORLD_TTS_MODEL
    if is_inworld:
        model_id = INWORLD_TTS_MODEL
    speaker = (str(speaker or "").strip() or DEFAULT_TTS_SPEAKER)
    if is_inworld:
        inworld_voice = str(voice or "").strip()
        if inworld_voice not in INWORLD_TTS_VOICES:
            inworld_voice = INWORLD_DEFAULT_VOICE
        # This model takes its performance note as a square-bracket line before the script, not
        # as a separate field. `style` is the Gemini directive and `voice_instruction` the Seed
        # one; either may arrive here, and both mean the same thing to a caller.
        note = str(voice_instruction or "").strip() or str(style or "").strip()
        if not note and auto_upbeat:
            note = INWORLD_SHORT_DIRECTION
        # A Gemini `style` is a 400-character briefing; Inworld's own example is one short
        # clause. Handing it the long one makes the delivery lurch - it reads as instruction
        # weight, not as performance. Map the known Gemini styles onto their Inworld twins and
        # trim anything else to a single clause.
        note = INWORLD_STYLE_EQUIVALENTS.get(note.strip(), note)
        if len(note) > 160:
            note = note.split(".")[0].strip()[:160]
        body = inworld_direction(text, note)
        if len(body) > INWORLD_TEXT_LIMIT:
            raise RuntimeError(
                f"Inworld accepts {INWORLD_TEXT_LIMIT} characters per call and this part is "
                f"{len(body)} - the caller must chunk with pipeline.tts_text_limit().")
        payload = {"text": body, "voice_id": inworld_voice,
                   "speaking_rate": float(tts_speed or 1.0),
                   "output_format": "MP3"}
        status_log(status_cb, f"Generating voiceover ({inworld_voice}) with {model_id}...")
    elif is_seed:
        ...
        # A delivery instruction typed by the user reached this branch and was dropped: only
        # `style` was consulted, and every caller leaves that at None (the default directive).
        # Gemini takes its directive through the same text prefix, so an explicit instruction
        # becomes the style here rather than silently doing nothing.
        directive = style
        if directive is None and str(voice_instruction or "").strip():
            directive = str(voice_instruction).strip()
        # The narrator is a TOP-LEVEL `voice`, per the model's own schema. The `speakers` array
        # is the multi-speaker form: sending it for a single narrator made the provider fail the
        # job every time (1405), which is what got all Gemini aliases remapped onto ByteDance -
        # and that remap is what made every narrator sound identical.
        payload = {
            "text": format_tts_script(speaker, text, style=directive),
    ...
    try:
        ...
        # Gemini TTS has vanished from this provider once before: on 2026-08-20 every
        # google/gemini-*/text-to-speech slug answered 400 "Model not found", a run produced NO
        # voiceover, and the pipeline carried on with estimated word timing - so captions and
        # cuts drifted against a track that did not exist. That is the failure this catch
        # exists for. Seed is always available and always has a usable voice, so a narrator
        # nobody chose is still better than a silent short with drifting captions.
        if is_seed:
            raise                       # Seed IS the fallback; there is nowhere left to go
        status_log(status_cb,
                   f"{'Inworld' if is_inworld else 'Gemini'} TTS failed ({type(exc).__name__}); "
                   f"falling back to {SEED_SPEECH_TTS_MODEL} so the run still gets a voiceover. "
                   f"The chosen narrator {voice!r} does not exist there, so it reads in "
                   f"stokie_en.")
        return generate_speech_gemini(
            text, out_path, key=key, speaker=speaker, voice="stokie_en",
            model=SEED_SPEECH_TTS_MODEL, language=language, cancel_event=cancel_event,
            status_cb=status_cb, style=style, voice_instruction=voice_instruction,
            auto_upbeat=auto_upbeat, tts_speed=tts_speed, volume=volume, pitch=pitch,
...
def run_subprocess_with_cancel(cmd, config=None):
    # NOTHING IS EVER TYPED AT THIS. ffmpeg watches stdin for its interactive keys, and the app
    # runs under a hidden console, so the child inherits a handle nobody writes to and has
    # something it can block on. This helper runs the metadata sanitize and the FINAL MUX of
    # every render, and the loop below polls with no timeout at all - so a child that never
    # exits hangs the render for ever rather than raising after an hour.
    #
    # The stall this was written after is measured (322 CPU-seconds, then 0.00 across two
    # 15-second samples, still alive after two hours, in action_editor's own runner). That stdin
    # CAUSED it is inferred and not established - see the longer note there. A precaution on the
    # widest-reach path, not a demonstrated fix.
...
def export_timeline_caption_pngs(config, chunks, width, height, out_dir, hook_end=0.0):
    """Export absolute-clock timeline captions without converting them back to scene time.

    Timeline renders previously skipped this export entirely, leaving stale manifests (and in
    one affected project tens of thousands of zero-length PNG entries) beside a newly rendered
    video.  The editor and the final burn now share the same small, canonical caption list.
    """
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    for old in out_dir.glob("caption_*.png"):
        try:
            old.unlink()
        except OSError:
            pass
    ...
    for idx, chunk in enumerate(chunks, 1):
        text = " ".join(str(w.get("text") or "") for w in chunk.get("words", [])).strip()
        if not text:
            continue
        start = round(float(chunk.get("start", 0.0) or 0.0), 3)
        end = round(max(start + 0.05, float(chunk.get("end", start) or start)), 3)
        image = render_caption_overlay(chunk, width, height, config, is_hook=(start < hook_end))
        fname = f"caption_{idx:03d}_{start:07.2f}s.png"
        image.save(out_dir / fname)
        manifest.append({
            "file": fname, "index": idx, "start": start, "end": end, "text": text,
            "words": [{"word": str(w.get("text") or ""),
                       "start": round(float(w.get("start", 0.0) or 0.0), 3),
                       "end": round(float(w.get("end", 0.0) or 0.0), 3)}
                      for w in chunk.get("words", [])],
        })
    (out_dir / "captions.json").write_text(
        json.dumps({"width": width, "height": height, "count": len(manifest),
                    "captions": manifest}, indent=2), encoding="utf-8")
    return manifest
...
def seedance_clip_start_trim(config, scene=None):
    """Where inside its source file a scene starts playing.

    Two different numbers used to share one clamp. The CONFIG value is a lead-in for generated
    clips - it skips the first half second of a Seedance render, and three seconds is a sane
    ceiling for that. The SCENE value is something else entirely: for scraped footage it is the
    in-point of the window the vision review chose, and that can be anywhere inside a two-minute
    TikTok. Clamping it to three seconds meant the edit played second 3.0 of the source instead.

    Measured 2026-09-05 across the finished projects:

        konbini   9 of 10 scenes had an in-point past 3s   (up to 47.6s)
        couples   7 of 10                                   (up to 27.9s)

    Every one of them rendered from second 3.0. That is how a window reviewed as "a smiling
    person holding up a phone case with couple photos" reached the screen as a wooden mallet in
    a yard, under the word COUPLES - and why the timeline editor showed a frame that had nothing
    to do with the render. The scene's own in-point is now taken as it is.
    """
    if scene is not None:
        raw = scene.get("seedance_start_trim") if hasattr(scene, "get") else None
        if raw is not None:
            try:
                ...
                pass
    try:
        value = float((config or {}).get("seedance_clip_start_trim", 0.5))
...
def render_blocks_enabled(config):
    """Are the hard render gates allowed to REFUSE a render? Off by default, on the user's
    instruction - the checks still run and still report, they just no longer stop the render."""
    try:
...
def clip_ref_source_path(ref):
    """The ABSOLUTE file a scene reference points at, or None if it is only a bare name.

    _clip_ref_basename deliberately throws the directory away, because a scene's clip is resolved
    inside the project's own folder. That is right for a clip that lives there and wrong for one
    dragged in from somewhere else: the basename alone silently resolves to whatever same-named
    file the target project happens to contain, or to nothing at all. Callers that need to COPY
    the source in need the whole path, so they get it here.
    """
...
class SceneClip:
    ...
    def frame(self, t, hold_last=False):
        if self.duration > 0:
            # Never wrap a short source back to its beginning. Short clips are rejected by the
            # render preflight above; clamping here is only a last-frame rounding safeguard.
    ...
    def frame_interpolated(self, t, hold_last=False):
        ...
        """
        if self.duration > 0:
            # Modulo sampling visibly replayed short clips until a timeline slot was full.
            # Sampling is deliberately monotonic and non-wrapping.
            t = clamp(float(t), 0.0, max(0.0, self.duration - 1.0 / max(self.fps, 1.0)))
...
def find_ffmpeg():
    ...
    if local:
        # Winget's Gyan.FFmpeg package is the normal Windows installation used
        # by Shortslab.  It is not always added to PATH, so relying only on
        # ``shutil.which`` made fresh Clip Short/V4 runs fail before they had
        # even started inspecting media.
        winget = Path(local) / "Microsoft" / "WinGet" / "Packages"
        if winget.exists():
            candidates.extend(winget.glob("Gyan.FFmpeg_*/*/bin/ffmpeg.exe"))
    ...
    # Some desktop installs keep CapCut next to the workspace rather than
    # below LOCALAPPDATA. Its bundled ffmpeg is valid for Shortslab rendering.
    workspace_capcut = ROOT.parent / "capcut"
    if workspace_capcut.exists():
        candidates.extend(workspace_capcut.rglob("ffmpeg.exe"))
    bundled_runtime = ROOT / "tools" / "runtime"
    if bundled_runtime.exists():
        candidates.extend(bundled_runtime.rglob("ffmpeg.exe"))
    seen = []
    for candidate in candidates:
...
# The folder of hand-picked transition sounds. Everything a cut, a shot change or a caption pop
# fires comes from here, so the whole short shares one deliberate sonic vocabulary instead of
# whatever the keyword matcher happened to find.
TRANSITION_SFX_DIRNAME = "transitionsfx"
_SFX_ANCHOR_CACHE = {}
_CURATED_ANNOUNCED = set()


def sfx_transient_anchor(path, ffmpeg=None):
    """Seconds from the START of an SFX file to the moment a listener hears it LAND.

    A transition sound was placed with its file start on the cut, which puts the actual hit
    anywhere from 14 to 164 milliseconds late - measured across the transition folder, worst on
    flip-fx.mp3 - while whooshes got a flat 60 ms of lead regardless of their real shape. Neither
    number was measured from the audio. Subtracting this anchor from the cut time puts the hit ON
    the cut: the bubble pops exactly when the picture changes.

    Percussive sounds (a pop, a click) are anchored to their ATTACK, because that is the instant
    the ear timestamps. Swells (a whoosh, a flip) are anchored to their PEAK, because a whoosh is
    meant to arrive at the cut rather than start on it. The two are told apart by how fast the
    envelope climbs: meme-click.mp3 peaks 116 ms in but attacks at 14 ms, and anchoring it to the
    peak would drag the click a tenth of a second early.
    """
    key = str(path)
    if key in _SFX_ANCHOR_CACHE:
        return _SFX_ANCHOR_CACHE[key]
    anchor = 0.0
    try:
        ffmpeg = ffmpeg or find_ffmpeg()
        rate = 48000
        ...
        if samples.size > int(rate * 0.02):
            hop, win = int(rate * 0.002), int(rate * 0.006)
            count = (samples.size - win) // hop + 1
            if count >= 3:
                frames = np.lib.stride_tricks.sliding_window_view(samples, win)[::hop][:count]
                env = np.sqrt(np.maximum(0.0, (frames.astype(np.float64) ** 2).mean(axis=1)))
                peak_i = int(np.argmax(env))
                peak = float(env[peak_i])
                if peak > 0:
                    def first_at(fraction, before=None):
                        window = env[:before] if before else env
                        hit = np.where(window >= peak * fraction)[0]
                        return float(hit[0] * hop / rate) if hit.size else 0.0
                    # ONSET, NOT "A FIFTH OF THE PEAK". The first threshold was 0.20 of peak,
                    # which is roughly -14 dB and lands well after a sound has become audible:
                    # measured across the transition folder it read transicion-1.mp3 at 37 ms
                    # against a real onset near 10 ms, and ui-pop-a-dop at 28 against 10. -30 dB
                    # is the usual onset definition, and searching only BEFORE the peak stops a
                    # long decay tail from being mistaken for the start.
                    onset_floor = 10.0 ** (-30.0 / 20.0)
                    attack = first_at(onset_floor, before=max(1, peak_i + 1))
                    half = first_at(0.50)
                    percussive = (half - attack) <= 0.045
                    anchor = attack if percussive else float(peak_i * hop / rate)
    except Exception:      # noqa: BLE001 - an unmeasurable file just keeps the old behaviour
        anchor = 0.0
    # A quarter second of lead is already suspicious; beyond that trust the file start.
    anchor = max(0.0, min(0.25, float(anchor)))
    _SFX_ANCHOR_CACHE[key] = anchor
    return anchor


def transition_sfx_files(root):
    """Every sound in the curated transition folder, sorted for a stable rotation."""
    folder = Path(root) / TRANSITION_SFX_DIRNAME
...
SFX_TRANSITION_CATEGORIES = {
    "analog_transitions", "subtle_transitions", "bright_whoosh", "swipe_whoosh",
    "caption_pop", "ui_click", "whoosh_hit_combo", "motion_whoosh_like",
    # A click at a cut is a cut sound. Left out, the SFX Master's "click" category resolved to an
    # empty folder on this install and could never place anything at all.
    "cuts_clicks_ui",
}
...
def sfx_category_files(config, category):
    ...
    # THE CURATED FOLDER WINS. Every cut, shot change and caption pop is supposed to come from
    # soundeffects/transitionsfx, which is picked by hand; the classified library underneath it
    # holds hundreds of sounds and a single misfiled ding turns a cut into a doorbell. If the
    # folder exists it IS the answer for transition categories, with no fallback mixed in.
    if category in SFX_TRANSITION_CATEGORIES:
        curated = transition_sfx_files(root)
        if curated:
            # Say it out loud once per category per process. "The cut sounds are wrong" is
            # otherwise indistinguishable from "the app is running yesterday's code", and the
            # difference took a round trip to establish.
            if category not in _CURATED_ANNOUNCED:
                _CURATED_ANNOUNCED.add(category)
                print(f"[sfx] {category}: using {len(curated)} curated sound(s) from "
                      f"{root / TRANSITION_SFX_DIRNAME}")
            return curated
    ...
    for ev in ([] if is_scrape else plan_sfx_events(config, has_speech)):
        ...
        if ov.get("start_abs") is not None:
            try:
                ...
                pass
        # Line the audible HIT up with the cut, not the file's first sample: a transition sound
        # starts quiet and its transient arrives 8-14ms in (250ms for a riser), so playing it at
        # the cut time puts the audible moment after the cut, every time.
        #
        # This used to be skipped whenever the payload carried a start_abs, on the reasoning that
        # a sound the user dragged by hand should stay exactly where they put it. But the editor
        # writes start_abs for EVERY sound it round-trips, moved or not - so once the timeline
        # editor became the normal path, the correction stopped happening at all and every
        # transition sat late. `start_abs_user` is set only where a person actually moved one.
        if is_transition and not ov.get("start_abs_user"):
            start_at = round(max(0.0, start_at - sfx_transient_anchor(path)), 3)
...
def render_video(config, basename=None):
    ...
    if captions_enabled and config.get("timeline_editor_render"):
        ...
        try:
            ...
            caption_track = [
                {"start": row.get("start", 0), "end": row.get("end", 0),
                 "text": row.get("text", ""),
                 # Redo-captions stores sentence-local word boundaries here.  Keep them:
                 # replacing these with an empty list made the renderer reload the legacy
                 # voice/word_timestamps cache, whose timestamps may describe the unsped
                 # voice.  The resulting error grows throughout a speed-adjusted voiceover.
                 "word_timings": list(row.get("word_timings") or [])}
        ...
        if timeline_caption_chunks and bool(config.get("export_caption_pngs", True)):
            try:
                hook_end = (float(config["scenes"][0].get("end", 0.0))
                            if config.get("scenes") else 0.0)
                count = len(export_timeline_caption_pngs(
                    config, timeline_caption_chunks, width, height,
                    render_dir.parent / "captions", hook_end=hook_end))
                status_log(status_cb, f"Saved {count} synchronized caption PNG(s) to captions/.")
            except Exception as exc:
    ...
    editorial_enabled = editorial_quality.enabled(config)
    editorial_cut_cache = {}
    editorial_used_ranges = {}
    config["editorial_preflight"] = []
    if use_clips:
        ...
        for i, scene in enumerate(config["scenes"], 1):
            ...
            if path and path.exists():
                ...
                if (config.get("clip_source") == "scrape" and identity == previous_identity
                        and scene.get("seedance_start_trim") is None):
                    offset = previous_offset + previous_consumed
                else:
                    offset = 0.0
                ...
                speed_factor = 1.0
                # A slightly short source may be slowed down, but never below 0.8x. Replaying the
                # beginning or appending a static tail is forbidden: a larger shortage must be
                # resolved upstream by another source or by splitting the scene into real shots.
                if usable_duration + 0.08 < scene_duration:
                    safe_usable = max(0.0, usable_duration - (1.0 / max(clip_obj.fps, 1.0)))
                    required_speed = safe_usable / scene_duration
                    if required_speed >= 0.8:
                        speed_factor = min(1.0, required_speed)
                        status_log(status_cb, f"Scene {scene_id}: source is slightly short; using "
                                   f"{speed_factor:.2f}x instead of repeating footage.")
                    else:
                        # PRESSING RENDER RENDERS. This used to raise and throw the whole job
                        # away over one short clip - the user saw "Timeline repair required" and
                        # got no file at all, which is worse than a slightly slow shot.
                        # The clip is slowed as far as it needs to go and the shortfall is
                        # reported, so it can still be fixed in the editor afterwards.
                        # Footage is still never looped and no still is appended.
                        speed_factor = max(0.05, min(1.0, required_speed))
                        status_log(status_cb, f"Scene {scene_id}: clip is {usable_duration:.2f}s "
                                   f"for a {scene_duration:.2f}s slot - playing it at "
                                   f"{speed_factor:.2f}x. That is below the usual 0.8x floor, so "
                                   "this shot will look slow; replace or split it in the editor "
                                   "if you want it tighter.")
                clip_speed_factors[scene_id] = speed_factor
                window_start = max(0.0, start_trim + offset)
                window_end = min(clip_obj.duration, window_start + scene_duration * speed_factor)
                if editorial_enabled:
                    cut_key = str(path.resolve())
                    if cut_key not in editorial_cut_cache:
                        editorial_cut_cache[cut_key] = editorial_quality.scan_cuts(path, find_ffmpeg())
                    # A declared multi-shot scene keeps its intended internal edits.
                    source_cuts = editorial_cut_cache[cut_key]
                    planned_cuts = [window_start + float(shot.get("at", 0)) * speed_factor
                                    for shot in scene.get("shots", [])]
                    inspected = (None if source_cuts is None else
                                 [c for c in source_cuts if not any(abs(c - t) < .06 for t in planned_cuts)])
                    repaired_start, repaired_speed, result = editorial_quality.repair_window(
                        window_start, window_end, inspected, scene_duration)
                    if result == "needs_replacement":
                        status_log(status_cb, f"Scene {scene_id}: checking up to three clean alternatives in this source...")
                        replacement = editorial_quality.reselect_window(
                            scene, path, window_start, window_end, source_cuts, scene_duration, config,
                            render_dir / f"{basename}_editorial" / f"repair_{i}",
                            occupied=editorial_used_ranges.get(identity, []))
                        if replacement:
                            repaired_start, repaired_speed = replacement["start"], replacement["speed"]
                            result = "reselected"
                    editorial_used_ranges.setdefault(identity, []).append(
                        (repaired_start, repaired_start + scene_duration * repaired_speed))
                    config["editorial_preflight"].append({"scene": str(scene_id), "status": result,
                        "original_start": window_start, "original_end": window_end,
                        "played_start": repaired_start,
                        "played_end": repaired_start + scene_duration * repaired_speed})
                    if result in {"repaired", "reselected"}:
                        start_trim = repaired_start - offset
                        speed_factor = repaired_speed
                        clip_speed_factors[scene_id] = speed_factor
                        window_start, window_end = repaired_start, repaired_start + scene_duration * speed_factor
                        status_log(status_cb, f"Scene {scene_id}: {result} source window; "
                                   f"using {window_start:.3f}-{window_end:.3f}s at {speed_factor:.3f}x.")
                ...
                if stall >= 0.15:
                    # PRESSING RENDER RENDERS. A defect is reported, never a reason to hand the
                    # user nothing: a short with one laggy shot can be fixed in the editor, a
                    # refused render cannot be fixed at all.
                    if status_cb:
                        status_cb(f"Render warning: scene {scene_id} has a {stall:.2f}s "
                                  "repeated-frame stall - rendering anyway; retrim or replace it "
                                  "in the editor if it shows.")
                ...
                if cadence_hitches:
                    ...
                    if status_cb:
                        status_cb(f"Render warning: scene {scene_id} has isolated duplicate-frame "
                                  f"hitches at {shown} - rendering anyway.")
                clips[scene_id] = clip_obj
                clip_start_trims[scene_id] = start_trim
                previous_identity = identity
                previous_offset = offset
                previous_consumed = scene_duration * speed_factor
            else:
                previous_identity = None
    ...
    try:
        ...
        for frame_no in range(total_frames):
            ...
            if use_clip_frame:
                ...
                speed_factor = clip_speed_factors.get(scene_id, 1.0)
                ...
                # SceneClip never loops. The preflight rejects genuinely short sources; this flag
                # only makes the non-wrapping intent explicit at the final sampling call.
                _hold_last = True
    ...
    # Subtitles first, then publish: renders.publish carries a same-stem .srt along with the
    # video, so the pair stays together in .renders too.
    write_render_subtitles(config, output, status_cb=status_cb)
    # Every finished render also lands in ONE folder, so a delivered video can be found without
    # knowing which mode wrote it or how that mode names its output directory.
    import renders
    editorial_publishable = True
    if editorial_enabled:
        status_log(status_cb, "Editorial review: checking finished cuts, repeats and action evidence...")
        try:
            quality = editorial_quality.audit_export(config, output, ffmpeg)
            editorial_publishable = quality.get("status") == "passed"
            status_log(status_cb, f"Editorial review: {quality['status']}; "
                       f"{len(quality['repair_queue'])} scene(s) need attention.")
        except Exception as exc:
            editorial_publishable = False
            quality = {"status": "unverified", "error": type(exc).__name__}
            report_path = output.with_suffix(".editorial.json")
            report_path.write_text(json.dumps(quality, indent=2), encoding="utf-8")
            config["editorial_quality"] = dict(quality, report=str(report_path))
            status_log(status_cb, f"Editorial review unavailable: {type(exc).__name__}; export is unverified.")
    if editorial_publishable:
        renders.publish(output, project=config.get("project_slug"), status_cb=status_cb)
    else:
        status_log(status_cb, "Editorial review did not pass; render kept in the project for "
                              "repair and withheld from .renders.")
    return output


def render_word_timings(config):
    """Every spoken word of a finished render with its time ON THE VIDEO CLOCK.

    The scenes carry forced-aligned word timings in scene-local seconds - the same ones the
    caption renderer uses - so shifting each by its scene start gives the render clock directly.
    A timeline-edited render may have moved its clips around after those were written, and there
    the canonical answer is audio_analysis.json, which is re-derived from the edited voiceover.
    """
    words = []
    for scene in sorted((config.get("scenes") or []),
                        key=lambda s: float(s.get("start") or 0.0)):
        offset = float(scene.get("start") or 0.0)
        for entry in (scene.get("word_timings") or []):
            text = str((entry or {}).get("word") or "").strip()
            if not text:
                continue
            start = offset + float(entry.get("start") or 0.0)
            words.append({"word": text, "start": start,
                          "end": offset + float(entry.get("end") or start)})
    if words and not config.get("timeline_editor_render"):
        return words
    try:
        _, asset_dir, _, _ = project_paths(config)
        analysis = json.loads((asset_dir.parent / "input" / "audio_analysis.json")
                              .read_text(encoding="utf-8"))
    except Exception:                                                   # noqa: BLE001
        return words
    canonical = []
    for row in (analysis.get("sentence_timestamps") or []):
        for entry in (row.get("word_timings") or []) if isinstance(row, dict) else ():
            text = str((entry or {}).get("word") or "").strip()
            if not text:
                continue
            start = float(entry.get("start") or 0.0)
            canonical.append({"word": text, "start": start,
                              "end": float(entry.get("end") or start)})
    canonical.sort(key=lambda w: w["start"])
    return canonical or words


def write_render_subtitles(config, output, status_cb=None):
    """Write a .srt beside a finished render, cut by the same rules every mode uses.

    Asked for by name for the clip shorts: a delivered video should arrive with the text of
    what is said in it, not only burned-in caption pixels that nothing can read back.
    """
    import subtitles as srt_rules
    try:
        words = render_word_timings(config)
        if not words:
            status_log(status_cb, "Subtitles skipped: this render has no word timings "
                                  "(no voiceover, or alignment never ran).")
            return None
        cues = srt_rules.subtitle_cues(words)
        path = srt_rules.write_srt(cues, Path(output).with_suffix(".srt"))
        status_log(status_cb, f"Subtitles written: {path.name} ({len(cues)} cue(s)).")
        return path
    except Exception as exc:                                            # noqa: BLE001
        # A finished video must never be lost to a subtitle problem.
        status_log(status_cb, f"Subtitles skipped ({type(exc).__name__}: {exc}).")
        return None
