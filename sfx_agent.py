...
AGENT_SFX_CATEGORIES = {
    "transition": {"lib": "subtle_transitions", "vol": 0.146, "max_dur": 0.9,
                   "desc": "soft whoosh/slide for a cut or image change"},
    # transition and whoosh both fire on or into a cut, so both read from the curated
    # soundeffects/transitionsfx folder via sfx_category_files - one vocabulary for the whole
    # video. impact/boom/reaction stay on the classified library: they are content, not cuts.
    "whoosh":     {"lib": "motion_whoosh_like", "vol": 0.157, "max_dur": 0.9,
                   "desc": "motion whoosh for fast movement, a swipe, or a reveal pushing in"},
    "impact":     {"lib": "subtle_impacts",     "vol": 0.168, "max_dur": 1.0,
                   "desc": "subtle impact to punctuate an important word"},
    "boom":       {"lib": "hits_impacts",       "vol": 0.224, "max_dur": 1.4,
                   "desc": "stronger hit/boom for a dramatic reveal, shock, or hard fact"},
    "stinger":    {"lib": "jingles_stingers",   "vol": 0.168, "max_dur": 1.6,
                   "desc": "short musical stinger to mark a punchline or twist"},
    "riser":      {"lib": "subtle_tones",       "vol": 0.134, "max_dur": 2.2,
                   "desc": "tension riser / tone building anticipation before a payoff"},
    "foley":      {"lib": "serious_foley",      "vol": 0.168, "max_dur": 1.4,
                   "desc": "diegetic foley matching an on-screen action or object"},
    "click":      {"lib": "cuts_clicks_ui",     "vol": 0.123, "max_dur": 0.5,
...
def resolve_vision_segments(events, data, duration, existing_onsets=None, sfx_amount="medium",
    ...
    # THIS PATH READS THE HUMAN-LABELLED LIBRARY, NOT sfx_category_files, so restricting the
    # curated folder there did not reach Redo SFX in the timeline editor - it kept casting cut
    # sounds from every labelled whoosh and click in the collection. Cuts and clicks are the same
    # vocabulary everywhere; impacts and reactions stay on the labelled library because they are
    # content, chosen for what is happening on screen.
    _curated = [str(f) for f in pipeline.transition_sfx_files(
        pipeline.sfx_library_root({}) or pipeline.default_sfx_root())]
    # the OPENING sound must be a dedicated hook_riser file - never a generic body riser
    hook_pool = data.get("hook_risers") or []
    reactions = data.get("reactions") or {}
    if _curated:
        # "Only these sounds" means only these. Cuts, clicks, impacts AND reactions all draw from
        # the curated folder - a reaction sound is exactly the "some other SFX" that kept turning
        # up after the cut categories alone had been narrowed. This has to run AFTER `reactions`
        # is read from the data: written above it, the comprehension referenced a local that did
        # not exist yet and the whole Master died with UnboundLocalError.
        # The dedicated hook riser keeps its own pool: a riser is a build-up, not a cut sound,
        # and the curated folder contains none.
        transition_pool = list(_curated)
        ui_pool = list(_curated)
        impact_pool = list(_curated)
        reactions = {key: list(_curated) for key in reactions}
    ...
    for ev in (_structural + _reactions):
        ...
        # The agent picks the MOMENT, not the file offset. Placing the file start there put the
        # audible hit late by whatever lead-in the sound carries, so a pop landed after the cut it
        # was chosen for. Subtracting the file's measured transient anchor lands the hit on the
        # moment; for a sound with no lead-in the anchor is a couple of milliseconds and nothing
        # moves.
        _at = max(0.0, round(t - pipeline.sfx_transient_anchor(path), 3))
        segments.append({"path": Path(path), "start": _at,
