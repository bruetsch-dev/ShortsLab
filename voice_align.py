...
# os.add_dll_directory returns a HANDLE, and dropping it removes the directory from the search
# path again when it is collected. The model then builds on cuda (that needs no cuBLAS) and dies
# at the first inference with "cublas64_12.dll is not found" - which reads exactly like the
# library being absent. Hold the handles for the life of the process.
_CUDA_DLL_HANDLES = []


def _register_cuda_dll_dirs(status_cb=None):
    """Put the pip-installed CUDA runtime on the DLL search path (Windows).

    ctranslate2 needs cuBLAS and cuDNN beside it, and the nvidia-*-cu12 wheels drop those DLLs
    into site-packages/nvidia/*/bin - a directory Windows does not search. Without this the GPU
    attempt fails with "Library cublas64_12.dll is not found or cannot be loaded", every
    alignment silently falls back to CPU, and a timeline render spends minutes where it could
    spend seconds. Safe to call when the wheels are absent: it simply finds nothing.
    """
    ...
    try:
        ...
        roots = []
    for root in roots:
        for sub in sorted(Path(root).glob("*/bin")):
            try:
                if not any(sub.glob("*.dll")):
                    continue
                # BOTH mechanisms are needed. add_dll_directory only covers loads that ask for
                # the augmented search order; ctranslate2 pulls cuBLAS in lazily by bare name,
                # which searches PATH only - so the model builds happily on cuda and dies at the
                # first inference with "cublas64_12.dll is not found", which reads exactly like
                # the library being absent.
                _CUDA_DLL_HANDLES.append(os.add_dll_directory(str(sub)))
                if str(sub) not in os.environ.get("PATH", ""):
                    os.environ["PATH"] = str(sub) + os.pathsep + os.environ.get("PATH", "")
                added += 1
            except OSError:
                continue
    if added and status_cb:
        status_cb(f"Voice aligner: registered {added} CUDA library folder(s) for the GPU path.")
    return bool(added)
...
def _load_model(model_name=None, status_cb=None):
    ...
    with _MODEL_LOCK:
        ...
        if _MODEL is not None and _MODEL_KEY == key:
            return _MODEL
        _register_cuda_dll_dirs(status_cb)
...
def release_gpu_models():
    """Drop the cached ASR models and give their VRAM back to the card.

    The models stay resident for the whole run by design - alignment is called repeatedly. But
    ProPainter needs ~5-6 GiB for its own inference, and with whisper still holding its weights
    the card is short exactly that: every caption clean of the night died OOM at "5.5 GiB free,
    6.05 GiB allowed" while this cache sat on the rest. Alignment transparently reloads on the
    next call, so the only cost is one model load."""
    global _MODEL, _MODEL_KEY, _WX_MODEL
    with _MODEL_LOCK:
        _MODEL = None
        _MODEL_KEY = None
        _WX_MODEL = None
