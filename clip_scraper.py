...
try:
    import brightdata_tiktok         # paid keyword fallback when TikTok search hits daily limit
except Exception:  # pragma: no cover - optional credential/integration
    brightdata_tiktok = None
try:
    import brightdata_instagram     # Instagram reels by ACCOUNT - the API offers no keyword search
except Exception:                                                       # noqa: BLE001
    brightdata_instagram = None
...
# ---- Search/download BACKENDS: the logged-in TikTok session, plus Instagram Reels ----
# TikTok (tiktok_login.py) is the primary backend; Instagram (instagram_login.py) is searched
# alongside it when its login is saved. Both return items in the same schema that _item_meta
# understands, then yt-dlp downloads the mp4 using cookies exported from the respective session.
# No API keys involved. X/Twitter was removed on 2026-09-03: it had stopped returning usable
# video, and every query spent against it was a query not spent on TikTok.
...
def normalize_platforms(platforms=None):
    ...
    # A saved project or preset can still name "twitter"/"x" in its platform list. Those names
    # resolve to nothing now rather than to an unknown backend, so an old project simply runs on
    # TikTok instead of asking for a platform that no longer exists.
    requested = {str(p or "").strip().lower() for p in (platforms or ("tiktok",))}
    out = set()
...
def set_brightdata_only(enabled=True):
    """Run entirely on the paid API, with no logged-in browser sessions at all."""
    global BRIGHTDATA_ONLY
    BRIGHTDATA_ONLY = bool(enabled)
    return BRIGHTDATA_ONLY


def set_instagram_accounts(accounts):
    """The creators this run will pull Instagram reels from. Resets the cached pool."""
    global _IG_ACCOUNTS, _IG_POOL
    # `if str(a).strip()` is not the same test: str(None) is the four-character string "None",
    # which is truthy, and would have been sent to the API as a handle.
    _IG_ACCOUNTS = [str(a).strip() for a in (accounts or ())
                    if a is not None and str(a).strip()]
    _IG_POOL = None
    _IG_SERVED.clear()
    return list(_IG_ACCOUNTS)


def instagram_pool(status_cb=None, per_account=7, timeout_s=300):
    """This run's Instagram reels, fetched once and reused for every query.

    Eight accounts at seven reels is 56 records - just inside the 60 the budget allows - and it is
    deliberately larger than it was: while TikTok refuses every download, this pool is the only
    footage the pipeline can actually fetch, so a thin pool means a chapter with nothing in it.
    """
    global _IG_POOL
    if _IG_POOL is not None:
        return _IG_POOL
    if brightdata_instagram is None or not _IG_ACCOUNTS:
        _IG_POOL = []
        return _IG_POOL
    _IG_POOL = brightdata_instagram.reels_for_accounts(
        _IG_ACCOUNTS, per_account=per_account, status_cb=status_cb, timeout_s=timeout_s) or []
    return _IG_POOL
...
def backend_active(platforms=None):
    ...
    if BRIGHTDATA_ONLY:
        # There is no login to check in this mode; the API key IS the backend.
        return bool(brightdata_tiktok is not None and brightdata_tiktok.available())
...
_BRIGHTDATA_RECORDS_USED = 0
BRIGHTDATA_RECORD_BUDGET = 60
# A Bright Data call is only worth starting if the caller can still fetch what it returns, and no
# single call may swallow a chapter's whole clock: one 600s call inside a 150s chapter is how 30
# paid records ended up discarded unfetched.
# Measured, not guessed: a discover_new keyword job submitted at t+0 reported `running` until
# t+103 and `ready` at t+111, then returned 10 clean records. A 120s cap therefore killed every
# call a few seconds before it paid out - a whole evening of "received 0 usable record(s)" from
# an API that was working the entire time. Give it real headroom.
# When the login backends are off, every result comes from the paid API. This is a deliberate
# operating mode, not a fallback: the browser sessions are skipped entirely, so nothing waits on
# a session refresh, a captcha, or TikTok's daily search ceiling.
BRIGHTDATA_ONLY = str(os.environ.get("SCRAPE_BACKEND", "")).strip().lower() == "brightdata"

# Instagram discovery is account-based (the keyword and hashtag datasets are not collectible on
# this account), so a run names the creators who cover its topic ONCE and every query is served
# from that one paid pull. Fetching per query would multiply the bill by the number of queries
# for the same handful of accounts.
_IG_ACCOUNTS = []
_IG_POOL = None
# Which pooled reels have already been offered this run. Without it the SAME pool came back for
# every query - 135 times in one measured run - so the round-robin that hands out download slots
# saw those reels in every bucket and gave them every slot: 30 of 30 downloaded proxies were
# pooled Instagram reels and not one TikTok keyword hit was ever fetched.
_IG_SERVED = set()

BRIGHTDATA_MIN_REMAINING_S = 130.0
BRIGHTDATA_CALL_TIMEOUT_S = 240.0
X_QUERY_STOPWORDS = {
...
def reset_backend_search_health():
    global _IG_ZERO_STREAK, _IG_UNAVAILABLE
    global _IG_SERVED
    global _BRIGHTDATA_RECORDS_USED
    _SEEN_QUERIES.clear()               # fresh run -> allow every query once again
    _IG_ZERO_STREAK = 0
    _IG_UNAVAILABLE = False
    _BRIGHTDATA_RECORDS_USED = 0
    for mod in (tiktok_login, instagram_login):
        if mod is not None:
            try:
...
def _session_cookie_text(module):
    """The login cookies for one platform, from the live session or the last export on disk.

    `export_cookies_txt` returns None whenever the live session object cannot hand its cookies
    over in this process - and it then discards a perfectly good `cookies.txt` already written by
    an earlier export. Measured 2026-09-02: the TikTok file on disk held all five session cookies
    while the merge returned nothing, so yt-dlp ran ANONYMOUSLY through every V4 run and met
    TikTok's bot-check page instead of the video. A stale cookie simply fails the same way it
    would have anyway, so reading the file back can only help.
    """
    try:
        exported = module.export_cookies_txt()
        if exported and Path(exported).exists():
            return Path(exported).read_text("utf-8")
    except Exception:                                                   # noqa: BLE001
        pass
    try:
        saved = Path(getattr(module, "COOKIES_TXT", "") or "")
        if not saved.is_file():
            return ""
        text = saved.read_text("utf-8", errors="ignore")
        names = {line.split("\t")[5] for line in text.splitlines()
                 if not line.startswith("#") and len(line.split("\t")) >= 7}
        wanted = set(getattr(module, "SESSION_COOKIE_NAMES", ()) or ())
        return text if (wanted and names & wanted) else ""
    except Exception:                                                   # noqa: BLE001
        return ""
...
def _merge_backend_cookies(platforms=None):
    ...
    selected = normalize_platforms(platforms)
    parts = []
    for name, module, ready in (("tiktok", tiktok_login, tiktok_backend_ready),
                                ("instagram", instagram_login, instagram_backend_ready)):
        if name not in selected or module is None or not ready():
            continue
        text = _session_cookie_text(module)
        if text:
            parts.append(text)
    if not parts:
        return None
...
def _instagram_hashtag_fallback(query):
    """Use the strongest native concept as an IG tag; keyword search often returns Home feed."""
    tokens = re.findall(r"[\u3040-\u30ff\u3400-\u9fff]{2,}", str(query or ""))
    if not tokens:
        return ""
    # Prefer a concrete token over generic geography/school scaffolding.
    generic = {"日本", "学校", "生徒", "東京"}
    token = next((value for value in tokens if value not in generic), tokens[0])
    fallback = "#" + token[:30]
    # A query that is ALREADY that tag has nothing to fall back to. Without this, every
    # zero-result Japanese hashtag was searched a second time with the identical string:
    # "#名頃かかしの里" produced the fallback "#名頃かかしの里". That is a wasted search per empty
    # query - on a backend whose empty results are what the health check is counting.
    if fallback.casefold() == str(query or "").strip().casefold():
        return ""
    return fallback
...
def backend_search(query, want, status_cb=None, sort="MOST_LIKED", platforms=None, deadline=None):
    ...
    # In Bright-Data-only mode no browser session is touched at all: no cookie refresh, no X or
    # Instagram worker thread, no waiting on a login wall. Everything below comes from the API.
    tt_ready = (_ensure_tiktok_cookies(status_cb, selected)
                if not BRIGHTDATA_ONLY and "tiktok" in selected
                and tiktok_backend_ready() else False)
    ig_future = None
    if not BRIGHTDATA_ONLY and "instagram" in selected and instagram_backend_ready():
    ...
    # TikTok returns status 2484 after the account's daily keyword budget is exhausted.  Bright
    # Data is a paid, keyword-capable fallback supplied by the user.  Keep a hard per-run record
    # ceiling so retry rounds cannot turn one Short into an open-ended bill.
    tt_daily_limited = False
    if tiktok_login is not None:
        try:
            tt_daily_limited = bool((tiktok_login.search_stats() or {}).get("daily_limit"))
        except Exception:
            pass
    # Normally this is the fallback for TikTok's daily ceiling. In Bright-Data-only mode it is
    # the primary source, so it runs whether or not the browser ever reported a limit.
    if ((BRIGHTDATA_ONLY or (not items and tt_daily_limited))
            and "tiktok" in selected
            and brightdata_tiktok is not None and brightdata_tiktok.available()
            and _BRIGHTDATA_RECORDS_USED < BRIGHTDATA_RECORD_BUDGET):
        allowance = min(10, int(want), BRIGHTDATA_RECORD_BUDGET - _BRIGHTDATA_RECORDS_USED)
        bd_remaining = 600.0 if deadline is None else max(0.0, deadline - time.monotonic())
        # Buying records there is no time left to DOWNLOAD is the worst of both worlds: the bill
        # is charged, the clock runs out, and the caller discards the results unseen. That is
        # exactly what happened on 2026-08-26 - 30 paid records, 0 clips fetched.
        if allowance > 0 and bd_remaining >= BRIGHTDATA_MIN_REMAINING_S:
            bd_items = brightdata_tiktok.search(
                query, limit=allowance, status_cb=status_cb,
                timeout_s=min(BRIGHTDATA_CALL_TIMEOUT_S, bd_remaining)) or []
            # Charge the local safety budget only for records actually returned. Bright Data may
            # bill fewer than requested; this tracks the conservative upper bound per run.
            # Bright Data has no server-side ordering, so "relevancy / most liked" cannot be
            # asked of the API - it is applied locally further down, and that sort keys on
            # _query_relevance. Browser results get that stamped by tiktok_login's scorer;
            # API results never passed through it, so they fell back to a made-up 0.5 and the
            # sort the user picked meant nothing for them. Same scorer, one scale.
            #
            # They are scored but NOT dropped the way browser results are: an evidence-free
            # browser page means TikTok served a stale feed, while these came back from an
            # explicit keyword query and were paid for. They rank last instead.
            if bd_items and tiktok_login is not None:
                blind = 0
                for candidate in bd_items:
                    score = float(tiktok_login._query_relevance(candidate, query) or 0.0)
                    candidate["_query_relevance"] = round(score, 4)
                    blind += 1 if score <= 0.0 else 0
                if blind:
                    _status(status_cb,
                            f"Bright Data: {blind}/{len(bd_items)} record(s) carry no keyword "
                            "evidence in their caption; they sort last.")
            _BRIGHTDATA_RECORDS_USED += max(len(bd_items), allowance if bd_items else 0)
            items.extend(bd_items)
            _status(status_cb, f"Bright Data budget: {_BRIGHTDATA_RECORDS_USED}/"
                               f"{BRIGHTDATA_RECORD_BUDGET} records used this run.")
    # Instagram, API-side, is a pool of reels from the creators this run named - there is no
    # keyword search to send. The pull is paid for ONCE and every query is served from it; asking
    # per query would multiply the bill by the query count for the same handful of accounts.
    # Scored on the same scale as everything else so the caller's sort still means something.
    if BRIGHTDATA_ONLY and "instagram" in selected:
        pool = instagram_pool(status_cb=status_cb)
        if pool:
            scored = []
            for candidate in pool:
                entry = dict(candidate)
                try:
                    score = float(tiktok_login._query_relevance(entry, query) or 0.0)                         if tiktok_login is not None else 0.0
                except Exception:                                       # noqa: BLE001
                    score = 0.0
                entry["_query_relevance"] = round(score, 4)
                scored.append((score, entry))
            scored.sort(key=lambda row: row[0], reverse=True)
            # Only reels that actually answer THIS query - and always BEHIND the keyword
            # results. An account pool is topical, not per-beat: creators post about the subject,
            # not the specific action a beat narrates. Measured on the first API-only run, every
            # beat-carrying reel from the pool was rejected by the vision grader ("someone
            # talking to camera, not b-roll") while TikTok's keyword search found the action
            # itself. So the pool supplements; it may not outrank a keyword hit.
            # Each pooled reel is offered ONCE per run, and only for a minority of a query's
            # slots. It is the same thirty reels whatever the query, so handing them out again
            # for every beat let them win every download slot the keyword search had paid for.
            keep = [entry for score, entry in scored
                    if score > 0.0 and str(entry.get("id") or "") not in _IG_SERVED]
            keep = keep[:max(1, int(want) // 4)]
            for entry in keep:
                _IG_SERVED.add(str(entry.get("id") or ""))
                entry["_query_relevance"] = round(
                    min(float(entry.get("_query_relevance") or 0.0), 0.35), 4)
            _status(status_cb, f"Bright Data Instagram: {len(keep)} fresh pooled reel(s) for "
                               f"{query!r} ({len(_IG_SERVED)}/{len(pool)} of the pool used, "
                               f"ranked behind keyword results).")
            items.extend(keep)
    if ig_future is not None:
        try:
            ...
            ig_items = []
        if not ig_items:
            ig_fallback = _instagram_hashtag_fallback(query)
            if ig_fallback and (deadline is None or time.monotonic() < deadline):
                _status(status_cb, f"Instagram: {query!r} had no relevant Reels; trying tag "
                                   f"{ig_fallback!r} once.")
                try:
                    retry_wait = 45.0 if deadline is None else max(0.1, deadline - time.monotonic())
                    ig_items = instagram_login.search_async(
                        ig_fallback, want=int(want), status_cb=lambda _message: None,
                        timeout_s=retry_wait, sort=sort).result(timeout=min(50.0, retry_wait + 5)) or []
                except Exception:
                    ig_items = []
        _status(status_cb, f"Instagram search for {query!r}: {len(ig_items)} relevant Reel(s).")
        if ig_items:
            _IG_ZERO_STREAK = 0
...
def _ytdlp_fetch(url, dest, status_cb=None, max_seconds=16.0):
    """Download a video URL to an exact path (clean mp4).

    `max_seconds` None or 0 fetches the WHOLE post. The 16-second default belongs to the old
    preview use; a V4 source is inspected across its whole length and its chosen window can sit
    at 37s, so a truncated file there is not a cheaper download, it is the wrong footage.
    """
...
def _tiktok_session_media_urls(item):
    """Return signed MP4 URLs already supplied by the authenticated TikTok feed.

    Search responses contain ``video.playAddr`` and bitrate variants.  Asking
    yt-dlp to reopen the public post page can fail even though the user's
    session has already received a valid signed media URL.  These URLs are
    therefore a download fallback, not a page resolver or a bypass.
    """
    video = item.get("video") if isinstance(item, dict) and isinstance(item.get("video"), dict) else {}
    urls, seen = [], set()
    def add(value):
        if isinstance(value, str):
            values = [value]
        elif isinstance(value, dict):
            values = value.get("UrlList") or value.get("url_list") or value.get("urlList") or []
    ...
    for rate in video.get("bitrateInfo") or []:
        if isinstance(rate, dict):
            add(rate.get("PlayAddr") or rate.get("playAddr"))
    return urls


def _tiktok_session_media_fetch(item, dest, status_cb=None):
    """Download one signed media URL emitted by the logged-in TikTok session."""
    urls = _tiktok_session_media_urls(item)
    if not urls:
        return None
    try:
        import requests
    ...
    referer = str(item.get("webVideoUrl") or "https://www.tiktok.com/")
    # The signed CDN URL is issued inside the logged-in page, but it still
    # checks the same first-party session cookies.  Pass only TikTok-domain
    # cookies already exported by the user's dedicated profile; never read an
    # everyday browser profile or persist credentials in a project.
    cookie_bits = []
    cookie_file = str(_COOKIES_SPEC or os.environ.get("COOKIES_FILE", "") or "").strip()
    try:
        for raw in Path(cookie_file).read_text("utf-8", errors="ignore").splitlines():
            line = raw[10:] if raw.startswith("#HttpOnly_") else raw
            if not line or line.startswith("#"):
                continue
            fields = line.split("\t")
            if len(fields) >= 7 and "tiktok" in fields[0].casefold() and fields[5] and fields[6]:
                cookie_bits.append(f"{fields[5]}={fields[6]}")
    except Exception:
        pass
        ...
        try:
            with requests.get(url, stream=True, timeout=(20, 180), headers=headers) as response:
                ...
                with open(temp, "wb") as handle:
                    for block in response.iter_content(1024 * 512):
                        if block:
                            handle.write(block)
            with open(temp, "rb") as handle:
                header = handle.read(64)
            if temp.stat().st_size > 64 * 1024 and b"ftyp" in header and b"<html" not in header.lower():
                temp.replace(dest)
                _status(status_cb, "TikTok download: used the signed media URL from the logged-in search session.")
                return dest
        except Exception:
            pass
...
def backend_download(item, dest, status_cb=None, max_seconds=16.0):
    """Download a candidate item. The public extractor first; the session only if it fails.

    SEARCH needs the login. DOWNLOAD does not, and has not for a while.
    A note here said "yt-dlp CANNOT fetch TikTok any more" on the strength of the capsule-hotel
    run (2026-09-03): 116 calls, 116 failures. That was true then, and it made the logged-in
    session the FIRST thing tried for every TikTok candidate - a single-threaded browser, a cookie
    export and a rate-limit risk per clip.

    Re-measured 2026-09-09 against eight post ids from a finished run, with no cookie file at all:
    8 of 8 resolved, and one downloaded whole - 30.3 MB, a real MP4 (ftypisom), in four seconds.
    The installed yt-dlp is 2026.08.19 and its TikTok extractor was last touched 2026-08-27, so
    the fix landed after the measurement that is quoted here, and nobody looked again. A dead
    measurement had become the architecture.
    """
    ...
    downloaded = None
    if url:
        downloaded = _ytdlp_fetch(url, dest, status_cb=status_cb, max_seconds=max_seconds)
    # Bright discovery records carry the original TikTok post URL.  Opening
    # that one known post inside the already-authorized persistent context is
    # fundamentally different from searching TikTok again: it does not spend a
    # keyword-search attempt and the player itself fetches its signed MP4.
    if not downloaded and url and tiktok_backend_ready():
        try:
            downloaded = tiktok_login.download_sync(url, dest, status_cb=status_cb)
            if downloaded and not _looks_like_video(downloaded):
                # The post session sometimes writes a body that is not a video at all - measured
                # at 6 of 25 downloads in one run, all random bytes rather than an HTML error
                # page. Returning it unchecked meant the signed-URL path below, which DOES check,
                # was never reached, so every one of those candidates was simply lost.
                _status(status_cb, "TikTok download: the post session wrote a body that is not "
                                   "a video; trying the signed media URL instead.")
                try:
                ...
                _status(status_cb, "TikTok download: captured media through the logged-in post session.")
        except Exception:
            downloaded = None
    return downloaded or _tiktok_session_media_fetch(item, dest, status_cb=status_cb)


def _looks_like_video(path):
    """Cheap container sniff - the same test the signed-URL path already applies."""
    try:
        target = Path(path)
    ...
    if b"<html" in lowered or b"<!doctype" in lowered:
        return False
    return any(sig in head for sig in (b"ftyp", b"moov", b"mdat", b"\x1aE\xdf\xa3", b"FLV"))


def close_backend():
    """Tear down the shared TikTok + Instagram sessions at the end of a run."""
    for mod in (tiktok_login, instagram_login):
        if mod is not None:
            try:
                mod.close_session()
...
def detect_letterbox(src, ffmpeg, start=0.0, seconds=2.0):
    """The crop that removes baked-in black bars, or "" when there are none worth removing.

    A very common upload is a landscape video placed inside a 9:16 canvas with black above and
    below. The clip is already 1080x1920, so scale+crop-to-fill is a no-op on it and the bars
    survive all the way into the finished Short. cropdetect finds them; this only accepts a
    result that actually removes a meaningful, roughly centred band, so ordinary dark footage is
    left alone.
    """
    try:
        res = subprocess.run(
            [ffmpeg, "-hide_banner", "-ss", str(max(0.0, start)), "-t", str(seconds),
             "-i", str(src), "-vf", "cropdetect=24:2:0", "-an", "-f", "null", "-"],
            capture_output=True, text=True, timeout=90, encoding="utf-8", errors="replace")
    except Exception:                                                   # noqa: BLE001
        return ""
    found = re.findall(r"crop=(\d+):(\d+):(\d+):(\d+)", res.stderr or "")
    if not found:
        return ""
    w, h, x, y = (int(v) for v in found[-1])
    # clip_scraper has its own prober; caption_remover's _dimensions is a different module.
    src_w, src_h = _probe_dims(src, _ffmpeg_tools()[1]) or (0, 0)
    if not src_w or not src_h or w <= 0 or h <= 0:
        return ""
    # Only act on a real band: under a tenth of the height this is noise, and a crop that is not
    # roughly centred is a moving subject rather than a border.
    if h >= src_h * 0.92:
        return ""
    if abs((src_h - h) - 2 * y) > src_h * 0.06:
        return ""
    if w < src_w * 0.5 or h < src_h * 0.25:
        return ""
...
def normalize_clip(src, out_path, ffmpeg, seconds=DEFAULT_CLIP_SECONDS, start=0.0):
    ...
    debar = detect_letterbox(src, ffmpeg, start=start)
    vf = (
        (debar + "," if debar else "")
...
def _caption_text_stroke_mask(frames, per_frame_boxes, allowed_regions=None):
    ...
    for frame, boxes in zip(frames, per_frame_boxes):
        ...
        for box in boxes:
            ...
            # COLOUR-AGNOSTIC INK. The two branches above only know white and yellow ink. The
            # pink-on-white caption in the Zauo footage matched neither, so the mask came out as
            # confetti - about a tenth of the glyph pixels, scattered - and the text stayed
            # perfectly readable straight through the blur. A caption stroke is a thin
            # high-contrast structure inside the OCR line, not a hue, so detect it
            # morphologically in BOTH polarities on luma AND on saturation: that covers dark ink
            # on light, light ink on dark, and coloured ink on a desaturated background alike.
            ...
            glyph |= resp >= max(18, int(np.percentile(resp, 88)))
            glyph_u8 = (glyph.astype(np.uint8) * 255)
            # Close BEFORE dilating: merging the strokes of one character into a solid blob is
            # what makes the blur unrecoverable. A blurred isolated stroke is still read by the
            # eye, which reconstructs the letter from its surviving edges.
            close = max(3, int(round(h * 0.18)) | 1)
            glyph_u8 = cv2.morphologyEx(glyph_u8, cv2.MORPH_CLOSE,
    ...
    if not int(np.count_nonzero(mask)):
        return None
    # COVER THE CONFIRMED LINE, NOT JUST ITS DETECTED STROKES. Masking individual glyphs was the
    # original design, on the argument that a covered box looks worse than covered letters. It
    # does - but no stroke detector finds every stroke: at a measured 73-82% coverage the missed
    # characters stayed sharp at the edges of the blur, and a caption that is partly readable is
    # the one outcome that is not allowed. These regions are OCR-confirmed caption lines, so the
    # line is covered outright, rounded off at the corners so it reads as a smudge rather than
    # as a hard rectangle. The glyph mask still contributes, catching ink outside the line box.
    # THE LINE IS AS WIDE AS ITS INK, NOT AS WIDE AS OCR READ IT. On a neon-gradient title the
    # recognizer returned only the first two characters of a line and none of the rest, at every
    # confidence from 0.35 down to 0.10, so half the title was covered and the other half stayed
    # perfectly readable. OCR is reliable about WHERE a caption line sits; it is not reliable
    # about how far the line runs. So take the y-band from OCR and the horizontal extent from the
    # colour-agnostic ink response, closed across the letter gaps: characters the recognizer
    # never returned are still ink on the same line.
    ink = None
    if allowed_regions:
        heights = [max(8, int(h)) for _x, _y, _w, h in allowed_regions]
        kk = max(3, int(round(sum(heights) / len(heights) * 0.55)) | 1)
        kern = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kk, kk))
        # MEASURE THE EXTENT ON THE TEMPORAL MEAN. Burned-in text - a caption or a watermark - is
        # identical in every frame, so it stays sharp when the frames are averaged while the
        # moving scene smears into mush. Measuring on a raw frame instead let sharp scene edges
        # dominate the ink map: on "Discover Zauo" the middle of the watermark was covered and
        # both ends stayed readable, because the faint overlay never outscored the counter and
        # kitchen behind it. On a locked-off shot the mean is just the frame again.
        # NO CLAHE HERE. It is right for the OCR pass, which needs a faint overlay lifted enough
        # to be recognised, and wrong for measuring extent: on a low-variance area it amplifies
        # noise into ink and the cover then ran to the frame edge. Morphological tophat is already
        # contrast-relative, so the plain mean is what the extent is measured on.
        flat = np.mean(np.stack([f.astype(np.float32) for f in frames]), axis=0).astype(np.uint8)
        g = cv2.cvtColor(flat, cv2.COLOR_BGR2GRAY)
        sat = cv2.cvtColor(flat, cv2.COLOR_BGR2HSV)[:, :, 1]
        resp = np.maximum(
            np.maximum(cv2.morphologyEx(g, cv2.MORPH_TOPHAT, kern),
                       cv2.morphologyEx(g, cv2.MORPH_BLACKHAT, kern)),
            np.maximum(cv2.morphologyEx(sat, cv2.MORPH_TOPHAT, kern),
                       cv2.morphologyEx(sat, cv2.MORPH_BLACKHAT, kern)))
        ink = (resp >= max(20, int(np.percentile(resp, 90)))).astype(np.uint8) * 255

    expanded = []
    for X, Y, W, H in (allowed_regions or []):
        ...
        if ink is not None:
            band = ink[Y0:Y1, :]
            # Close along the row so the characters of one line become one run, then keep only
            # the run that actually touches what OCR read - unrelated text elsewhere on the same
            # row (a shop sign beside the caption) stays out.
            gap = max(9, int(round((Y1 - Y0) * 1.2)) | 1)
            joined = cv2.morphologyEx(band, cv2.MORPH_CLOSE,
                                      cv2.getStructuringElement(cv2.MORPH_RECT, (gap, 3)))
            cols = ((joined > 0).mean(axis=0) > 0.12).astype(np.uint8)
            # The OCR box is known text, so it joins the run. Without this the walk started from
            # a column the ink map happened to call empty: on "Discover Zauo" the ink run ended
            # six pixels short of the box and the leftward walk stopped there, leaving "Dis"
            # readable beside a covered "cover Zauo".
            cols[X0:X1] = 1
            # Walk outward across GAPS, not just over solid ink. A caption line has spaces in it:
            # "Discover Zauo" stopped expanding at the space and both words stayed readable, one
            # covered in the middle and legible at each end. CLOSING the column profile bridges a
            # word gap; a plain dilation would also push the run past the outermost ink by the
            # kernel width, which spread the cover to the frame edge on a short centred line.
            # Pad before closing. A run that starts near the frame edge dilates past it, and the
            # erosion that should pull it back cannot shrink at the border - so the cover stuck
            # to column 0 and swallowed the left of the picture on a short centred line.
            pad = gap + 1
            padded = np.zeros(cols.size + 2 * pad, dtype=np.uint8)
            padded[pad:pad + cols.size] = cols
            padded = cv2.morphologyEx(padded.reshape(1, -1), cv2.MORPH_CLOSE,
                                      cv2.getStructuringElement(cv2.MORPH_RECT, (gap * 2 + 1, 1)))[0]
            cols = padded[pad:pad + cols.size] > 0
            lo, hi = X0, X1
            while lo > 0 and cols[lo - 1]:
                lo -= 1
            while hi < fw and cols[min(hi, fw - 1)]:
                hi += 1
            X0, X1 = lo, min(fw, hi)
    ...
    for X0, Y0, X1, Y1 in expanded:
        ...
        ph, pw = patch.shape[:2]
        r = max(1, int(min(ph * 0.35, pw * 0.12)))
        blob = np.zeros((ph, pw), dtype=np.uint8)
        if ph > 2 * r and pw > 2 * r:
            blob[r:ph - r, r:pw - r] = 255
            blob = cv2.dilate(blob, cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE, (2 * r + 1, 2 * r + 1)))
        else:
            blob[:, :] = 255
        np.maximum(patch, blob, out=patch)
    # A small feather hides mask edges but cannot expand into a rectangular patch.
    return cv2.GaussianBlur(mask, (0, 0), sigmaX=1.6)


def blur_caption_regions(path, ffmpeg, seconds=DEFAULT_CLIP_SECONDS, status_cb=None, info=None):
    ...
    if not bgr or _get_ocr() is None:
        return 0
    # A SEMI-TRANSPARENT WATERMARK IS INVISIBLE TO OCR, FRAME BY FRAME. "Discover Zauo" sat
    # across the middle of a shot, perfectly readable, and the recognizer returned nothing for it
    # at any confidence from 0.35 down to 0.10 - it is too low-contrast to read against a busy
    # scene. But it is IDENTICAL in every frame while the scene moves, so the temporal mean keeps
    # it sharp and smears everything else into mush; CLAHE then lifts what is left. On the frame
    # that prompted this, OCR goes from missing the watermark entirely to reading it at the
    # normal 0.35 confidence. On a locked-off shot the mean is just the frame again, so nothing
    # changes and nothing is lost.
    if len(bgr) >= 4:
        try:
            _mean = np.mean(np.stack([f.astype(np.float32) for f in bgr]), axis=0).astype(np.uint8)
            _lab = cv2.cvtColor(_mean, cv2.COLOR_BGR2LAB)
            _lab[:, :, 0] = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8)).apply(_lab[:, :, 0])
            bgr.append(cv2.cvtColor(_lab, cv2.COLOR_LAB2BGR))
        except Exception:      # noqa: BLE001 - an extra detection pass must never break the blur
            pass
    gh, gw = bgr[0].shape[:2]
    ...
    # COVER THE WHOLE GLYPH, NOT ITS CORE. A stroke blurred only where it was detected stays
    # perfectly readable - the eye reconstructs a letter from its edges, and the user reported
    # captions "komplett easy lesen" on footage this pass had already processed. Dilating the
    # glyph mask thickens each stroke and closes the gaps between neighbouring letters while
    # keeping the organic letter shape; that is the part that matters, because the padded
    # RECTANGLE this replaced looked worse than the caption it covered. The feather afterwards
    # keeps the blend from showing a hard edge.
    # BINARISE FIRST. maskedmerge blends proportionally, so a half-lit mask pixel keeps half the
    # original ink - and the half-lit pixels are exactly the glyph edges the eye reads a letter
    # from. Measured on the reported footage the mask covered ~78% of each caption line but only
    # ~65% at full strength, and the captions stayed legible. Hardening every touched pixel to
    # full opacity, then closing across the letter gaps, makes the interior solid; the feather
    # afterwards softens only the outer rim, so the cover still reads as a smudge, not a box.
    mask_full = ((mask_full > 24).astype(np.uint8)) * 255
    _close = max(3, int(round(vh * 0.010)) | 1)
    mask_full = cv2.morphologyEx(mask_full, cv2.MORPH_CLOSE,
                                 cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (_close, _close)))
    _grow = max(3, int(round(vh * 0.006)) | 1)          # ~0.6% of height, odd
    mask_full = cv2.dilate(mask_full, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (_grow, _grow)))
    _feather = max(3, int(round(vh * 0.004)) | 1)
    mask_full = cv2.GaussianBlur(mask_full, (_feather, _feather), 0)
    # A CLIP THAT NEEDS A THIRD OF ITS FRAME SMUDGED IS NOT WORTH RESCUING. Covering confirmed
    # caption lines outright is what finally made the text unreadable, but OCR also clusters
    # in-scene signage - a shop's own board, a parking sign - into caption-shaped lines. On a
    # storefront clip that meant the caption AND the signboard AND the faces beneath it went
    # under one smudge. Rather than tune the caption-vs-signage guess, refuse the whole clip:
    # returning 0 makes the caller drop its copy and treat the footage as unusable, which is the
    # right answer for something this plastered with text.
    # CALIBRATED, NOT GUESSED. The first ceiling was 0.22 and it refused 38% of a real 24-clip
    # sample from the capsule-hotel run - the same mistake as the old blanket caption rejection,
    # which once left twelve of fourteen beats borrowing one clip. Measured coverages were 22,
    # 23, 26, 26, 28, 30, 32, 33 and 48 percent: only the last is a clip actually plastered in
    # text, the rest are ordinary TikToks with several caption lines, where covering the text IS
    # the point. At 0.40 one clip in that sample is refused and the storefront cases that started
    # this (47%) still are.
    _covered = float((mask_full > 32).mean())
    if _covered > 0.40:
        _status(status_cb, f"Caption blur: {_covered:.0%} of the frame would have to be covered - "
                           f"the clip is text-plastered rather than captioned; leaving it alone.")
        # Say WHY this returned nothing. "0" otherwise reads as "no captions here", and the
        # caller then uses the clip untouched - with its captions fully legible, which is the
        # complaint this whole pass exists to answer. `info` lets the caller tell "clean" apart
        # from "too covered in text to save".
        if info is not None:
            info["refused"] = True
            info["covered"] = _covered
        return 0
    ...
    # maskedmerge works PER PLANE, so a gray mask against yuv420p video negotiates to yuv420p
    # with the mask's chroma planes at the neutral 128 - i.e. luma was replaced properly inside
    # the mask while COLOUR was blended at 50% across the whole frame. Pink-on-tan captions are
    # almost pure chroma, so the letters came through crisp on top of a blurred background: the
    # "still completely legible" report, and invisible to any luma-based check. Measured on that
    # footage: saturation structure inside the mask 13.1 with maskedmerge against 30.1 in the
    # original, versus 3.2 once the blend runs on every plane. alphamerge puts the mask in the
    # alpha channel and overlay blends Y, U and V by it, which is what this needs; yuv444p keeps
    # the mask edge from being halved by chroma subsampling.
    graph = ("[0:v]format=yuv444p,split=2[base][blur_src];"
             # sigma 40 left thin light-on-dark strokes recoverable; 80 destroys them.
             "[blur_src]gblur=sigma=80[blurred];"
             "[1:v]format=gray[mask];"
             "[blurred][mask]alphamerge[blurred_a];"
             "[base][blurred_a]overlay=format=yuv444[v]")
...
def scrape_bucket(out_dir, queries, want, bucket_id="", tier="exact", bucket_terms="",
    ...
    search_jobs = []
    max_queries = max(len(tiktok_queries), len(instagram_queries))
    platform_order = ("tiktok", "instagram")
    q_by_platform = {"tiktok": tiktok_queries, "instagram": instagram_queries}
