...
class ScriptCreatorVarietyTests(unittest.TestCase):
    ...
    def test_scraped_runs_are_warned_off_supporting_infrastructure(self):
        """Measured on a capsule-hotel run: every beat that matched was what visitors show off -
        the pod, the curtain, the corridor. Every beat that failed was supporting infrastructure -
        a wristband unlocking the lift, the gender-segregated floor, a suitcase chained in the
        luggage room. All of those are physical and filmable, which the old rule already asked
        for; none of them is what anyone points a camera at, which is what actually decides
        whether footage exists."""
        with tempfile.TemporaryDirectory() as tmp:
            history = Path(tmp) / "history.json"
            captured = []

            def fake_post(model, messages, max_tokens, temperature, timeout=180):
                # The footage rules live in the SYSTEM message, the topic in the user one.
                captured.append(chr(10).join(m["content"] for m in messages))
                return {"topic": "Capsules", "script": "One visible thing happens here every day. " * 12,
                        "hook_keywords": []}

            with patch.object(agent_core, "_SCRIPT_TOPIC_HISTORY", history),                  patch.object(agent_core, "_post_llm_json", side_effect=fake_post):
                agent_core.generate_viral_script("capsule hotels", footage_source="scrape")
            self.assertIn("PHYSICAL IS NOT ENOUGH", captured[0])
            self.assertIn("supporting infrastructure", captured[0].lower())

            captured.clear()
            with patch.object(agent_core, "_SCRIPT_TOPIC_HISTORY", history),                  patch.object(agent_core, "_post_llm_json", side_effect=fake_post):
                agent_core.generate_viral_script("capsule hotels", footage_source="generate")
            self.assertNotIn("PHYSICAL IS NOT ENOUGH", captured[0],
                             "a generated run renders anything and needs no footage-reality rule")

    def test_explainer_topics_keep_tiktok_as_illustrative_broll(self):
        captured = []

        def fake_post(model, messages, max_tokens, temperature, timeout=180):
            captured.extend(str(m.get("content") or "") for m in messages)
            return {"topic": "TSMC and Chinese chips",
                    "script": "Advanced chip factories make different generations of processors. " * 12,
                    "hook_keywords": []}

        topic = "How much better are TSMC chips than Chinese chips?"
        with tempfile.TemporaryDirectory() as tmp, \
             patch.object(agent_core, "_SCRIPT_TOPIC_HISTORY", Path(tmp) / "history.json"), \
             patch.object(agent_core, "_post_llm_json", side_effect=fake_post):
            self.assertTrue(agent_core.is_explainer_topic(topic))
            agent_core.generate_viral_script(topic, footage_source="scrape")

        prompt = "\n".join(captured)
        self.assertIn("EXPLAINER VISUAL REALITY", prompt)
        self.assertIn("TikTok", prompt)
        self.assertIn("not the proof", prompt)
        self.assertIn("ANALYSIS TOPIC", prompt)
        self.assertIn("TOPIC OVERRIDE", prompt)

    def test_everyday_topic_keeps_material_first_route(self):
        self.assertFalse(agent_core.is_explainer_topic("walking while eating street food"))
        self.assertTrue(agent_core.is_explainer_topic("Why does China threaten Taiwan?"))
